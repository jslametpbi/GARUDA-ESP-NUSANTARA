...
// ===== FINAL V14: approved professional Atlas, AI Tutor, and domain-only room links =====
const ROOM_BASE_URL_V14 = "https://room.garudaesp.id";
let editingRoomIdV14 = null;

function v14Hash(input=""){
  let h=2166136261;
  for(let i=0;i<input.length;i++){h^=input.charCodeAt(i);h=Math.imul(h,16777619);}
  const chars="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
  let out="";
  h=Math.abs(h);
  for(let i=0;i<4;i++){out+=chars[h%chars.length];h=Math.floor(h/chars.length);}
  return out || "A91K";
}
function v14MonthNames(){return ["January","February","March","April","May","June","July","August","September","October","November","December"];}
function v14TodayParts(){
  const d=new Date();
  return {day:String(d.getDate()).padStart(2,"0"),month:v14MonthNames()[d.getMonth()],year:String(d.getFullYear())};
}
function v14MonthNumber(month){
  const i=v14MonthNames().indexOf(month);
  return String((i>=0?i:0)+1).padStart(2,"0");
}
function v14DateISO(day,month,year){
  return `${year}-${v14MonthNumber(month)}-${String(day||"01").padStart(2,"0")}`;
}
function roomCodeFrom(title="",dateValue="",timeValue=""){
  let day,month,year;
  if(dateValue && /^\d{4}-\d{2}-\d{2}$/.test(dateValue)){
    const [y,m,d]=dateValue.split("-");
    year=y; day=d; month=v14MonthNames()[Number(m)-1]||"January";
  }else{
    const t=v14TodayParts(); day=t.day; month=t.month; year=t.year;
  }
  const dateCode=String(year).slice(-2)+v14MonthNumber(month)+String(day).padStart(2,"0");
  const suffix=v14Hash(`${title}|${dateCode}|${timeValue||"09:00"}`);
  return `RM-${dateCode}-${suffix}`;
}
function generateRoomLink(title="", dateValue="", timeValue=""){
  return `${ROOM_BASE_URL_V14}/${roomCodeFrom(title,dateValue,timeValue)}`;
}
function roomDateLabel(r){
  const day=r.day || (r.date?String(r.date).split("-")[2]:"");
  const month=r.month || (r.date?v14MonthNames()[Number(String(r.date).split("-")[1])-1]:"");
  const year=r.year || (r.date?String(r.date).split("-")[0]:"");
  const time=[r.startTime,r.endTime].filter(Boolean).join(" – ");
  return `${day} ${month} ${year}${time?` • ${time}`:""}${r.timezone?` ${r.timezone}`:""}`;
}
function syncRoomDateParts(){
  const day=String(byId("roomDay")?.value||"").padStart(2,"0");
  const month=byId("roomMonth")?.value||v14TodayParts().month;
  const year=byId("roomYear")?.value||v14TodayParts().year;
  const hidden=byId("roomDate");
  if(hidden) hidden.value=v14DateISO(day,month,year);
}
function refreshRoomLink(){
  syncRoomDateParts();
  const title=byId("roomTitle")?.value || "GARUDA ESP Room";
  const date=byId("roomDate")?.value || v14DateISO(v14TodayParts().day,v14TodayParts().month,v14TodayParts().year);
  const start=byId("roomStartTime")?.value || "09:00";
  const code=roomCodeFrom(title,date,start);
  if(byId("roomCode")) byId("roomCode").value=code;
  const finalLink=generateRoomLink(title,date,start); if(byId("roomLink")) byId("roomLink").value=finalLink; if(byId("roomLinkText")) byId("roomLinkText").textContent=finalLink;
}
function roomForm(room=null){
  const isEdit=!!room;
  editingRoomIdV14=room?.id||null;
  const t=v14TodayParts();
  let day=room?.day||t.day, month=room?.month||t.month, year=room?.year||t.year;
  if(room?.date && /^\d{4}-\d{2}-\d{2}$/.test(room.date)){
    const [y,m,d]=room.date.split("-");
    day=room.day||d; year=room.year||y; month=room.month||v14MonthNames()[Number(m)-1]||month;
  }
  const features=room?.features||["Video","Audio","Chat","Screen Share","Whiteboard","Breakout Rooms","Recording"];
  const featureOptions=["Video","Audio","Chat","Screen Share","Whiteboard","Breakout Rooms","Recording","Attendance","Consultation","Assessment Feedback"];
  setTimeout(()=>refreshRoomLink(),30);
  return `<section class="online-v14-card" id="roomFormPanel">
    <div class="online-v14-card-head">
      <div><h3>${isEdit?"Edit Virtual Room":"Create Virtual Room"}</h3><p>Fill in the complete meeting metadata and generate a professional room short link.</p></div>
      <button class="btn soft" onclick="resetRoomForm()">Reset</button>
    </div>
    <input id="roomDate" type="hidden" value="${esc(v14DateISO(day,month,year))}">
    <div class="online-v14-grid">
      <label>Meeting Title <span>*</span><input id="roomTitle" value="${esc(room?.title||"")}" placeholder="Week 5 – Technical Report Writing Workshop" oninput="refreshRoomLink()"></label>
      <label>Host / Facilitator <span>*</span><input id="roomHost" value="${esc(room?.host||"")}" placeholder="Dr. / Lecturer / Facilitator"></label>
      <label>Meeting Date <span>*</span><input id="roomDay" type="number" min="1" max="31" value="${esc(String(day).replace(/^0/,""))}" oninput="refreshRoomLink()"></label>
      <label>Month <span>*</span><select id="roomMonth" onchange="refreshRoomLink()">${v14MonthNames().map(m=>`<option ${m===month?"selected":""}>${m}</option>`).join("")}</select></label>
      <label>Year <span>*</span><select id="roomYear" onchange="refreshRoomLink()">${Array.from({length:8},(_,i)=>String(2026+i)).map(y=>`<option ${y===String(year)?"selected":""}>${y}</option>`).join("")}</select></label>
      <label>Start Time <span>*</span><input id="roomStartTime" type="time" value="${esc(room?.startTime||"09:00")}" oninput="refreshRoomLink()"></label>
      <label>End Time <span>*</span><input id="roomEndTime" type="time" value="${esc(room?.endTime||"10:30")}"></label>
      <label>Timezone <span>*</span><select id="roomTimezone">${["Asia/Jakarta (GMT+7)","WIB","WITA","WIT","UTC"].map(tz=>`<option ${room?.timezone===tz?"selected":""}>${tz}</option>`).join("")}</select></label>
      <label>Related ESP Course <span>*</span><select id="roomCourse">${disciplines.map(d=>`<option value="${d.id}" ${room?.courseId===d.id?"selected":""}>${esc(d.title)}</option>`).join("")}</select></label>
      <label>Related Week <span>*</span><select id="roomWeek">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${String(room?.week||"")===String(i+1)?"selected":""}>Week ${i+1}</option>`).join("")}</select></label>
      <label>Meeting Type <span>*</span><select id="roomType">${["Virtual Meeting (Live)","Speaking Practice","Writing Workshop","Assessment Feedback","Project Consultation","Lecturer Coordination","Student Support"].map(t=>`<option ${room?.type===t?"selected":""}>${t}</option>`).join("")}</select></label>
      <label class="room-code-box">Room Code <small>(Auto-generated)</small><input id="roomCode" value="${esc(room?.code||"")}" readonly></label>
      <label>Agenda <span>*</span><textarea id="roomAgenda" placeholder="Guided practice, consultation focus, expected output, and post-meeting task.">${esc(room?.agenda||"")}</textarea></label>
      <label>Notes <small>(Optional)</small><textarea id="roomNotes" placeholder="Preparation notes, materials, or attendance instructions.">${esc(room?.notes||"")}</textarea></label>
      <div class="feature-checks">
        <strong>Features <span>*</span></strong>
        ${featureOptions.map(f=>`<button type="button" class="${features.includes(f)?"selected":""}" onclick="this.classList.toggle('selected')">${esc(f)}</button>`).join("")}
      </div>
      <div class="room-link-panel">
        <div><small>Room Short Link (Auto-generated)</small><strong id="roomLinkText">${esc(room?.link||"")}</strong></div>
        <input id="roomLink" value="${esc(room?.link||"")}" readonly>
        <button class="btn soft" onclick="copyGeneratedRoomLink()">Copy</button>
        <button class="btn primary" onclick="shareGeneratedRoomLink()">Share</button>
      </div>
    </div>
    <div class="actions end">
      <button class="btn soft" onclick="previewRoomMetadata()">Preview Metadata</button>
      <button class="btn primary" onclick="saveRoom()">${isEdit?"Update Room":"Create Room"}</button>
      ${isEdit?`<button class="btn soft" onclick="editingRoomIdV14=null;renderOnline()">Cancel Edit</button>`:""}
    </div>
  </section>`;
}
function resetRoomForm(){editingRoomIdV14=null;renderOnline();toast("Reset","Room form has been reset.");}
function selectedRoomFeatures(){
  return [...document.querySelectorAll(".feature-checks button.selected")].map(b=>b.textContent.trim());
}
function copyGeneratedRoomLink(){
  refreshRoomLink();
  const link=byId("roomLink")?.value||"";
  if(!link) return toast("No link","Generate the room link first.");
  navigator.clipboard?.writeText(link).then(()=>toast("Copied","Professional room link copied.")).catch(()=>openModal(`<h2>Professional Room Link</h2><p>${esc(link)}</p>`));
}
function shareGeneratedRoomLink(){
  refreshRoomLink();
  const link=byId("roomLink")?.value||"";
...
function saveRoom(){
  ...
  const agenda=byId("roomAgenda").value.trim();
  const day=String(byId("roomDay").value||"").padStart(2,"0");
  const month=byId("roomMonth").value;
  const year=byId("roomYear").value;
  const startTime=byId("roomStartTime").value;
  const endTime=byId("roomEndTime").value;
  if(!title||!host||!agenda||!day||!month||!year||!startTime) return toast("Incomplete","Complete meeting title, host, date, month, year, start time, and agenda.");
  const old=editingRoomIdV14?db.rooms.find(r=>r.id===editingRoomIdV14):null;
  const payload={
    id:editingRoomIdV14||uid("ROOM"),
    title, host, agenda,
    day, month, year,
    date:v14DateISO(day,month,year),
    startTime,endTime,
    timezone:byId("roomTimezone").value,
    courseId:byId("roomCourse").value,
    week:byId("roomWeek").value,
    type:byId("roomType").value,
    code:byId("roomCode").value,
    link:byId("roomLink").value,
    scope:"all",
    notes:byId("roomNotes").value.trim(),
    features:selectedRoomFeatures(),
    attendance:old?.attendance||[],
    createdBy:old?.createdBy||session.name||session.role,
    createdAt:old?.createdAt||new Date().toISOString(),
    updatedAt:new Date().toISOString()
  };
  if(editingRoomIdV14){
    const i=db.rooms.findIndex(r=>r.id===editingRoomIdV14);
    if(i>=0) db.rooms[i]=payload;
    toast("Room updated","Professional online-room metadata has been updated.");
  }else{
    db.rooms.unshift(payload);
    toast("Room created","Professional short link and meeting metadata have been generated.");
  }
  editingRoomIdV14=null;
  save();
  renderOnline();
}
function roomRowV14(r){
  const course=disciplines.find(d=>d.id===r.courseId)?.title || "General ESP";
  const status=(r.date && r.date>=v14DateISO(v14TodayParts().day,v14TodayParts().month,v14TodayParts().year))?"Live Soon":"Scheduled";
  const attendance=(r.attendance||[]).length;
  return `<tr>
    <td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(status)}</span></td>
    <td>${esc(course)}<br><small>Week ${esc(r.week||"-")}</small></td>
    <td>${esc(roomDateLabel(r))}</td>
    <td>${esc(r.host)}</td>
    <td>${attendance} participant(s)</td>
    <td><strong>${esc(r.link||generateRoomLink(r.title,r.date,r.startTime))}</strong><button class="mini-copy" onclick="copyRoomLink('${r.id}')">Copy</button></td>
    <td class="room-actions">
      <button class="btn primary" onclick="joinRoom('${r.id}')">Join Room</button>
      <button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy</button>
      <button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button>
      <button class="btn soft" onclick="openRoomDetails('${r.id}')">Details</button>
      <button class="btn soft" onclick="editRoom('${r.id}')">Edit</button>
      <button class="btn danger" onclick="deleteRoom('${r.id}')">Delete</button>
    </td>
  </tr>`;
}
function renderOnline(){
  const rooms=db.rooms.filter(r=>r.scope==="all"||r.scope===session.role||!r.scope);
  const manage=["lecturer","admin"].includes(session.role);
  byId("mainView").innerHTML=`
    <section class="online-v14">
      ${manage?roomForm():`<section class="online-v14-card"><h3>Online Room</h3><p>Join scheduled ESP learning rooms, consultations, assessment feedback sessions, and professional-practice meetings.</p></section>`}
      <section class="online-v14-card">
        <div class="online-v14-card-head">
          <div><h3>Meeting Schedule</h3><p>View and manage scheduled virtual rooms with complete metadata, attendance, and professional short links.</p></div>
          ${manage?`<button class="btn primary" onclick="editingRoomIdV14=null;document.getElementById('roomFormPanel')?.scrollIntoView({behavior:'smooth'});">+ Create Room</button>`:""}
        </div>
        <div class="table-wrap online-room-table">
          <table><thead><tr><th>Meeting Title</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Short Link</th><th>Actions</th></tr></thead><tbody>
            ${rooms.map(roomRowV14).join("") || `<tr><td colspan="7"><div class="empty">No online room has been created yet.</div></td></tr>`}
          </tbody></table>
        </div>
      </section>
    </section>`;
  setTimeout(()=>refreshRoomLink(),50);
}
function previewRoomMetadata(){
  syncRoomDateParts();
  refreshRoomLink();
  const course=disciplines.find(d=>d.id===byId("roomCourse")?.value)?.title || "";
  openModal(`<h2>Meeting Metadata Preview</h2>
    <div class="table-wrap"><table><tbody>
      <tr><td><strong>Title</strong></td><td>${esc(byId("roomTitle")?.value||"-")}</td></tr>
      <tr><td><strong>Date</strong></td><td>${esc(byId("roomDay")?.value||"-")} ${esc(byId("roomMonth")?.value||"-")} ${esc(byId("roomYear")?.value||"-")}</td></tr>
      <tr><td><strong>Time</strong></td><td>${esc(byId("roomStartTime")?.value||"-")} – ${esc(byId("roomEndTime")?.value||"-")} ${esc(byId("roomTimezone")?.value||"")}</td></tr>
      <tr><td><strong>Course</strong></td><td>${esc(course)} • Week ${esc(byId("roomWeek")?.value||"-")}</td></tr>
      <tr><td><strong>Type</strong></td><td>${esc(byId("roomType")?.value||"-")}</td></tr>
      <tr><td><strong>Room Code</strong></td><td>${esc(byId("roomCode")?.value||"-")}</td></tr>
      <tr><td><strong>Professional Link</strong></td><td>${esc(byId("roomLink")?.value||"-")}</td></tr>
      <tr><td><strong>Agenda</strong></td><td>${esc(byId("roomAgenda")?.value||"-")}</td></tr>
      <tr><td><strong>Features</strong></td><td>${selectedRoomFeatures().map(esc).join(", ")||"-"}</td></tr>
    </tbody></table></div>`);
}
function openRoomDetails(id){
  const r=db.rooms.find(x=>x.id===id);
  ...
  openModal(`<h2>${esc(r.title)}</h2>
    <div class="table-wrap"><table><tbody>
      ...
      <tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr>
      <tr><td><strong>Course</strong></td><td>${esc(course)} • Week ${esc(r.week||"-")}</td></tr>
      <tr><td><strong>Type</strong></td><td>${esc(r.type||"Virtual Meeting")}</td></tr>
      <tr><td><strong>Code</strong></td><td>${esc(r.code||"-")}</td></tr>
      <tr><td><strong>Professional Link</strong></td><td>${esc(r.link||"-")}</td></tr>
      <tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr>
      <tr><td><strong>Notes</strong></td><td>${esc(r.notes||"-")}</td></tr>
      <tr><td><strong>Features</strong></td><td>${(r.features||[]).map(esc).join(", ")||"-"}</td></tr>
      <tr><td><strong>Attendance</strong></td><td>${(r.attendance||[]).map(a=>esc(a.user)+" ("+new Date(a.date).toLocaleString()+")").join("<br>")||"No attendance yet."}</td></tr>
    </tbody></table></div>
    <div class="actions"><button class="btn primary" onclick="joinRoom('${r.id}')">Join Room</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy Link</button><button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button></div>`);
}
function copyRoomLink(id){
  const r=db.rooms.find(x=>x.id===id);
  if(!r) return;
  navigator.clipboard?.writeText(r.link).then(()=>toast("Copied","Professional room link copied.")).catch(()=>openModal(`<h2>Room Link</h2><p>${esc(r.link)}</p>`));
}
function shareRoomLink(id){
  const r=db.rooms.find(x=>x.id===id);
  if(!r) return;
...
function editRoom(id){
  ...
  if(!room) return toast("Room not found");
  editingRoomIdV14=id;
  byId("mainView").innerHTML=`<section class="online-v14">${roomForm(room)}<section class="online-v14-card"><h3>Meeting Schedule</h3><div class="table-wrap online-room-table"><table><thead><tr><th>Meeting Title</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Short Link</th><th>Actions</th></tr></thead><tbody>${db.rooms.map(roomRowV14).join("")}</tbody></table></div></section></section>`;
  setTimeout(()=>document.getElementById("roomFormPanel")?.scrollIntoView({behavior:"smooth",block:"start"}),80);
}
function deleteRoom(id){
  if(!confirm("Delete this online room?")) return;
...
function joinRoom(id){
  ...
  if(!r.attendance.some(a=>a.user===(session.email||session.name||session.role))){
    r.attendance.push({user:session.email||session.name||session.role,date:new Date().toISOString()});
  }
  save();
  toast("Attendance recorded",`Opening ${r.code || "online room"} in a new tab.`);
  window.open(r.link,"_blank","noopener,noreferrer");
}

// Approved-map implementation: realistic Indonesia map visual, active pins, one-screen Atlas page.
function renderAtlas(){
  const current=selectedProvince();
  const search = byId("atlasSearch")?.value?.toLowerCase() || "";
  const filtered=provinceAtlas.filter(p=>(p.name+" "+p.heritage+" "+p.discipline+" "+provinceCategory(p)).toLowerCase().includes(search));
  const pack=provincePack(current);
  byId("mainView").innerHTML=`
    <section class="atlas-v14">
      <section class="atlas-v14-head">
        <div><h3>Nusantara Atlas</h3><p>Explore Indonesia • Local Wisdom • ESP Discipline Pathways</p></div>
        <div class="atlas-v14-top-actions"><button class="btn primary" onclick="openProvinceMaterial('${current.id}')">Open Module</button><button class="btn soft" onclick="navigate('tutor')">AI Tutor</button></div>
      </section>
      <section class="atlas-v14-content">
        <div class="atlas-v14-main">
          <div class="atlas-v14-tabs">
            <button class="active">Map</button>
            <button onclick="renderProvinceListModal()">Province List</button>
            <button onclick="renderAtlasBookmarks()">Bookmarks</button>
            <label><input id="atlasSearch" value="${esc(search)}" placeholder="Search province..." oninput="renderAtlas()"></label>
            <button onclick="openModal(atlasMethodologyHTML())">Learning Design</button>
          </div>
          <div class="atlas-v14-map">
            ${provinceAtlas.map(p=>`<button class="map-pin-v14 ${p.id===current.id?'active':''}" style="left:${p.x}%;top:${p.y}%;" onclick="selectProvince('${p.id}')" title="${esc(p.name)}"><span>${esc(p.name)}</span></button>`).join("")}
          </div>
          <div class="atlas-v14-stat-row">
            <div><strong>38</strong><span>Province Modules</span></div>
            <div><strong>Local Wisdom</strong><span>Cultural insights & heritage exploration</span></div>
            <div><strong>ESP Discipline Links</strong><span>Connected to professional contexts</span></div>
            <div><strong>Assessment Tasks</strong><span>Quizzes, tasks & performance checks</span></div>
          </div>
        </div>
        <aside class="atlas-v14-side">
          <div class="province-card-v14">
            <small>Province</small>
            <h3>${esc(current.name)}</h3>
            <p>${esc(current.heritage)}</p>
            <div class="province-info-item"><strong>Cultural Focus</strong><span>${esc(current.heritage)}</span></div>
            <div class="province-info-item"><strong>ESP Link</strong><span>${esc(current.discipline)}</span></div>
            <div class="province-info-item"><strong>Professional Learning</strong><span>${esc(pack.focus)} This module connects local wisdom with real-world professional scenarios to build meaningful, job-ready skills.</span></div>
            <button class="btn primary" onclick="openProvinceMaterial('${current.id}')">Open Module →</button>
            <button class="btn soft" onclick="navigate('tutor')">AI Tutor →</button>
            <button class="btn soft" onclick="openProvinceSkills('${current.id}')">Six-Skill Plan →</button>
            <button class="btn soft" onclick="bookmarkProvince('${current.id}')">Bookmark</button>
          </div>
          <div class="province-buttons-v14">
            <h3>Province Buttons</h3>
            <div>${filtered.map(p=>`<button class="${p.id===current.id?'active':''}" onclick="selectProvince('${p.id}')"><strong>${esc(p.name)}</strong><small>${esc(provinceCategory(p))}</small></button>`).join("")}</div>
          </div>
        </aside>
      </section>
    </section>`;
...
// Approved AI Tutor implementation with original generated Garuda character, not the old pasted screenshot.
function renderAITutor(){
  const u=currentUser()||{};
  const courseId=selectedCourseId || u.disciplineId || "tourism";
  const m=courseMaterial(courseId, selectedWeek || 1);
  const province=selectedProvince();
  byId("mainView").innerHTML=`
    <section class="ai-v14">
      <section class="ai-v14-phone">
        <div class="ai-v14-phone-head"><div><h3>AI Tutor</h3><span>● Online</span></div><b>⋮</b></div>
        <div class="ai-v14-hero">
          <div class="ai-v14-bubble"><strong>Hello, I am Garuda.</strong><br>How can I help you today?</div>
          <img src="assets/garuda-ai-character-original-v14.png" alt="Original Garuda AI Tutor">
        </div>
        <div class="ai-v14-mode-buttons">
          <button class="${selectedTutorMode==='speaking'?'active':''}" onclick="setTutorMode('speaking')">🎙 Practice Speaking</button>
          <button class="${selectedTutorMode==='writing'?'active':''}" onclick="setTutorMode('writing')">✏ Improve Writing</button>
          <button class="${selectedTutorMode==='pronunciation'?'active':''}" onclick="setTutorMode('pronunciation')">〰 Check Pronunciation</button>
          <button class="${selectedTutorMode==='feedback'?'active':''}" onclick="setTutorMode('feedback')">⭐ Get Feedback</button>
        </div>
        <div class="ai-v14-focus"><small>Today’s Focus</small><strong>${esc(m.path || "Professional Presentation")}</strong><div class="ai-v14-ring">${tutorScoreEstimate()}%</div></div>
        <div class="ai-v14-feedback"><div class="ai-v14-grade">${tutorLetter()}</div><p><strong>Feedback Summary</strong><br>${tutorSummary()}</p></div>
        <div class="ai-v14-bottom"><button class="active" onclick="setTutorMode(selectedTutorMode)">Tutor</button><button onclick="openTutorHistory()">History</button><button onclick="openTutorGoals()">Goals</button><button onclick="openTutorSettings()">Settings</button></div>
      </section>
      <section class="ai-v14-workspace">
        <div class="ai-v14-work-head"><h3>AI Tutor Workspace</h3><p>Practice, get feedback, and improve your ESP skills.</p><button class="btn soft" onclick="openTutorGoals()">Tutor Guidelines</button></div>
        <div class="ai-v14-select-grid">
          <label>Course<select onchange="selectedCourseId=this.value;localStorage.setItem('gesn.selectedCourseId',selectedCourseId);renderAITutor()">${disciplines.map(d=>`<option value="${d.id}" ${d.id===courseId?"selected":""}>${esc(d.title)}</option>`).join("")}</select></label>
          <label>Week<select onchange="selectedWeek=Number(this.value);renderAITutor()">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${selectedWeek===i+1?"selected":""}>Week ${i+1}</option>`).join("")}</select></label>
          <label>Province<select onchange="selectedProvinceId=this.value;localStorage.setItem('gesn.selectedProvinceId',selectedProvinceId);renderAITutor()">${provinceAtlas.map(p=>`<option value="${p.id}" ${p.id===province.id?"selected":""}>${esc(p.name)}</option>`).join("")}</select></label>
          <label>Heritage<input value="${esc(province.heritage)}" readonly></label>
        </div>
        ${tutorWorkspaceHTML(m,province)}
      </section>
    </section>`;
...
function tutorWorkspaceHTML(m,province){
  const prompt = {
    speaking:`Prepare a professional presentation about ${province.heritage} for international audiences.`,
    writing:`Write a professional 180–220 word text for ${m.discipline} connected to ${province.heritage}.`,
    pronunciation:`Read aloud: This local wisdom supports professional communication for international audiences.`,
    feedback:`Paste your speaking transcript or writing draft for ESP feedback.`
  }[selectedTutorMode] || "";
  return `<div class="ai-v14-task">
    <label>Your Prompt <input id="tutorPrompt" value="${esc(prompt)}" maxlength="250" oninput="updatePromptCount()"><small id="promptCount">${prompt.length} / 250</small></label>
    <label>Your Response / Notes<textarea id="tutorInput" placeholder="Type or record your response here...">${esc(localStorage.getItem("gesn.tutor.lastText")||"")}</textarea></label>
    <div class="ai-v14-actions">
      <button class="btn primary" onclick="runTutorAnalysis()">Analyze</button>
      <button class="btn gold" onclick="speak(byId('tutorInput').value || byId('tutorPrompt').value)">▶ Play</button>
      <button class="btn dark" onclick="stopSpeak()">■ Stop</button>
      <button class="btn primary" onclick="recordSpeech('tutorRecordOut')">🎙 Record</button>
      <button class="btn soft" onclick="insertTutorSample()">Insert Sample</button>
      <button class="btn soft" onclick="clearTutorInput()">Clear</button>
      <button class="btn gold" onclick="saveTutorNote()">Save Note</button>
    </div>
    <div id="tutorRecordOut"></div>
    <div id="tutorResult">${localStorage.getItem("gesn.tutor.lastResult")||""}</div>
    <div class="ai-v14-tip"><strong>Tip:</strong> Be specific in your prompt for better ESP feedback. Use Insert Sample to see a model response.</div>
  </div>`;
}
function updatePromptCount(){const el=byId("tutorPrompt"), c=byId("promptCount"); if(el&&c)c.textContent=`${el.value.length} / 250`;}

Object.assign(window,{generateRoomLink,roomCodeFrom,refreshRoomLink,renderOnline,roomForm,saveRoom,copyGeneratedRoomLink,shareGeneratedRoomLink,copyRoomLink,shareRoomLink,editRoom,deleteRoom,joinRoom,renderAtlas,renderAITutor,updatePromptCount});


// ===== FINAL V14B: force all existing/old room links to professional domain =====
function professionalRoomLink(r){
  if(!r) return "";
  const existing=String(r.link||"");
  if(existing && existing.includes("room.garudaesp.id") && !existing.includes("github") && !existing.includes("#room-")) return existing;
  return generateRoomLink(r.title||"GARUDA ESP Room", r.date||v14DateISO(r.day||v14TodayParts().day,r.month||v14TodayParts().month,r.year||v14TodayParts().year), r.startTime||"09:00");
}
function roomRowV14(r){
  const course=disciplines.find(d=>d.id===r.courseId)?.title || "General ESP";
  const link=professionalRoomLink(r);
  const status=(r.date && r.date>=v14DateISO(v14TodayParts().day,v14TodayParts().month,v14TodayParts().year))?"Live Soon":"Scheduled";
  const attendance=(r.attendance||[]).length;
  return `<tr>
    <td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(status)}</span></td>
    ...
    <td>${esc(r.host)}</td>
    <td>${attendance} participant(s)</td>
    <td><strong>${esc(link)}</strong><button class="mini-copy" onclick="copyRoomLink('${r.id}')">Copy</button></td>
    <td class="room-actions">
      <button class="btn primary" onclick="joinRoom('${r.id}')">Join Room</button>
      <button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy</button>
      <button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button>
...
function openRoomDetails(id){
  ...
  openModal(`<h2>${esc(r.title)}</h2>
    <div class="table-wrap"><table><tbody>
      ...
      <tr><td><strong>Course</strong></td><td>${esc(course)} • Week ${esc(r.week||"-")}</td></tr>
      <tr><td><strong>Type</strong></td><td>${esc(r.type||"Virtual Meeting")}</td></tr>
      <tr><td><strong>Code</strong></td><td>${esc(r.code||roomCodeFrom(r.title,r.date,r.startTime))}</td></tr>
      <tr><td><strong>Professional Link</strong></td><td>${esc(link)}</td></tr>
      <tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr>
      <tr><td><strong>Notes</strong></td><td>${esc(r.notes||"-")}</td></tr>
      <tr><td><strong>Features</strong></td><td>${(r.features||[]).map(esc).join(", ")||"-"}</td></tr>
      <tr><td><strong>Attendance</strong></td><td>${(r.attendance||[]).map(a=>esc(a.user)+" ("+new Date(a.date).toLocaleString()+")").join("<br>")||"No attendance yet."}</td></tr>
...
function copyRoomLink(id){
  ...
  if(!r) return;
  const link=professionalRoomLink(r);
  navigator.clipboard?.writeText(link).then(()=>toast("Copied","Professional room link copied.")).catch(()=>openModal(`<h2>Room Link</h2><p>${esc(link)}</p>`));
}
function shareRoomLink(id){
  const r=db.rooms.find(x=>x.id===id);
  if(!r) return;
  const link=professionalRoomLink(r);
  if(navigator.share){navigator.share({title:r.title,text:`${r.title} • ${roomDateLabel(r)}`,url:link}).catch(()=>{});}
  else copyRoomLink(id);
}
function joinRoom(id){
  const r=db.rooms.find(x=>x.id===id);
  ...
  r.link=professionalRoomLink(r);
  save();
  toast("Attendance recorded",`Opening ${r.code || "online room"} in a new tab.`);
  window.open(r.link,"_blank","noopener,noreferrer");
}
Object.assign(window,{professionalRoomLink,roomRowV14,openRoomDetails,copyRoomLink,shareRoomLink,joinRoom});


// ===== FINAL V15: rebuilt admin workspace chrome + clean AI Tutor showcase =====
const ADMIN_NAV_V15 = [
  {section:'', items:[['dashboard','⌂','Dashboard']]},
  {section:'LEARNING MANAGEMENT', items:[['courses','▣','Courses'],['students','◫','Students'],['instructors','◎','Instructors'],['classes','▤','Classes'],['online','▭','Online Room'],['materials','▥','Materials'],['assignments','▧','Assignments'],['assessments','◩','Assessments'],['attendance','▦','Attendance']]},
  {section:'COMMUNICATION', items:[['announcements','◌','Announcements'],['messages_center','✉','Messages'],['notifications','◔','Notifications']]},
  {section:'REPORTS & ANALYTICS', items:[['reports','▨','Reports'],['analytics','◲','Analytics']]},
  {section:'SYSTEM MANAGEMENT', items:[['users','♚','Users'],['roles','◈','Roles & Permissions'],['settings','⚙','Settings']]}
];

function workspaceProfileHTML(){
  const name = session?.role==='admin' ? 'Garuda Admin' : (session?.name || 'Workspace User');
  const role = session?.role==='admin' ? 'Super Administrator' : (session?.role || 'Member');
  return `<div class="profile-chip-v15"><img src="assets/logo-square.png" alt="Profile"><div><strong>${esc(name)}</strong><small>${esc(role)}</small></div><span>▾</span></div>`;
}
function applyWorkspaceChrome(){
  const top = document.querySelector('.top-actions');
  if(top){
    top.innerHTML = `<button onclick="navigate('forum')">Forum</button><button onclick="navigate('chat')">Chat</button><button onclick="navigate('helpdesk')">Help Desk</button><button onclick="exportData()">Export</button><button class="logout" onclick="safeLogout(event)">Logout</button>${workspaceProfileHTML()}`;
  }
}
function renderNav(role){
  ...
  if(!nav) return;
  if(role === 'admin'){
    nav.innerHTML = ADMIN_NAV_V15.map(group => `
      ${group.section ? `<div class="nav-section-title">${group.section}</div>` : ''}
      ${group.items.map(([p,i,l]) => `<button data-page="${p}"><span>${i}</span>${l}</button>`).join('')}
    `).join('') + `<div class="nav-help-box"><button data-page="help_support"><span>◍</span>Help & Support</button></div>`;
  } else {
    nav.innerHTML = (navs[role]||[]).map(([p,i,l])=>`<button data-page="${p}"><span>${i}</span>${l}</button>`).join('');
  }
  nav.querySelectorAll('button').forEach(b=>b.onclick=()=>navigate(b.dataset.page));
  applyWorkspaceChrome();
}
function renderAliasInfo(title, description, actionLabel, actionPage){
  byId('mainView').innerHTML = `<section class="panel"><h3>${esc(title)}</h3><p>${esc(description)}</p><div class="actions"><button class="btn primary" onclick="navigate('${actionPage}')">${esc(actionLabel)}</button></div></section>`;
}
function navigate(page){
  currentPage = page;
  document.querySelectorAll('.side-nav button').forEach(b => b.classList.toggle('active', b.dataset.page===page));
  ...
  applyWorkspaceChrome();
  const adminTitles = {
    dashboard:'Dashboard',courses:'Courses',students:'Students',instructors:'Instructors',classes:'Classes',online:'Online Room',materials:'Materials',assignments:'Assignments',assessments:'Assessments',attendance:'Attendance',announcements:'Announcements',messages_center:'Messages',notifications:'Notifications',reports:'Reports',analytics:'Analytics',users:'Users',roles:'Roles & Permissions',settings:'Settings',help_support:'Help & Support'
  };
  const standardTitles = {
    forum:'Forum',chat:'Chat',helpdesk:'Help Desk',atlas:'Nusantara Atlas',tutor:'AI Tutor',class:'My Class',gradebook:'Gradebook',discussion:'Discussion Forum',messages:'Messages',calendar:'Calendar'
  };
  const t = session.role === 'admin' ? (adminTitles[page] || standardTitles[page] || page.replace(/\b\w/g,m=>m.toUpperCase())) : ((navs[session.role]||[]).find(n=>n[0]===page)?.[2] || standardTitles[page] || page.replace(/\b\w/g,m=>m.toUpperCase()));
  byId('pageTitle').textContent = t;
  ...
  const route = {
    ...
    class: renderClass,
    assignments: session.role==='student'?renderAssignments:renderTasks,
    assessments: renderTasks,
    gradebook: session.role==='student'?renderPortfolio:renderReview,
    messages: renderChat,
    messages_center: renderChat,
    calendar: renderCalendar,
    discussion: renderForum,
    reports: session.role==='student'?renderPortfolio:session.role==='admin'?renderSystemAnalytics:renderLecturerAnalytics,
    online: renderOnline,
    ...
    instructors: renderUsers,
    roles: renderSettings,
    help_support: renderHelpdesk,
    classes: renderCourseDashboard,
    attendance: session.role==='admin'?renderSystemAnalytics:renderClass,
    announcements: renderForum,
    notifications: renderHelpdesk
  };
  (route[page] || renderCourseDashboard)();
  applyWorkspaceChrome();
}

function aiTutorModeTabsV15(){
  return `<div class="ai15-mode-row">
    <button class="${selectedTutorMode==='speaking'?'active':''}" onclick="setTutorMode('speaking')">Practice Speaking</button>
    <button class="${selectedTutorMode==='writing'?'active':''}" onclick="setTutorMode('writing')">Improve Writing</button>
    <button class="${selectedTutorMode==='pronunciation'?'active':''}" onclick="setTutorMode('pronunciation')">Check Pronunciation</button>
    <button class="${selectedTutorMode==='feedback'?'active':''}" onclick="setTutorMode('feedback')">Get Feedback</button>
...
function renderAITutor(){
  ...
  const prompt = {
    speaking:`Prepare a professional presentation about ${province.heritage} for international audiences.`,
    writing:`Write a professional 180–220 word ESP response about ${province.heritage} for ${m.discipline}.`,
    pronunciation:`Read aloud: This local wisdom supports professional communication for international audiences.`,
    feedback:`Paste your speaking transcript or writing draft for ESP feedback.`
  }[selectedTutorMode] || '';
  byId('mainView').innerHTML = `
    <section class="ai15-layout">
      <aside class="ai15-showcase panel">
        <img src="assets/ai-tutor-reference-full.png" alt="Garuda AI Tutor reference showcase">
        <div class="ai15-showcase-note">Approved Garuda AI Tutor visual direction. The interactive controls are provided in the workspace panel on the right.</div>
      </aside>
      <section class="ai15-panel panel">
        <div class="ai15-head"><h3>AI Tutor Workspace</h3><p>Practice, get feedback, and improve your ESP skills.</p></div>
        <button class="ai15-guide-btn" onclick="openTutorGoals()">Tutor Guidelines</button>
        ${aiTutorModeTabsV15()}
        <div class="ai15-form-grid">
          <label>Course<select onchange="selectedCourseId=this.value;localStorage.setItem('gesn.selectedCourseId',selectedCourseId);renderAITutor()">${disciplines.map(d=>`<option value="${d.id}" ${d.id===courseId?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label>
          <label>Week<select onchange="selectedWeek=Number(this.value);renderAITutor()">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${selectedWeek===i+1?'selected':''}>Week ${i+1}</option>`).join('')}</select></label>
          <label>Province<select onchange="selectedProvinceId=this.value;localStorage.setItem('gesn.selectedProvinceId',selectedProvinceId);renderAITutor()">${provinceAtlas.map(p=>`<option value="${p.id}" ${p.id===province.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label>
          <label>Heritage<input value="${esc(province.heritage)}" readonly></label>
        </div>
        <div class="ai15-input-block">
          <label>Your Prompt<input id="tutorPrompt" value="${esc(prompt)}" maxlength="250" oninput="updatePromptCount()"><small id="promptCount">${prompt.length} / 250</small></label>
          <label>Your Response / Notes<textarea id="tutorInput" placeholder="Type or record your response here...">${esc(localStorage.getItem('gesn.tutor.lastText')||'')}</textarea></label>
        </div>
        <div class="ai15-actions">
          <button class="btn primary" onclick="runTutorAnalysis()">Analyze</button>
          <button class="btn gold" onclick="speak(byId('tutorInput').value || byId('tutorPrompt').value)">Play</button>
          <button class="btn dark" onclick="stopSpeak()">Stop</button>
          <button class="btn primary" onclick="recordSpeech('tutorRecordOut')">Record</button>
        ...
        <div id="tutorRecordOut"></div>
        <div id="tutorResult" class="ai15-result-box">${localStorage.getItem('gesn.tutor.lastResult')||'<div class="empty">Local AI-assisted feedback will appear here after you click Analyze.</div>'}</div>
        <div class="ai15-tip">Tip: Be specific in your prompt for better ESP feedback. Use Insert Sample to see a model response.</div>
      </section>
    </section>`;
  updatePromptCount();
  applyWorkspaceChrome();
...
// keep top chrome updated after login / manual entry
const __oldForceOpenAdminWorkspace = window.forceOpenAdminWorkspace;
window.forceOpenAdminWorkspace = function(){ if(__oldForceOpenAdminWorkspace) __oldForceOpenAdminWorkspace(); setTimeout(applyWorkspaceChrome, 50); };
const __oldSafeAdminLogin = window.safeAdminLogin;
window.safeAdminLogin = function(ev){ const out = __oldSafeAdminLogin ? __oldSafeAdminLogin(ev) : undefined; setTimeout(applyWorkspaceChrome, 80); return out; };
Object.assign(window,{renderNav,navigate,renderAITutor,applyWorkspaceChrome});


// ===== FINAL V16: restore all requested core features in Admin Workspace =====
const ADMIN_NAV_V16 = [
  {section:'', items:[['dashboard','⌂','Dashboard']]},
  {section:'LEARNING MANAGEMENT', items:[
    ['courses','▣','My Courses'],
    ['materials','▤','Materials Library'],
    ['builder','▧','Content Builder'],
    ['assessments','□','Assessments'],
    ['atlas','⌖','Nusantara Atlas'],
    ['tutor','◉','AI Tutor'],
    ['online','▭','Online Room'],
    ['class','♙','Class & Attendance']
  ]},
  {section:'USERS & INSTITUTIONS', items:[
...
function adminTitleV16(page){
  return ({
    ...
    reports:'Reports',
    settings:'Settings'
  })[page] || page.replace(/\b\w/g,m=>m.toUpperCase());
}
...
function renderNav(role){
  ...
  if(!nav) return;
  if(role === 'admin'){
    nav.innerHTML = ADMIN_NAV_V16.map(group => `
      ${group.section ? `<div class="nav-section-title">${group.section}</div>` : ''}
      ${group.items.map(([p,i,l]) => `<button data-page="${p}"><span>${i}</span>${l}</button>`).join('')}
    `).join('');
  } else {
...
function navigate(page){
  ...
  document.querySelectorAll('.side-nav button').forEach(b => b.classList.toggle('active', b.dataset.page===page));
  byId('roleKicker').textContent = `${session.role.toUpperCase()} WORKSPACE`;
  byId('pageTitle').textContent = session.role === 'admin'
    ? adminTitleV16(page)
    : ((navs[session.role]||[]).find(n=>n[0]===page)?.[2] || adminTitleV16(page));
  applyWorkspaceChrome();
...
// Strong admin dashboard with direct feature cards so no core feature is hidden.
function adminFeatureCardV16(icon,title,desc,page,stat){
  return `<article class="feature-card-v16">
    <div class="feature-icon-v16">${icon}</div>
    <div>
      <h3>${esc(title)}</h3>
      <p>${esc(desc)}</p>
      ${stat?`<span class="tag red">${esc(stat)}</span>`:''}
    </div>
    <button class="btn primary" onclick="navigate('${page}')">Open</button>
  </article>`;
}
function renderAdminDashboard(){
  byId('mainView').innerHTML = `
    <section class="dashboard-hero-v16">
      <div>
        <h3>Garuda ESP Nusantara Command Dashboard</h3>
        <p>Integrated control center for users, institutions, self-enrolment courses, complete semester materials, AI Tutor, Nusantara Atlas, content authoring, assessments, online rooms, analytics, forum, chat, and help desk.</p>
      </div>
      <div class="grid cols-4">
        ${metric("Users",db.users.length,"Total accounts")}
        ${metric("Courses",disciplines.length,"ESP disciplines")}
        ${metric("Weeks",16,"Semester materials")}
        ${metric("Provinces",provinceAtlas.length,"Atlas modules")}
      </div>
    </section>
    <section class="feature-grid-v16">
      ${adminFeatureCardV16('▣','My Courses','Six ESP disciplines with self-enrolment control, course overview, learner pathway, and learning access.','courses',disciplines.length+' disciplines')}
      ${adminFeatureCardV16('▤','Materials Library','Complete 16-week semester materials with reading, listening, speaking, writing, vocabulary, grammar, resources, quizzes, and assignments.','materials','16 weeks')}
      ${adminFeatureCardV16('▧','Content Builder','Create or update materials, tasks, instructions, rubrics, learning outcomes, and resources.','builder','Authoring')}
      ${adminFeatureCardV16('□','Assessments','Publish assignments, quizzes, rubrics, due dates, and assessment feedback tasks.','assessments','Rubric-based')}
      ${adminFeatureCardV16('⌖','Nusantara Atlas','Interactive 38-province map linking Indonesian local wisdom and heritage to ESP disciplines and materials.','atlas','38 provinces')}
      ${adminFeatureCardV16('◉','AI Tutor','Local AI-assisted ESP companion for speaking practice, writing improvement, pronunciation awareness, and feedback.','tutor','No API key')}
      ${adminFeatureCardV16('▭','Online Room','Create professional virtual rooms with metadata, short links, copy/share, edit, attendance, and schedule management.','online','Room links')}
      ${adminFeatureCardV16('♚','Users & Institutions','Approve lecturers, manage users, map institutions, and connect accounts to ESP activities.','users','Approval')}
      ${adminFeatureCardV16('▥','System Analytics','Monitor enrolment, progress, submissions, attendance, learning evidence, and role-based reports.','analytics','Reports')}
    </section>`;
}

// Materials Library still opens full semester materials; this label also confirms completeness.
const __oldRenderMaterialsV16 = renderMaterials;
function renderMaterials(){
  __oldRenderMaterialsV16();
  const mv = byId('mainView');
  if(mv && !mv.querySelector('.materials-v16-topnote')){
    const note = document.createElement('section');
    note.className = 'materials-v16-topnote';
    note.innerHTML = `<strong>Complete Materials Library:</strong> six ESP disciplines, sixteen weeks, four core sessions, resources, audio play/stop/record, vocabulary, grammar, quiz, assignment, and assessment rubric integration.`;
    mv.prepend(note);
  }
}

// Content Builder enhanced entry controls without removing original builder.
const __oldRenderBuilderV16 = renderBuilder;
function renderBuilder(){
  __oldRenderBuilderV16();
  const mv = byId('mainView');
  if(mv && !mv.querySelector('.builder-quick-v16')){
    const quick = document.createElement('section');
    quick.className = 'panel builder-quick-v16';
    quick.innerHTML = `<h3>Builder Navigation</h3><p>Use these shortcuts to confirm the full authoring cycle is active.</p>
      <div class="actions">
        <button class="btn primary" onclick="navigate('materials')">Open Materials Library</button>
        <button class="btn soft" onclick="navigate('assessments')">Open Assessments</button>
        <button class="btn soft" onclick="navigate('atlas')">Open Nusantara Atlas</button>
        <button class="btn soft" onclick="navigate('tutor')">Open AI Tutor</button>
      </div>`;
    mv.prepend(quick);
  }
...
// Assessment page explicitly restored for admin.
const __oldRenderTasksV16 = renderTasks;
function renderTasks(){
  __oldRenderTasksV16();
  const mv = byId('mainView');
  if(mv && !mv.querySelector('.assessment-restored-v16')){
    const note = document.createElement('section');
    note.className = 'panel assessment-restored-v16';
    note.innerHTML = `<h3>Assessment Control Restored</h3><p>Create, publish, review, and connect assessments to semester materials, ESP disciplines, weeks, rubrics, submissions, and reports.</p>`;
    mv.prepend(note);
  }
}

// Lightweight diagnostics for final checking.
function runFeatureDiagnosticsV16(){
  const checks = [
    ['AI Tutor route', typeof renderAITutor === 'function'],
    ['Nusantara Atlas route', typeof renderAtlas === 'function'],
    ['Content Builder route', typeof renderBuilder === 'function'],
    ['Materials Library route', typeof renderMaterials === 'function'],
    ['Assessments route', typeof renderTasks === 'function'],
    ['Online Room route', typeof renderOnline === 'function'],
    ['Users & Institutions route', typeof renderUsers === 'function'],
    ['Professional room link generator', typeof generateRoomLink === 'function' && generateRoomLink('Test','2026-06-08','09:00').includes('room.garudaesp.id')],
    ['Admin nav includes AI Tutor', JSON.stringify(ADMIN_NAV_V16).includes('AI Tutor')],
    ['Admin nav includes Nusantara Atlas', JSON.stringify(ADMIN_NAV_V16).includes('Nusantara Atlas')],
    ['Admin nav includes Content Builder', JSON.stringify(ADMIN_NAV_V16).includes('Content Builder')],
    ['Admin nav includes Materials Library', JSON.stringify(ADMIN_NAV_V16).includes('Materials Library')],
    ['Admin nav includes Assessments', JSON.stringify(ADMIN_NAV_V16).includes('Assessments')]
  ];
  return checks;
}
Object.assign(window,{renderNav,navigate,renderAdminDashboard,renderMaterials,renderBuilder,renderTasks,runFeatureDiagnosticsV16});


// ===== FINAL V17: distinct modules, accordion sidebar, working AI Tutor, integrated class attendance =====
const ADMIN_NAV_V17 = [
  {key:'core', label:'CORE DASHBOARD', items:[['dashboard','⌂','Dashboard']]},
  {key:'learning', label:'LEARNING MANAGEMENT', items:[
    ['courses','▣','My Courses'],
    ['materials','▤','Materials Library'],
    ['builder','▧','Content Builder'],
    ['assessments','□','Assessments'],
  ...
  {key:'people', label:'USERS & INSTITUTIONS', items:[
    ['users','♚','Users & Institutions'],
    ['students','◫','Students'],
    ['instructors','◎','Lecturers']
  ]},
  ...
  {key:'insight', label:'REPORTS & ANALYTICS', items:[
    ['analytics','▥','System Analytics'],
    ['reports','▨','Reports']
  ]},
  {key:'system', label:'SYSTEM MANAGEMENT', items:[
    ['settings','⚙','Settings']
  ]}
];

function currentNavGroupV17(page=currentPage){
  const g=ADMIN_NAV_V17.find(group=>group.items.some(item=>item[0]===page));
  return g?g.key:'core';
}
function toggleNavGroupV17(key){
  document.querySelectorAll('.nav-items-v17').forEach(el=>{
    const isTarget=el.dataset.group===key;
    el.classList.toggle('open',isTarget ? !el.classList.contains('open') : false);
  });
}
function renderNav(role){
  const nav=byId('sideNav');
  if(!nav) return;
  if(role==='admin'){
    const activeGroup=currentNavGroupV17();
    nav.innerHTML=ADMIN_NAV_V17.map(group=>{
      const open = group.key===activeGroup ? 'open' : '';
      return `<div class="nav-group-v17">
        <button class="nav-group-title-v17 ${open}" onclick="toggleNavGroupV17('${group.key}')"><span>▾</span>${group.label}</button>
        <div class="nav-items-v17 ${open}" data-group="${group.key}">
          ${group.items.map(([p,i,l])=>`<button data-page="${p}"><span>${i}</span>${l}</button>`).join('')}
        </div>
      </div>`;
    }).join('');
...
function adminTitleV17(page){
  return ({
    dashboard:'Dashboard',courses:'My Courses',materials:'Materials Library',builder:'Content Builder',
    assessments:'Assessments',assignments:'Assessments',atlas:'Nusantara Atlas',tutor:'AI Tutor',
    online:'Online Room',class:'Class & Attendance',users:'Users & Institutions',students:'Students',
    instructors:'Lecturers',forum:'Forum',chat:'Chat',helpdesk:'Help Desk',analytics:'System Analytics',
...
function navigate(page){
  ...
  byId('pageTitle').textContent=session.role==='admin'
    ? adminTitleV17(page)
    : ((navs[session.role]||[]).find(n=>n[0]===page)?.[2] || adminTitleV17(page));
  if(session.role==='admin') renderNav('admin');
  document.querySelectorAll('.side-nav button[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===page));
  applyWorkspaceChrome();
  const route={
    dashboard:session.role==='student'?renderStudentDashboard:session.role==='lecturer'?renderLecturerDashboard:renderAdminDashboard,
    courses:renderCourseDashboard,
    materials:renderMaterials,
    builder:renderBuilder,
    assessments:renderAssessmentsV17,
    assignments:session.role==='student'?renderAssignments:renderAssessmentsV17,
    atlas:renderAtlas,
    tutor:renderAITutor,
    online:renderOnline,
    class:renderClass,
    users:renderUsersAndInstitutionsV17,
    students:renderStudentsV17,
    instructors:renderLecturersV17,
    forum:renderForum,
    chat:renderChat,
    helpdesk:renderHelpdesk,
    analytics:renderSystemAnalyticsV17,
    reports:renderReportsV17,
    settings:renderSettings,
    gradebook:session.role==='student'?renderPortfolio:renderReview,
    messages:renderChat,
    discussion:renderForum,
...
/* ---------- Materials Library: complete but different from Builder and Assessment ---------- */
function renderMaterials(){
  const courseId=selectedCourseId || 'tourism';
  const course=disciplines.find(d=>d.id===courseId)||disciplines[0];
  const week=Number(selectedWeek||1);
  const m=courseMaterial(courseId,week);
  const weeks=Array.from({length:16},(_,i)=>courseMaterial(courseId,i+1));
  byId('mainView').innerHTML=`
    <section class="materials-v17">
      <section class="mat-head-v17">
        <div>
          <h3>Semester ESP Materials</h3>
          <p>Complete semester library for all six ESP disciplines. Each week contains reading, listening, speaking, writing, vocabulary, grammar, resources, quiz, assignment, and rubric evidence.</p>
        </div>
        <div class="mat-controls-v17">
          <label>ESP Discipline<select onchange="selectedCourseId=this.value;localStorage.setItem('gesn.selectedCourseId',selectedCourseId);renderMaterials()">${disciplines.map(d=>`<option value="${d.id}" ${d.id===courseId?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label>
          <label>Week<select onchange="selectedWeek=Number(this.value);renderMaterials()">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${week===i+1?'selected':''}>Week ${i+1}</option>`).join('')}</select></label>
        </div>
      </section>
      <section class="mat-week-strip-v17">${weeks.map(w=>`<button class="${w.week===week?'active':''}" onclick="selectedWeek=${w.week};renderMaterials()">Week ${w.week}<small>${esc(w.path)}</small></button>`).join('')}</section>
      <section class="mat-detail-v17">
        <aside class="mat-left-v17">
          <div class="mat-week-badge-v17"><span>WEEK</span><strong>${week}</strong></div>
          <h3>${esc(m.path)} in ${esc(course.title.replace('English for ',''))}</h3>
          <p>${esc(m.overview)}</p>
          <div class="mat-meta-grid-v17">
            <div><small>Province Focus</small><strong>${esc(m.province)}</strong></div>
            <div><small>Local Wisdom</small><strong>${esc(m.heritage)}</strong></div>
            <div><small>Essential Question</small><strong>${esc(m.essential)}</strong></div>
          </div>
          <div class="actions">
            <button class="btn primary" onclick="openWeek(${week})">Open Detailed Module</button>
            <button class="btn soft" onclick="navigate('builder')">Edit in Builder</button>
            <button class="btn soft" onclick="navigate('assessments')">Assessment</button>
          </div>
        </aside>
        <main class="mat-session-grid-v17">
          ${materialSessionCardV17('Reading',m.reading,'Read professional context and answer comprehension questions.',true)}
          ${materialSessionCardV17('Listening',m.listening,'Play, stop, and record listening reflection.',true)}
          ${materialSessionCardV17('Speaking',m.speaking,'Record a role-play or presentation performance.',true)}
          ${materialSessionCardV17('Writing',m.writing,'Draft and submit a professional ESP text.',false)}
          <section class="mat-session-card-v17"><h4>Vocabulary & Grammar</h4><p>${esc(m.grammar)}</p><div class="table-wrap fit-small-table">${vocabTable(m)}</div></section>
          <section class="mat-session-card-v17"><h4>Assessment Rubric</h4><div class="table-wrap fit-small-table">${rubricTable(m)}</div><button class="btn soft" onclick="navigate('assessments')">Create Related Assessment</button></section>
          <section class="mat-session-card-v17 resources-v17"><h4>Resources</h4>${m.resources.map(r=>`<div class="resource-row-v17"><span>${esc(r)}</span><button onclick="openResourceV17('${esc(r)}')">Open</button></div>`).join('')}</section>
          <section class="mat-session-card-v17"><h4>Quiz & Assignment</h4><p><strong>Quiz:</strong> ${m.quiz.length} items for Week ${week} understanding.</p><p><strong>Assignment:</strong> ${esc(m.assignment)}</p><div class="actions"><button class="btn primary" onclick="takeQuiz(${week})">Start Quiz</button><button class="btn soft" onclick="submitAssignment(${week})">Submit Assignment</button></div></section>
        </main>
      </section>
    </section>`;
}
function materialSessionCardV17(title,text,brief,audio){
  return `<section class="mat-session-card-v17"><h4>${esc(title)}</h4><p>${esc(brief)}</p><div class="mat-text-v17">${esc(String(text).slice(0,350))}${String(text).length>350?'...':''}</div>${audio?`<div class="actions"><button class="btn primary" onclick="speak('${esc(text)}')">Play</button><button class="btn soft" onclick="stopSpeak()">Stop</button><button class="btn soft" onclick="recordSpeech('record-${esc(title)}')">Record</button></div><div id="record-${esc(title)}"></div>`:`<button class="btn soft" onclick="openWritingEditorV17('${esc(title)}')">Open Writing Editor</button>`}</section>`;
}
function openResourceV17(name){openModal(`<h2>${esc(name)}</h2><p>This resource is connected to the selected week, discipline, skill session, assessment rubric, and student portfolio evidence.</p><div class="actions"><button class="btn primary" onclick="closeModal()">Use Resource</button></div>`)}
function openWritingEditorV17(title){openModal(`<h2>${esc(title)} Editor</h2><textarea id="writingEditorV17" style="min-height:260px" placeholder="Write your ESP draft here..."></textarea><div class="actions"><button class="btn primary" onclick="toast('Saved','Writing draft saved locally.');closeModal()">Save Draft</button><button class="btn soft" onclick="speak(byId('writingEditorV17').value)">Play Draft</button></div>`)}

/* ---------- Content Builder: authoring only ---------- */
function renderBuilder(){
  const courseId=selectedCourseId||'tourism';
  const week=selectedWeek||1;
  const m=courseMaterial(courseId,week);
  byId('mainView').innerHTML=`
    <section class="builder-v17">
      <section class="builder-head-v17 panel">
        <h3>Content Builder</h3>
        <p>Author and update semester materials. This page is only for creating/editing content, not for reading materials or publishing assessment tasks.</p>
        <div class="actions"><button class="btn primary" onclick="loadBuilderWeekV17()">Load Selected Week</button><button class="btn soft" onclick="saveBuilderWeekV17()">Save Content</button><button class="btn soft" onclick="previewBuilderWeekV17()">Preview</button></div>
      </section>
      <section class="builder-grid-v17">
        <aside class="panel">
          <h3>Builder Metadata</h3>
          <label>ESP Discipline<select id="builderDiscipline" onchange="selectedCourseId=this.value;localStorage.setItem('gesn.selectedCourseId',selectedCourseId);renderBuilder()">${disciplines.map(d=>`<option value="${d.id}" ${d.id===courseId?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label>
          <label>Week<select id="builderWeek" onchange="selectedWeek=Number(this.value);renderBuilder()">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${week===i+1?'selected':''}>Week ${i+1}</option>`).join('')}</select></label>
          <label>Title<input id="builderTitle" value="${esc(m.path)}"></label>
          <label>Province Focus<input id="builderProvince" value="${esc(m.province)}"></label>
          <label>Local Wisdom<input id="builderHeritage" value="${esc(m.heritage)}"></label>
          <label>Essential Question<textarea id="builderEssential">${esc(m.essential)}</textarea></label>
        </aside>
        <main class="panel">
          <h3>Session Authoring Canvas</h3>
          <div class="builder-tabs-v17">
            <button onclick="setBuilderTabV17('reading')">Reading</button>
            <button onclick="setBuilderTabV17('listening')">Listening</button>
            <button onclick="setBuilderTabV17('speaking')">Speaking</button>
            <button onclick="setBuilderTabV17('writing')">Writing</button>
            <button onclick="setBuilderTabV17('grammar')">Vocabulary & Grammar</button>
            <button onclick="setBuilderTabV17('resources')">Resources</button>
          </div>
          <label>Reading Text<textarea id="builderReading">${esc(m.reading)}</textarea></label>
          <label>Listening Script<textarea id="builderListening">${esc(m.listening)}</textarea></label>
          <label>Speaking Task<textarea id="builderSpeaking">${esc(m.speaking)}</textarea></label>
          <label>Writing Task<textarea id="builderWriting">${esc(m.writing)}</textarea></label>
          <label>Vocabulary & Grammar<textarea id="builderGrammar">${esc(m.grammar)}</textarea></label>
          <label>Resources<textarea id="builderResources">${esc(m.resources.join('\n'))}</textarea></label>
        </main>
      </section>
    </section>`;
}
function setBuilderTabV17(id){toast('Builder section opened',`${id} authoring field is active.`)}
function loadBuilderWeekV17(){renderBuilder();toast('Loaded','Selected week content loaded into builder.')}
function saveBuilderWeekV17(){
  const week=Number(byId('builderWeek').value);
  const w=db.weeks.find(x=>x.week===week);
  if(!w) return toast('Week not found');
  w.title=byId('builderTitle').value.trim()||w.title;
  w.province=byId('builderProvince').value.trim()||w.province;
  w.heritage=byId('builderHeritage').value.trim()||w.heritage;
  w.essential=byId('builderEssential').value.trim()||w.essential;
  w.reading=byId('builderReading').value;
  w.listening=byId('builderListening').value;
  w.speaking=byId('builderSpeaking').value;
  w.writing=byId('builderWriting').value;
  w.grammar=byId('builderGrammar').value;
  w.resources=byId('builderResources').value.split('\n').map(x=>x.trim()).filter(Boolean);
  save(); toast('Content saved','The selected week content has been updated.'); selectedWeek=week;
}
function previewBuilderWeekV17(){openModal(`<h2>Builder Preview</h2><p><strong>${esc(byId('builderTitle').value)}</strong></p><p>${esc(byId('builderEssential').value)}</p><div class="actions"><button class="btn primary" onclick="closeModal();navigate('materials')">Open Materials View</button></div>`)}

/* ---------- Assessments: assessment management only ---------- */
function renderAssessmentsV17(){
  byId('mainView').innerHTML=`
    <section class="assessment-v17">
      <section class="panel assessment-create-v17">
        <h3>Assessment Center</h3>
        <p>Create and publish assignments, quizzes, speaking performances, projects, rubrics, and feedback tasks linked to ESP disciplines and weeks.</p>
        <div class="field-grid two">
          <label>ESP Discipline<select id="assessCourse">${disciplines.map(d=>`<option value="${d.id}">${esc(d.title)}</option>`).join('')}</select></label>
          <label>Week<select id="assessWeek">${Array.from({length:16},(_,i)=>`<option value="${i+1}">Week ${i+1}</option>`).join('')}</select></label>
          <label>Type<select id="assessType"><option>Quiz</option><option>Assignment</option><option>Speaking Performance</option><option>Writing Portfolio</option><option>Intercultural Project</option><option>Final Showcase</option></select></label>
          <label>Due Date<input id="assessDue" type="date"></label>
          <label>Title<input id="assessTitle" placeholder="Example: Week 4 Maritime Safety Briefing"></label>
          <label>Max Score<input id="assessScore" type="number" value="100"></label>
        </div>
        <label>Instructions<textarea id="assessInstructions" placeholder="Write task instructions, expected output, submission format, and evidence required."></textarea></label>
        <label>Rubric<textarea id="assessRubric">ESP accuracy 20; clarity and organization 20; professional tone 20; intercultural sensitivity 20; completion and reflection 20.</textarea></label>
        <div class="actions"><button class="btn primary" onclick="saveAssessmentV17()">Publish Assessment</button><button class="btn soft" onclick="loadAssessmentTemplateV17()">Use Template</button><button class="btn soft" onclick="navigate('materials')">Connect Materials</button></div>
      </section>
      <section class="panel">
        <h3>Published Assessments</h3>
        <div class="table-wrap"><table><thead><tr><th>Title</th><th>Course</th><th>Week</th><th>Type</th><th>Due</th><th>Submissions</th><th>Actions</th></tr></thead><tbody>
          ${db.tasks.length?db.tasks.map(t=>assessmentRowV17(t)).join(''):'<tr><td colspan="7">No published assessment yet.</td></tr>'}
        </tbody></table></div>
      </section>
      <section class="panel">
...
function assessmentRowV17(t){
  const course=courseTitle(t.courseId||t.disciplineId||selectedCourseId||'tourism');
  const subs=db.submissions.filter(s=>String(s.week)===String(t.week)).length;
  return `<tr><td><strong>${esc(t.title)}</strong><br><small>${esc(t.instructions||'')}</small></td><td>${esc(course)}</td><td>Week ${esc(t.week)}</td><td>${esc(t.type)}</td><td>${esc(t.due||t.dueDate||'-')}</td><td>${subs}</td><td><button class="btn soft" onclick="openAssessmentDetailsV17('${t.id}')">Details</button><button class="btn soft" onclick="deleteAssessmentV17('${t.id}')">Delete</button></td></tr>`;
}
function saveAssessmentV17(){
  const title=byId('assessTitle').value.trim();
  if(!title) return toast('Title needed','Please write the assessment title.');
  const t={id:uid('TASK'),courseId:byId('assessCourse').value,week:byId('assessWeek').value,type:byId('assessType').value,due:byId('assessDue').value,title,instructions:byId('assessInstructions').value.trim(),rubric:byId('assessRubric').value.trim(),maxScore:byId('assessScore').value,createdAt:new Date().toISOString()};
  db.tasks.unshift(t); save(); toast('Assessment published',title); renderAssessmentsV17();
}
function loadAssessmentTemplateV17(){
  byId('assessTitle').value='Professional ESP Performance Task';
  byId('assessInstructions').value='Submit one written product, one speaking recording, and one 100-word reflection connecting ESP communication with Indonesian local wisdom.';
  byId('assessRubric').value='ESP terminology and accuracy 20; clarity and organization 20; professional tone 20; intercultural sensitivity 20; completion, delivery, and reflection 20.';
}
function openAssessmentDetailsV17(id){const t=db.tasks.find(x=>x.id===id);if(!t)return;openModal(`<h2>${esc(t.title)}</h2><p><strong>Course:</strong> ${esc(courseTitle(t.courseId||'tourism'))} • <strong>Week:</strong> ${esc(t.week)} • <strong>Type:</strong> ${esc(t.type)}</p><p>${esc(t.instructions)}</p><p><strong>Rubric:</strong> ${esc(t.rubric)}</p>`)}
function deleteAssessmentV17(id){if(!confirm('Delete this assessment?'))return;db.tasks=db.tasks.filter(t=>t.id!==id);save();renderAssessmentsV17();}

/* ---------- Users, Students, Lecturers: separate pages ---------- */
function renderUsersAndInstitutionsV17(){
  const institutions=db.institutions.length?db.institutions:['Not specified'];
  byId('mainView').innerHTML=`
    <section class="userinst-v17">
      <section class="panel">
        <h3>Users & Institutions</h3>
        <p>Institution-level governance for registration, account mapping, course enrolment, lecturer approval, and learning evidence.</p>
        <div class="field-grid two">
          <label>Institution Name<input id="instNameV17" placeholder="Add institution name"></label>
          <button class="btn primary" onclick="addInstitutionV17()" style="align-self:end">Add Institution</button>
        </div>
        <div class="institution-tags-v17">${institutions.map(i=>`<span class="tag">${esc(i)}</span>`).join(' ')}</div>
      </section>
      <section class="panel">
        <h3>Institution Mapping Table</h3>
        ${institutionReport()}
      </section>
      <section class="panel">
        <h3>Account Mapping Actions</h3>
        <div class="actions"><button class="btn primary" onclick="navigate('students')">Manage Students</button><button class="btn soft" onclick="navigate('instructors')">Approve Lecturers</button><button class="btn soft" onclick="openCreateUser()">Create User</button><button class="btn soft" onclick="exportData()">Export Data</button></div>
      </section>
    </section>`;
}
function addInstitutionV17(){const input=byId('instNameV17');const v=input.value.trim();if(!v)return toast('Institution needed');if(!db.institutions.includes(v))db.institutions.push(v);save();toast('Institution added',v);renderUsersAndInstitutionsV17();}
function renderStudentsV17(){
  const students=db.users.filter(u=>u.role==='student');
  byId('mainView').innerHTML=`
    <section class="panel">
      <h3>Students</h3>
      <p>Student roster connected to institutions, ESP discipline enrolment, course progress, submissions, attendance, and reports.</p>
      <div class="grid cols-4">${metric('Students',students.length,'Registered learners')}${metric('Institutions',db.institutions.length,'Mapped institutions')}${metric('Submissions',db.submissions.length,'Learning evidence')}${metric('Attendance',db.rooms.reduce((a,r)=>a+(r.attendance||[]).length,0),'Room joins')}</div>
    </section>
    <section class="panel">
      <h3>Student Roster</h3>
      <div class="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Institution</th><th>ESP Discipline</th><th>Progress</th><th>Submissions</th><th>Attendance</th><th>Actions</th></tr></thead><tbody>
        ${students.length?students.map(studentRowV17).join(''):'<tr><td colspan="8">No student accounts yet.</td></tr>'}
      </tbody></table></div>
    </section>`;
}
function studentRowV17(u){
  const prog=completion(u);
  const subs=db.submissions.filter(s=>s.userId===u.id||s.user===u.name).length;
  const att=attendanceCount(u.name||u.email);
  return `<tr><td><strong>${esc(u.name)}</strong></td><td>${esc(u.email)}</td><td>${esc(u.institution||'Not specified')}</td><td>${esc(courseTitle(u.disciplineId||'tourism'))}</td><td><div class="progress mini"><span style="width:${prog}%"></span></div>${prog}%</td><td>${subs}</td><td>${att}</td><td><button class="btn soft" onclick="openUserDetails('${u.id}')">Details</button><button class="btn soft" onclick="editUser('${u.id}')">Edit</button></td></tr>`;
}
function renderLecturersV17(){
  const lecturers=db.users.filter(u=>u.role==='lecturer');
  const pending=lecturers.filter(u=>u.status==='pending');
  byId('mainView').innerHTML=`
    <section class="panel lecturer-head-v17">
      <h3>Lecturers</h3>
      <p>Lecturer approval, discipline assignment, institution mapping, content-authoring authority, and course-management readiness.</p>
      <div class="actions"><button class="btn primary" onclick="bulkApproveLecturers()">Approve All Pending</button><button class="btn soft" onclick="openCreateUser()">Create Lecturer</button><button class="btn soft" onclick="navigate('builder')">Content Builder</button></div>
    </section>
    <section class="panel">
      <h3>Pending Lecturer Approval</h3>
      ${pending.length?pending.map(u=>lecturerCardV17(u,true)).join(''):'<div class="empty">No lecturer is waiting for approval.</div>'}
    </section>
    <section class="panel">
      <h3>Lecturer Accounts</h3>
      <div class="grid cols-2">${lecturers.length?lecturers.map(u=>lecturerCardV17(u,false)).join(''):'<div class="empty">No lecturer accounts yet.</div>'}</div>
    </section>`;
}
function lecturerCardV17(u){
  return `<article class="lecturer-card-v17">
    <h3>${esc(u.name||'Unnamed Lecturer')}</h3>
    <p>${esc(u.email||'No email')}</p>
    <p><span class="tag red">${esc(statusLabel(u.status))}</span> <span class="tag">${esc(u.institution||'Not specified')}</span></p>
    <p><strong>Discipline:</strong> ${esc(courseTitle(u.disciplineId||'tourism'))}</p>
    <div class="actions">
      ${u.status!=='approved'?`<button class="btn primary" onclick="approveLecturer('${u.id}');renderLecturersV17()">Approve</button>`:''}
      ${u.status!=='rejected'?`<button class="btn soft" onclick="rejectLecturer('${u.id}');renderLecturersV17()">Reject</button>`:''}
      <button class="btn soft" onclick="editUser('${u.id}')">Edit</button>
      <button class="btn soft" onclick="openUserDetails('${u.id}')">Details</button>
    </div>
  </article>`;
}

/* ---------- Analytics and Reports: separate outputs ---------- */
function renderSystemAnalyticsV17(){
  const students=db.users.filter(u=>u.role==='student');
  const lecturers=db.users.filter(u=>u.role==='lecturer');
  const attendance=db.rooms.reduce((a,r)=>a+(r.attendance||[]).length,0);
  byId('mainView').innerHTML=`
    <section class="analytics-v17">
      <section class="panel">
        <h3>System Analytics</h3>
        <p>Live operational analytics calculated from registrations, courses, materials, assessments, virtual rooms, attendance, submissions, forum, chat, and help-desk activity.</p>
        <div class="grid cols-4">${metric('Students',students.length,'Active learners')}${metric('Lecturers',lecturers.length,'Teaching accounts')}${metric('Submissions',db.submissions.length,'Learning evidence')}${metric('Attendance',attendance,'Room joins')}</div>
      </section>
      <section class="panel"><h3>Course Enrolment Distribution</h3>${courseDistributionV17()}</section>
      <section class="panel"><h3>Recent Activity Log</h3>${db.activity.slice(0,12).map(a=>`<div class="card"><strong>${esc(a.type)}</strong><p>${esc(a.detail)}</p><small>${new Date(a.date).toLocaleString()} • ${esc(a.role)}</small></div>`).join('')||'<div class="empty">No activity yet.</div>'}</section>
    </section>`;
}
function courseDistributionV17(){
  return `<div class="table-wrap"><table><thead><tr><th>ESP Discipline</th><th>Students</th><th>Lecturers</th><th>Materials</th><th>Assessments</th></tr></thead><tbody>${disciplines.map(d=>`<tr><td>${esc(d.title)}</td><td>${db.users.filter(u=>u.role==='student'&&u.disciplineId===d.id).length}</td><td>${db.users.filter(u=>u.role==='lecturer'&&u.disciplineId===d.id).length}</td><td>16 weeks</td><td>${db.tasks.filter(t=>t.courseId===d.id||t.disciplineId===d.id).length}</td></tr>`).join('')}</tbody></table></div>`;
}
function renderReportsV17(){
  byId('mainView').innerHTML=`
    <section class="reports-v17">
      <section class="panel">
        <h3>Reports Center</h3>
        <p>Generate professional reports for course progress, institution mapping, class attendance, assessment evidence, lecturer approval, and semester readiness.</p>
        <div class="report-grid-v17">
          ${reportCardV17('Institution Mapping Report','Users, institutions, roles, submissions, and room attendance.','openReportV17("institution")')}
          ${reportCardV17('Student Progress Report','Enrolment, weekly completion, quizzes, submissions, attendance, and portfolio evidence.','openReportV17("student")')}
          ${reportCardV17('Assessment Evidence Report','Published assessments, due dates, submissions, grades, and rubric evidence.','openReportV17("assessment")')}
          ${reportCardV17('Class Attendance Report','Class metadata, student lists, room attendance, and meeting participation.','openReportV17("attendance")')}
          ${reportCardV17('Semester Materials Readiness','Six ESP disciplines, 16-week materials, resources, and AI Tutor integration.','openReportV17("materials")')}
          ${reportCardV17('Export Full Data','Download the current platform database as JSON.','exportData()')}
        </div>
      </section>
    </section>`;
}
function reportCardV17(title,desc,action){return `<article class="report-card-v17"><h3>${esc(title)}</h3><p>${esc(desc)}</p><button class="btn primary" onclick='${action}'>Open Report</button></article>`}
function openReportV17(type){
  const html={
    institution:`<h2>Institution Mapping Report</h2>${institutionReport()}`,
    student:`<h2>Student Progress Report</h2>${studentReportTable(db.users.filter(u=>u.role==='student'))}`,
    assessment:`<h2>Assessment Evidence Report</h2><p>Published tasks: ${db.tasks.length}. Submissions: ${db.submissions.length}. Grades: ${db.grades.length}.</p>${submissionsTable()}`,
    attendance:`<h2>Class Attendance Report</h2>${classAttendanceTableV17()}`,
    materials:`<h2>Semester Materials Readiness</h2><p>Six ESP disciplines, sixteen weeks, six integrated skills, resources, quizzes, assignments, rubrics, Nusantara Atlas, and AI Tutor integration are available.</p>${courseDistributionV17()}`
  }[type]||'<h2>Report</h2>';
  openModal(html);
}

/* ---------- Class & Attendance integrated with metadata ---------- */
function ensureClassDataV17(){
  db.classes ||= [];
  if(!db.classes.length){
    db.classes = disciplines.map((d,i)=>({id:uid('CLS'),name:`Semester ${i+1} – ${d.title}`,courseId:d.id,institution:db.institutions[0]||'Not specified',lecturer:'Unassigned',schedule:`Week ${i+1} consultation`,students:db.users.filter(u=>u.role==='student'&&u.disciplineId===d.id).map(u=>u.id),createdAt:new Date().toISOString()}));
    save();
  }
}
function renderClass(){
  ensureClassDataV17();
  byId('mainView').innerHTML=`
    <section class="class-v17">
      <section class="panel">
        <h3>Class & Attendance</h3>
        <p>Integrated class metadata based on students, institutions, ESP disciplines, courses, online-room attendance, and assessment evidence.</p>
        <div class="grid cols-4">${metric('Classes',db.classes.length,'Course groups')}${metric('Students',db.users.filter(u=>u.role==='student').length,'Mapped learners')}${metric('Institutions',db.institutions.length,'Institution data')}${metric('Attendance',db.rooms.reduce((a,r)=>a+(r.attendance||[]).length,0),'Room joins')}</div>
      </section>
      <section class="panel">
        <h3>Create / Map Class</h3>
        <div class="field-grid two">
          <label>Class Name<input id="classNameV17" placeholder="Example: ESP Tourism Class A"></label>
          <label>ESP Course<select id="classCourseV17">${disciplines.map(d=>`<option value="${d.id}">${esc(d.title)}</option>`).join('')}</select></label>
          <label>Institution<select id="classInstitutionV17"><option>Not specified</option>${db.institutions.map(i=>`<option>${esc(i)}</option>`).join('')}</select></label>
          <label>Lecturer<select id="classLecturerV17"><option>Unassigned</option>${db.users.filter(u=>u.role==='lecturer').map(u=>`<option>${esc(u.name)}</option>`).join('')}</select></label>
          <label>Schedule<input id="classScheduleV17" placeholder="Monday 09:00 WIB"></label>
          <button class="btn primary" onclick="createClassV17()" style="align-self:end">Create Class</button>
        </div>
      </section>
      <section class="panel">
        <h3>Class Roster & Attendance</h3>
        ${classAttendanceTableV17()}
      </section>
    </section>`;
}
function createClassV17(){
  ensureClassDataV17();
  const name=byId('classNameV17').value.trim();
  if(!name) return toast('Class name needed');
  const courseId=byId('classCourseV17').value;
  const students=db.users.filter(u=>u.role==='student'&&u.disciplineId===courseId).map(u=>u.id);
  db.classes.unshift({id:uid('CLS'),name,courseId,institution:byId('classInstitutionV17').value,lecturer:byId('classLecturerV17').value,schedule:byId('classScheduleV17').value,students,attendance:[],createdAt:new Date().toISOString()});
  save();toast('Class created',name);renderClass();
}
function classAttendanceTableV17(){
  ensureClassDataV17();
  return `<div class="table-wrap"><table><thead><tr><th>Class</th><th>Course</th><th>Institution</th><th>Lecturer</th><th>Students</th><th>Schedule</th><th>Room Attendance</th><th>Actions</th></tr></thead><tbody>
    ${db.classes.map(c=>{const stu=db.users.filter(u=>(c.students||[]).includes(u.id));const roomAtt=db.rooms.reduce((a,r)=>a+(r.attendance||[]).filter(x=>stu.some(s=>s.name===x.user||s.email===x.user)).length,0);return `<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(courseTitle(c.courseId))}</td><td>${esc(c.institution)}</td><td>${esc(c.lecturer)}</td><td>${stu.length}</td><td>${esc(c.schedule||'-')}</td><td>${roomAtt}</td><td><button class="btn soft" onclick="openClassDetailsV17('${c.id}')">Roster</button><button class="btn soft" onclick="markClassAttendanceV17('${c.id}')">Mark Attendance</button></td></tr>`}).join('')}
  </tbody></table></div>`;
}
function openClassDetailsV17(id){
  const c=db.classes.find(x=>x.id===id);if(!c)return;
  const stu=db.users.filter(u=>(c.students||[]).includes(u.id));
  openModal(`<h2>${esc(c.name)}</h2><p>${esc(courseTitle(c.courseId))} • ${esc(c.institution)} • Lecturer: ${esc(c.lecturer)}</p><div class="table-wrap"><table><thead><tr><th>Student</th><th>Email</th><th>Progress</th><th>Submissions</th></tr></thead><tbody>${stu.map(s=>`<tr><td>${esc(s.name)}</td><td>${esc(s.email)}</td><td>${completion(s)}%</td><td>${db.submissions.filter(x=>x.userId===s.id||x.user===s.name).length}</td></tr>`).join('')||'<tr><td colspan="4">No mapped students.</td></tr>'}</tbody></table></div>`);
}
function markClassAttendanceV17(id){
  const c=db.classes.find(x=>x.id===id);if(!c)return;
  c.attendance ||= [];
  c.attendance.push({date:new Date().toISOString(),markedBy:session.name||session.role,count:(c.students||[]).length});
  save();toast('Attendance marked',`${c.name}: ${(c.students||[]).length} student(s) recorded.`);renderClass();
}

/* ---------- AI Tutor fully working from prompt OR canvas ---------- */
function renderAITutor(){
  const u=currentUser()||{};
  const courseId=selectedCourseId || u.disciplineId || 'tourism';
  const m=courseMaterial(courseId, selectedWeek || 1);
  const province=selectedProvince();
  const prompt = localStorage.getItem('gesn.tutor.prompt') || {
    speaking:`Prepare a professional presentation about ${province.heritage} for international audiences.`,
    writing:`Write a professional 180–220 word ESP response about ${province.heritage} for ${m.discipline}.`,
    pronunciation:`Read aloud: This local wisdom supports professional communication for international audiences.`,
    feedback:`Paste your speaking transcript or writing draft for ESP feedback.`
  ...
  byId('mainView').innerHTML=`
    <section class="ai17-layout">
      <aside class="ai17-showcase panel">
        <img src="assets/ai-tutor-reference-full.png" alt="Garuda AI Tutor">
        <div class="ai17-quick">
          <button class="${selectedTutorMode==='speaking'?'active':''}" onclick="setTutorModeV17('speaking')">Practice Speaking</button>
          <button class="${selectedTutorMode==='writing'?'active':''}" onclick="setTutorModeV17('writing')">Improve Writing</button>
          <button class="${selectedTutorMode==='pronunciation'?'active':''}" onclick="setTutorModeV17('pronunciation')">Check Pronunciation</button>
          <button class="${selectedTutorMode==='feedback'?'active':''}" onclick="setTutorModeV17('feedback')">Get Feedback</button>
        </div>
      </aside>
      <section class="ai17-work panel">
        <div class="ai17-head"><h3>AI Tutor Workspace</h3><p>Write a prompt or a response. The local AI Tutor will generate an ESP response and feedback without external API keys.</p></div>
        <div class="ai17-meta">
          <label>Course<select onchange="selectedCourseId=this.value;localStorage.setItem('gesn.selectedCourseId',selectedCourseId);renderAITutor()">${disciplines.map(d=>`<option value="${d.id}" ${d.id===courseId?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label>
          <label>Week<select onchange="selectedWeek=Number(this.value);renderAITutor()">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${selectedWeek===i+1?'selected':''}>Week ${i+1}</option>`).join('')}</select></label>
          <label>Province<select onchange="selectedProvinceId=this.value;localStorage.setItem('gesn.selectedProvinceId',selectedProvinceId);renderAITutor()">${provinceAtlas.map(p=>`<option value="${p.id}" ${p.id===province.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label>
          <label>Mode<select onchange="setTutorModeV17(this.value)"><option value="speaking" ${selectedTutorMode==='speaking'?'selected':''}>Speaking</option><option value="writing" ${selectedTutorMode==='writing'?'selected':''}>Writing</option><option value="pronunciation" ${selectedTutorMode==='pronunciation'?'selected':''}>Pronunciation</option><option value="feedback" ${selectedTutorMode==='feedback'?'selected':''}>Feedback</option></select></label>
        </div>
        <label>Your Prompt<input id="tutorPrompt" value="${esc(prompt)}" maxlength="320" oninput="localStorage.setItem('gesn.tutor.prompt',this.value);updatePromptCountV17()"><small id="promptCount">0 / 320</small></label>
        <label>Your Response / Canvas<textarea id="tutorInput" placeholder="Type your answer here, or leave it blank and click Generate / Analyze to get a model response from your prompt.">${esc(localStorage.getItem('gesn.tutor.lastText')||'')}</textarea></label>
        <div class="ai17-actions">
          <button class="btn primary" onclick="runTutorAnalysis()">Generate / Analyze</button>
          <button class="btn gold" onclick="speak(byId('tutorInput').value || byId('tutorPrompt').value)">Play</button>
          <button class="btn dark" onclick="stopSpeak()">Stop</button>
          <button class="btn primary" onclick="recordSpeech('tutorRecordOut')">Record</button>
          <button class="btn soft" onclick="insertTutorSample()">Insert Sample</button>
          <button class="btn soft" onclick="clearTutorInput()">Clear</button>
          <button class="btn gold" onclick="saveTutorNote()">Save Note</button>
          <button class="btn soft" onclick="openTutorHistory()">History</button>
        </div>
        <div id="tutorRecordOut"></div>
        <div id="tutorResult" class="ai17-result">${localStorage.getItem('gesn.tutor.lastResult')||'<div class="empty">AI Tutor response and feedback will appear here.</div>'}</div>
      </section>
    </section>`;
  updatePromptCountV17();
}
function setTutorModeV17(mode){
  selectedTutorMode=mode;localStorage.setItem('gesn.selectedTutorMode',mode);
  localStorage.removeItem('gesn.tutor.prompt');
  renderAITutor();
}
function updatePromptCountV17(){const el=byId('tutorPrompt'),c=byId('promptCount');if(el&&c)c.textContent=`${el.value.length} / 320`;}
function generateTutorModelResponseV17(prompt,m,province){
  return `Good morning. I would like to present ${province.heritage} from ${province.name} as a meaningful example of Indonesian local wisdom. In the context of ${m.discipline}, this heritage can be communicated professionally by explaining its cultural value, practical relevance, and benefit for international audiences. First, it reflects community knowledge and respectful interaction. Second, it can support clearer workplace communication because the speaker connects local meaning with a real professional task. For example, a learner may use this topic to guide visitors, support clients, explain procedures, promote sustainable practice, or deliver an educational message. I recommend using polite expressions, accurate ESP vocabulary, and a clear structure: opening, context, explanation, recommendation, and closing. This approach helps audiences understand Nusantara heritage while also developing professional English competence. Thank you for your attention. I would be pleased to answer any questions.`;
}
function runTutorAnalysis(){
  const prompt=(byId('tutorPrompt')?.value||'').trim();
  let text=(byId('tutorInput')?.value||'').trim();
  const courseId=selectedCourseId||'tourism';
  const m=courseMaterial(courseId,selectedWeek||1);
  const province=selectedProvince();
  let generated='';
  if(!text){
    generated=generateTutorModelResponseV17(prompt,m,province);
    text=generated;
    byId('tutorInput').value=text;
  }
  localStorage.setItem('gesn.tutor.prompt',prompt);
  localStorage.setItem('gesn.tutor.lastText',text);
  const words=text.split(/\s+/).filter(Boolean);
  const polite=(text.match(/\b(may|could|would|please|recommend|thank you|I would like|may I)\b/gi)||[]).length;
  const esp=(text.match(/\b(professional|audience|communication|service|heritage|local wisdom|intercultural|clarity|task|context|community|discipline|visitor|client|patient|technical|sustainability|procedure|portfolio)\b/gi)||[]).length;
  const sentenceCount=(text.match(/[.!?]+/g)||[]).length;
  const local=(text.match(/\b(local wisdom|heritage|culture|community|values|Indonesia|Nusantara|province|Bali|Papua|Kalimantan|Sulawesi|Java|Sumatra|Maluku)\b/gi)||[]).length;
  const clarity=Math.min(96,55+Math.min(26,Math.round(words.length/6))+Math.min(15,sentenceCount*3));
  const tone=Math.min(96,60+polite*8);
  const vocab=Math.min(96,58+esp*5);
  const culture=Math.min(96,55+local*9);
  const score=Math.round((clarity+tone+vocab+culture)/4);
  const grade=score>=90?'A':score>=80?'B':score>=70?'C':'D';
  const comments=[
    words.length<80?'Develop the response with fuller details, examples, and a clearer closing.':'Length is adequate for ESP performance practice.',
    polite<2?'Add polite expressions such as “May I explain,” “I would recommend,” and “Thank you for your attention.”':'Professional politeness is visible.',
    esp<5?'Add more discipline-specific ESP vocabulary.':'ESP vocabulary is meaningfully included.',
    local<2?'Make the Indonesian local-wisdom connection more explicit.':'Local wisdom and cultural relevance are present.',
    sentenceCount<4?'Use clearer sequence: opening, context, explanation, recommendation, closing.':'Organization shows an understandable sequence.'
  ];
  const html=`<section class="feedback-card"><h4>${generated?'Generated Model Response + Feedback':'AI Tutor Feedback'}</h4>${generated?`<p><strong>Model response generated from your prompt:</strong></p><div class="model-response-v17">${esc(generated)}</div>`:''}<p><strong>Overall Score:</strong> ${score}% (${grade})</p><div class="feedback-score"><span>Clarity ${clarity}%</span><span>Professional Tone ${tone}%</span><span>ESP Vocabulary ${vocab}%</span><span>Intercultural Link ${culture}%</span></div><ul class="check-list">${comments.map(c=>`<li>${esc(c)}</li>`).join('')}</ul><p><strong>Next revision target:</strong> Strengthen discipline-specific terminology, improve coherence, and connect the task explicitly to Nusantara local wisdom.</p></section>`;
  localStorage.setItem('gesn.tutor.lastResult',html);
  byId('tutorResult').innerHTML=html;
  toast('AI Tutor ready',generated?'A model response and feedback have been generated.':'Feedback has been generated.');
}
function insertTutorSample(){
  const m=courseMaterial(selectedCourseId||'tourism',selectedWeek||1);
  const p=selectedProvince();
  byId('tutorInput').value=generateTutorModelResponseV17(byId('tutorPrompt')?.value||'',m,p);
  localStorage.setItem('gesn.tutor.lastText',byId('tutorInput').value);
}
function clearTutorInput(){
  if(byId('tutorInput')) byId('tutorInput').value='';
  if(byId('tutorResult')) byId('tutorResult').innerHTML='<div class="empty">AI Tutor response and feedback will appear here.</div>';
  localStorage.removeItem('gesn.tutor.lastText');localStorage.removeItem('gesn.tutor.lastResult');
  toast('Cleared','AI Tutor canvas cleared.');
}
function saveTutorNote(){
  const text=byId('tutorInput')?.value?.trim()||'';
  if(!text) return toast('No text','Generate or write a response first.');
  const notes=JSON.parse(localStorage.getItem('gesn.tutor.notes')||'[]');
  notes.unshift({date:new Date().toISOString(),mode:selectedTutorMode,course:courseTitle(selectedCourseId||'tourism'),province:selectedProvince().name,text,score:tutorScoreEstimate()});
  localStorage.setItem('gesn.tutor.notes',JSON.stringify(notes.slice(0,50)));
  toast('Saved','AI Tutor note saved to History.');
}
function openTutorHistory(){
  const notes=JSON.parse(localStorage.getItem('gesn.tutor.notes')||'[]');
  openModal(`<h2>AI Tutor History</h2>${notes.length?notes.map(n=>`<div class="card"><strong>${esc(n.mode)} • ${esc(n.course)} • ${esc(n.province)}</strong><p>${esc(n.text.slice(0,260))}${n.text.length>260?'...':''}</p><span class="tag">${new Date(n.date).toLocaleString()}</span></div>`).join(''):'<div class="empty">No AI Tutor history yet.</div>'}`);
}
function openTutorGoals(){openModal(`<h2>AI Tutor Goals</h2><ul class="check-list"><li>Use discipline-specific ESP vocabulary.</li><li>Connect local wisdom to professional tasks.</li><li>Improve clarity, tone, organization, and intercultural sensitivity.</li><li>Save at least one practice note each week.</li></ul>`)}
function openTutorSettings(){openModal(`<h2>AI Tutor Settings</h2><p>The AI Tutor works locally without an external API key. It generates model responses and feedback using rule-based ESP guidance.</p><button class="btn primary" onclick="closeModal()">Done</button>`)}

/* ---------- Diagnostics ---------- */
function runFeatureDiagnosticsV17(){
  const htmlChecks=[
    ['Different Materials page', typeof renderMaterials==='function'],
    ['Different Builder page', typeof renderBuilder==='function'],
    ['Different Assessment page', typeof renderAssessmentsV17==='function'],
    ['Users page', typeof renderUsersAndInstitutionsV17==='function'],
    ['Students page', typeof renderStudentsV17==='function'],
    ['Lecturers page', typeof renderLecturersV17==='function'],
    ['Analytics page', typeof renderSystemAnalyticsV17==='function'],
    ['Reports page', typeof renderReportsV17==='function'],
    ['Class Attendance page', typeof renderClass==='function'],
    ['AI Tutor generator', typeof generateTutorModelResponseV17==='function'],
    ['Professional room link', typeof generateRoomLink==='function' && generateRoomLink('Test','2026-06-08','09:00').includes('room.garudaesp.id')]
  ];
  return htmlChecks;
}
Object.assign(window,{toggleNavGroupV17,renderNav,navigate,renderMaterials,renderBuilder,renderAssessmentsV17,renderTasks:renderAssessmentsV17,renderUsersAndInstitutionsV17,renderStudentsV17,renderLecturersV17,renderSystemAnalyticsV17,renderReportsV17,openReportV17,renderClass,createClassV17,openClassDetailsV17,markClassAttendanceV17,renderAITutor,runTutorAnalysis,insertTutorSample,clearTutorInput,saveTutorNote,openTutorHistory,openTutorGoals,openTutorSettings,setTutorModeV17,runFeatureDiagnosticsV17});


// ===== FINAL V18: working in-app rooms, role-correct assessments, profile photo upload =====
const ROOM_BASE_URL_V18 = 'https://room.garudaesp.id';
let editingRoomIdV18 = null;

function userPhotoV18(user=currentUser()){
  return user?.photoData || user?.avatar || 'assets/logo-square.png';
}
function professionalNameV18(){
  const u=currentUser()||{};
  return session?.role==='admin'?'Garuda Admin':(u.name||session?.name||'Workspace User');
}
function roleNameV18(){
  return session?.role==='admin'?'Super Administrator':(session?.role==='lecturer'?'Lecturer':session?.role==='student'?'Student':'Member');
}
function workspaceProfileHTML(){
  const u=currentUser()||{};
  return `<button class="profile-chip-v18" onclick="openProfileEditorV18()"><img src="${esc(userPhotoV18(u))}" alt="Profile"><div><strong>${esc(professionalNameV18())}</strong><small>${esc(roleNameV18())}</small></div><span>▾</span></button>`;
}
function renderUserCard(){
  const u=currentUser()||{};
  const card=byId('userCard');
  if(!card) return;
  if(session?.role==='admin'){card.innerHTML='';return;}
  card.innerHTML=`<div class="side-profile-v18"><img src="${esc(userPhotoV18(u))}" alt="Profile"><strong>${esc(u.name||session.name)}</strong><span>${esc(roleNameV18())}</span><small>${esc(courseTitle(u.disciplineId||'tourism'))}<br>${esc(u.institution||'Not specified')}</small><button onclick="openProfileEditorV18()">Edit Profile</button></div>`;
}
function applyWorkspaceChrome(){
  const top = document.querySelector('.top-actions');
  if(top){
...
function openProfileEditorV18(){
  if(!session || session.role==='admin'){
    openModal(`<h2>Admin Profile</h2><p>Administrator profile is protected by Admin PIN. User photo upload is available for students and lecturers.</p><button class="btn primary" onclick="closeModal()">Done</button>`);
    return;
  }
  const u=db.users.find(x=>x.id===session.id);
  if(!u) return toast('Profile not found');
  openModal(`<h2>Edit Profile</h2>
    <section class="profile-editor-v18">
      <div class="profile-photo-v18"><img id="profilePreviewV18" src="${esc(userPhotoV18(u))}" alt="Profile photo"><label class="upload-pill-v18">Upload Photo<input id="profilePhotoFileV18" type="file" accept="image/*" onchange="previewProfilePhotoV18(this)"></label><input id="profilePhotoDataV18" type="hidden" value="${esc(u.photoData||'')}"></div>
      <div class="field-grid two">
        <label>Full Name<input id="profileNameV18" value="${esc(u.name||'')}"></label>
        <label>Email<input id="profileEmailV18" value="${esc(u.email||'')}"></label>
        <label>Institution<input id="profileInstitutionV18" value="${esc(u.institution||'Not specified')}"></label>
        <label>${u.role==='lecturer'?'Expertise':'ESP Discipline'}${u.role==='lecturer'?`<input id="profileExpertiseV18" value="${esc(u.expertise||courseTitle(u.disciplineId||'tourism'))}">`:`<select id="profileDisciplineV18">${disciplines.map(d=>`<option value="${d.id}" ${u.disciplineId===d.id?'selected':''}>${esc(d.title)}</option>`).join('')}</select>`}</label>
        <label>Short Bio<textarea id="profileBioV18" placeholder="Write a short academic or learning profile.">${esc(u.bio||'')}</textarea></label>
        <label>Password<input id="profilePasswordV18" value="${esc(u.password||'')}"></label>
      </div>
      <div class="actions"><button class="btn primary" onclick="saveProfileV18()">Save Profile</button><button class="btn soft" onclick="closeModal()">Cancel</button></div>
    </section>`);
}
function previewProfilePhotoV18(input){
  const file=input.files && input.files[0];
  if(!file) return;
  if(file.size>1024*1024*2){toast('Image too large','Please use an image below 2 MB.');input.value='';return;}
  const reader=new FileReader();
  reader.onload=e=>{byId('profilePreviewV18').src=e.target.result;byId('profilePhotoDataV18').value=e.target.result;};
  reader.readAsDataURL(file);
}
function saveProfileV18(){
  const u=db.users.find(x=>x.id===session.id);
  if(!u) return toast('Profile not found');
  const name=byId('profileNameV18').value.trim();
  const email=byId('profileEmailV18').value.trim().toLowerCase();
  if(!name||!email) return toast('Name and email required');
  if(db.users.some(x=>x.id!==u.id && x.email===email)) return toast('Email already used');
  u.name=name; u.email=email; u.institution=byId('profileInstitutionV18').value.trim()||'Not specified';
  u.bio=byId('profileBioV18').value.trim(); u.password=byId('profilePasswordV18').value || u.password;
  const photo=byId('profilePhotoDataV18').value; if(photo) u.photoData=photo;
  if(u.role==='student') u.disciplineId=byId('profileDisciplineV18').value;
  if(u.role==='lecturer') u.expertise=byId('profileExpertiseV18').value.trim();
  session.name=u.name; session.email=u.email; localStorage.setItem('gesn.session.final',JSON.stringify(session));
  save(); closeModal(); renderUserCard(); applyWorkspaceChrome(); toast('Profile updated','Your profile and photo have been saved.');
  if(currentPage) navigate(currentPage);
}

// ---------- Professional room links + in-app join console ----------
function v18MonthNames(){return ['January','February','March','April','May','June','July','August','September','October','November','December'];}
function v18MonthNumber(month){const i=v18MonthNames().indexOf(month);return String((i>=0?i:0)+1).padStart(2,'0');}
function v18Today(){const d=new Date();return {day:String(d.getDate()).padStart(2,'0'),month:v18MonthNames()[d.getMonth()],year:String(d.getFullYear()),iso:d.toISOString().slice(0,10)};}
function v18DateISO(day,month,year){return `${year}-${v18MonthNumber(month)}-${String(day||'01').padStart(2,'0')}`;}
function v18Hash(s=''){let h=2166136261;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619);}const chars='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';let out='';h=Math.abs(h);for(let i=0;i<4;i++){out+=chars[h%chars.length];h=Math.floor(h/chars.length);}return out||'A91K';}
function roomCodeFrom(title='',dateValue='',timeValue=''){
  let y,m,d;
  if(/^\d{4}-\d{2}-\d{2}$/.test(dateValue||'')){[y,m,d]=dateValue.split('-');}
  else{const t=v18Today();y=t.year;m=v18MonthNumber(t.month);d=t.day;}
  return `RM-${String(y).slice(-2)}${m}${d}-${v18Hash(`${title}|${dateValue}|${timeValue}`)}`;
}
function generateRoomLink(title='',dateValue='',timeValue=''){
  return `${ROOM_BASE_URL_V18}/${roomCodeFrom(title||'GARUDA ESP Room',dateValue||v18Today().iso,timeValue||'09:00')}`;
}
function normalizeRoomV18(r){
  const t=v18Today();
  r.attendance ||= [];
  r.features = Array.isArray(r.features)?r.features:String(r.features||'Video, Audio, Chat, Screen Share, Whiteboard, Breakout Rooms, Recording').split(',').map(x=>x.trim()).filter(Boolean);
  if(!r.day||!r.month||!r.year){
    if(r.date && /^\d{4}-\d{2}-\d{2}$/.test(r.date)){const [y,m,d]=r.date.split('-');r.year=y;r.month=v18MonthNames()[Number(m)-1]||t.month;r.day=d;}
    else{r.day=t.day;r.month=t.month;r.year=t.year;}
  }
  r.startTime ||= '09:00'; r.endTime ||= '10:30'; r.timezone ||= 'Asia/Jakarta (GMT+7)';
  r.date = v18DateISO(r.day,r.month,r.year);
  r.code = r.code && String(r.code).startsWith('RM-') ? r.code : roomCodeFrom(r.title||'GARUDA ESP Room',r.date,r.startTime);
  r.link = `${ROOM_BASE_URL_V18}/${r.code}`;
  r.scope ||= 'all'; r.courseId ||= selectedCourseId || 'tourism'; r.week ||= selectedWeek || 1; r.type ||= 'Virtual Meeting (Live)';
  return r;
}
function normalizeRoomsV18(){db.rooms=(db.rooms||[]).map(normalizeRoomV18);save();}
function roomDateLabel(r){r=normalizeRoomV18(r);return `${String(r.day).padStart(2,'0')} ${r.month} ${r.year} • ${r.startTime}${r.endTime?' – '+r.endTime:''} ${r.timezone||''}`;}
function syncRoomLinkV18(){
  const title=byId('roomTitle')?.value||'GARUDA ESP Room';
  const day=String(byId('roomDay')?.value||v18Today().day).padStart(2,'0');
  const month=byId('roomMonth')?.value||v18Today().month;
  const year=byId('roomYear')?.value||v18Today().year;
  const start=byId('roomStartTime')?.value||'09:00';
  const date=v18DateISO(day,month,year);
  const code=roomCodeFrom(title,date,start); const link=`${ROOM_BASE_URL_V18}/${code}`;
  if(byId('roomDate')) byId('roomDate').value=date;
  if(byId('roomCode')) byId('roomCode').value=code;
  if(byId('roomLink')) byId('roomLink').value=link;
  if(byId('roomLinkText')) byId('roomLinkText').textContent=link;
}
function roomForm(room=null){
  room = room?normalizeRoomV18(room):null; editingRoomIdV18=room?.id||null;
  const t=v18Today(); const day=room?.day||t.day, month=room?.month||t.month, year=room?.year||t.year;
  const features=room?.features||['Video','Audio','Chat','Screen Share','Whiteboard','Breakout Rooms','Recording','Attendance'];
  const allFeatures=['Video','Audio','Chat','Screen Share','Whiteboard','Breakout Rooms','Recording','Attendance','Consultation','Assessment Feedback'];
  setTimeout(syncRoomLinkV18,50);
  return `<section class="online18-card" id="roomFormPanel">
    <div class="online18-card-head"><div><h3>${room?'Edit Virtual Room':'Create Virtual Room'}</h3><p>Create a professional in-app room with metadata, attendance, copy/share short link, and working join console.</p></div><button class="btn soft" onclick="resetRoomFormV18()">Reset</button></div>
    <input id="roomDate" type="hidden" value="${esc(v18DateISO(day,month,year))}">
    <div class="online18-grid">
      <label>Meeting Title *<input id="roomTitle" value="${esc(room?.title||'')}" placeholder="Week 5 – Technical Report Writing Workshop" oninput="syncRoomLinkV18()"></label>
      <label>Host / Facilitator *<input id="roomHost" value="${esc(room?.host||session.name||'')}" placeholder="Host name"></label>
      <label>Day *<input id="roomDay" type="number" min="1" max="31" value="${esc(String(day).replace(/^0/,''))}" oninput="syncRoomLinkV18()"></label>
      <label>Month *<select id="roomMonth" onchange="syncRoomLinkV18()">${v18MonthNames().map(m=>`<option ${m===month?'selected':''}>${m}</option>`).join('')}</select></label>
      <label>Year *<select id="roomYear" onchange="syncRoomLinkV18()">${Array.from({length:8},(_,i)=>String(2026+i)).map(y=>`<option ${y===String(year)?'selected':''}>${y}</option>`).join('')}</select></label>
      <label>Start Time *<input id="roomStartTime" type="time" value="${esc(room?.startTime||'09:00')}" oninput="syncRoomLinkV18()"></label>
      <label>End Time *<input id="roomEndTime" type="time" value="${esc(room?.endTime||'10:30')}"></label>
      <label>Timezone *<select id="roomTimezone">${['Asia/Jakarta (GMT+7)','WIB','WITA','WIT','UTC'].map(z=>`<option ${room?.timezone===z?'selected':''}>${z}</option>`).join('')}</select></label>
      <label>Related ESP Course *<select id="roomCourse">${disciplines.map(d=>`<option value="${d.id}" ${room?.courseId===d.id?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label>
      <label>Related Week *<select id="roomWeek">${Array.from({length:16},(_,i)=>`<option value="${i+1}" ${String(room?.week||'')===String(i+1)?'selected':''}>Week ${i+1}</option>`).join('')}</select></label>
      <label>Meeting Type *<select id="roomType">${['Virtual Meeting (Live)','Speaking Practice','Writing Workshop','Assessment Feedback','Project Consultation','Lecturer Coordination','Student Support'].map(t=>`<option ${room?.type===t?'selected':''}>${t}</option>`).join('')}</select></label>
      <label>Scope *<select id="roomScope"><option value="all" ${room?.scope==='all'?'selected':''}>All roles</option><option value="student" ${room?.scope==='student'?'selected':''}>Students</option><option value="lecturer" ${room?.scope==='lecturer'?'selected':''}>Lecturers</option><option value="admin" ${room?.scope==='admin'?'selected':''}>Admin</option></select></label>
      <label class="room-code-box">Room Code<input id="roomCode" value="${esc(room?.code||'')}" readonly></label>
      <label>Agenda *<textarea id="roomAgenda" placeholder="Meeting agenda, learning goals, expected output, and follow-up task.">${esc(room?.agenda||'')}</textarea></label>
      <label>Notes<textarea id="roomNotes" placeholder="Preparation notes, materials, or attendance instructions.">${esc(room?.notes||'')}</textarea></label>
      <div class="feature-checks-v18"><strong>Features</strong>${allFeatures.map(f=>`<button type="button" class="${features.includes(f)?'selected':''}" onclick="this.classList.toggle('selected')">${esc(f)}</button>`).join('')}</div>
      <div class="room-link-panel-v18"><div><small>Professional Room Link</small><strong id="roomLinkText">${esc(room?.link||'')}</strong></div><input id="roomLink" value="${esc(room?.link||'')}" readonly><button class="btn soft" onclick="copyGeneratedRoomLink()">Copy</button><button class="btn primary" onclick="shareGeneratedRoomLink()">Share</button></div>
    </div>
    <div class="actions end"><button class="btn soft" onclick="previewRoomMetadata()">Preview Metadata</button><button class="btn primary" onclick="saveRoom()">${room?'Update Room':'Create Room'}</button>${room?`<button class="btn soft" onclick="editingRoomIdV18=null;renderOnline()">Cancel Edit</button>`:''}</div>
  </section>`;
}
function selectedRoomFeaturesV18(){return [...document.querySelectorAll('.feature-checks-v18 button.selected')].map(b=>b.textContent.trim());}
function resetRoomFormV18(){editingRoomIdV18=null;renderOnline();}
function saveRoom(){
  if(!['admin','lecturer'].includes(session.role)) return toast('Unauthorized','Only lecturers and admins can create rooms.');
  syncRoomLinkV18();
  const title=byId('roomTitle').value.trim(), host=byId('roomHost').value.trim(), agenda=byId('roomAgenda').value.trim();
  if(!title||!host||!agenda) return toast('Incomplete','Complete meeting title, host, and agenda.');
  const old=editingRoomIdV18?db.rooms.find(r=>r.id===editingRoomIdV18):null;
  const room=normalizeRoomV18({
    id:editingRoomIdV18||uid('ROOM'), title, host, agenda, scope:byId('roomScope').value,
    day:String(byId('roomDay').value||v18Today().day).padStart(2,'0'), month:byId('roomMonth').value, year:byId('roomYear').value,
    date:byId('roomDate').value, startTime:byId('roomStartTime').value, endTime:byId('roomEndTime').value, timezone:byId('roomTimezone').value,
    courseId:byId('roomCourse').value, week:byId('roomWeek').value, type:byId('roomType').value,
    code:byId('roomCode').value, link:byId('roomLink').value, notes:byId('roomNotes').value.trim(), features:selectedRoomFeaturesV18(),
    attendance:old?.attendance||[], roomChat:old?.roomChat||[], createdBy:old?.createdBy||session.name||session.role,
    createdAt:old?.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString()
  });
  if(editingRoomIdV18){const i=db.rooms.findIndex(r=>r.id===editingRoomIdV18); if(i>=0) db.rooms[i]=room; toast('Room updated','Meeting metadata and room console updated.');}
  else{db.rooms.unshift(room); toast('Room created','Professional link and in-app room are ready.');}
  editingRoomIdV18=null; save(); renderOnline();
}
function renderOnline(){
  normalizeRoomsV18();
  const manage=['admin','lecturer'].includes(session.role);
  const rooms=db.rooms.filter(r=>r.scope==='all'||r.scope===session.role||!r.scope);
  byId('mainView').innerHTML=`<section class="online-v18">
    ${manage?roomForm():`<section class="online18-card"><h3>Online Room</h3><p>Join scheduled ESP virtual rooms. Attendance is recorded automatically when you enter the room console.</p></section>`}
    <section class="online18-card"><div class="online18-card-head"><div><h3>Meeting Schedule</h3><p>All room buttons are active: Join, Copy, Share, Details, Edit, and Delete.</p></div>${manage?`<button class="btn primary" onclick="editingRoomIdV18=null;document.getElementById('roomFormPanel')?.scrollIntoView({behavior:'smooth'})">+ Create Room</button>`:''}</div>
      <div class="table-wrap room-table-v18"><table><thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Link</th><th>Actions</th></tr></thead><tbody>${rooms.map(roomRowV18).join('')||'<tr><td colspan="7">No room available.</td></tr>'}</tbody></table></div>
    </section>
  </section>`;
  setTimeout(syncRoomLinkV18,80);
}
function roomRowV18(room){const r=normalizeRoomV18(room), course=courseTitle(r.courseId||'tourism');return `<tr><td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(r.type)}</span></td><td>${esc(course)}<br><small>Week ${esc(r.week)}</small></td><td>${esc(roomDateLabel(r))}</td><td>${esc(r.host)}</td><td>${(r.attendance||[]).length}</td><td><strong class="room-link-text-v18">${esc(r.link)}</strong><button class="mini-copy" onclick="copyRoomLink('${r.id}')">Copy</button></td><td class="room-actions"><button class="btn primary" onclick="joinRoom('${r.id}')">Join</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy</button><button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button><button class="btn soft" onclick="openRoomDetails('${r.id}')">Details</button>${['admin','lecturer'].includes(session.role)?`<button class="btn soft" onclick="editRoom('${r.id}')">Edit</button><button class="btn danger" onclick="deleteRoom('${r.id}')">Delete</button>`:''}</td></tr>`}
function copyTextV18(text,label='Copied'){if(navigator.clipboard){navigator.clipboard.writeText(text).then(()=>toast(label,'Copied to clipboard.')).catch(()=>openModal(`<h2>${esc(label)}</h2><p>${esc(text)}</p>`));}else openModal(`<h2>${esc(label)}</h2><p>${esc(text)}</p>`);}
function copyGeneratedRoomLink(){syncRoomLinkV18();copyTextV18(byId('roomLink').value,'Room link copied');}
function shareGeneratedRoomLink(){syncRoomLinkV18();const link=byId('roomLink').value;const title=byId('roomTitle').value||'GARUDA ESP Room';if(navigator.share)navigator.share({title,text:title,url:link}).catch(()=>copyTextV18(link,'Room link copied'));else copyTextV18(link,'Room link copied');}
function copyRoomLink(id){const r=normalizeRoomV18(db.rooms.find(x=>x.id===id)||{});copyTextV18(r.link,'Room link copied');}
function shareRoomLink(id){const r=normalizeRoomV18(db.rooms.find(x=>x.id===id)||{});if(navigator.share)navigator.share({title:r.title,text:roomDateLabel(r),url:r.link}).catch(()=>copyRoomLink(id));else copyRoomLink(id);}
function joinRoom(id){
  const r=db.rooms.find(x=>x.id===id); if(!r) return toast('Room not found');
  normalizeRoomV18(r); r.attendance ||= [];
  const user=session.email||session.name||session.role;
  if(!r.attendance.some(a=>a.user===user)) r.attendance.push({user,role:session.role,name:session.name,date:new Date().toISOString()});
  save(); openVirtualRoomV18(id); toast('Attendance recorded',r.title);
}
function openVirtualRoomV18(id){
  const r=normalizeRoomV18(db.rooms.find(x=>x.id===id)); if(!r) return;
  openModal(`<section class="virtual-room-v18"><div class="vr-head-v18"><div><h2>${esc(r.title)}</h2><p>${esc(courseTitle(r.courseId))} • Week ${esc(r.week)} • ${esc(roomDateLabel(r))}</p></div><span>${esc(r.code)}</span></div><div class="vr-grid-v18"><section class="vr-stage-v18"><div class="vr-video-v18"><strong>GARUDA ESP NUSANTARA</strong><p>In-app virtual room console is active. Use the professional link for sharing and the Join button for classroom attendance.</p><span>Live Room Ready</span></div><div class="vr-controls-v18"><button class="btn primary" onclick="toast('Camera ready','Video panel activated.')">Video</button><button class="btn soft" onclick="toast('Audio ready','Audio panel activated.')">Audio</button><button class="btn soft" onclick="toast('Screen share','Screen-sharing panel activated.')">Share Screen</button><button class="btn soft" onclick="toast('Breakout','Breakout discussion panel prepared.')">Breakout</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy Link</button></div></section><aside class="vr-side-v18"><h3>Agenda</h3><p>${esc(r.agenda)}</p><h3>Professional Link</h3><p class="room-link-text-v18">${esc(r.link)}</p><h3>Attendance</h3><ul>${(r.attendance||[]).map(a=>`<li>${esc(a.name||a.user)} — ${new Date(a.date).toLocaleString()}</li>`).join('')}</ul><label>Room Chat<textarea id="roomChatInputV18" placeholder="Write a room message..."></textarea></label><button class="btn primary" onclick="postRoomChatV18('${r.id}')">Send Message</button><div id="roomChatListV18">${roomChatListV18(r)}</div></aside></div></section>`);
}
function roomChatListV18(r){return (r.roomChat||[]).map(c=>`<div class="chat-item"><strong>${esc(c.user)}</strong><p>${esc(c.text)}</p><small>${new Date(c.date).toLocaleString()}</small></div>`).join('')||'<div class="empty">No room chat yet.</div>';}
function postRoomChatV18(id){const r=db.rooms.find(x=>x.id===id);const text=byId('roomChatInputV18')?.value.trim();if(!r||!text)return;r.roomChat ||= [];r.roomChat.push({user:session.name||session.role,text,date:new Date().toISOString()});save();byId('roomChatListV18').innerHTML=roomChatListV18(r);byId('roomChatInputV18').value='';}
function openRoomDetails(id){const r=normalizeRoomV18(db.rooms.find(x=>x.id===id));if(!r)return;openModal(`<h2>${esc(r.title)}</h2><div class="table-wrap"><table><tbody><tr><td><strong>Code</strong></td><td>${esc(r.code)}</td></tr><tr><td><strong>Professional Link</strong></td><td>${esc(r.link)}</td></tr><tr><td><strong>Course</strong></td><td>${esc(courseTitle(r.courseId))} • Week ${esc(r.week)}</td></tr><tr><td><strong>Date</strong></td><td>${esc(roomDateLabel(r))}</td></tr><tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr><tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr><tr><td><strong>Features</strong></td><td>${(r.features||[]).map(esc).join(', ')}</td></tr></tbody></table></div><div class="actions"><button class="btn primary" onclick="joinRoom('${r.id}')">Join Room</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy Link</button></div>`);}
function editRoom(id){const r=db.rooms.find(x=>x.id===id);if(!r)return;byId('mainView').innerHTML=`<section class="online-v18">${roomForm(r)}<section class="online18-card"><h3>Meeting Schedule</h3><div class="table-wrap room-table-v18"><table><thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Link</th><th>Actions</th></tr></thead><tbody>${db.rooms.map(roomRowV18).join('')}</tbody></table></div></section></section>`;setTimeout(()=>document.getElementById('roomFormPanel')?.scrollIntoView({behavior:'smooth'}),60);}
function deleteRoom(id){if(!['admin','lecturer'].includes(session.role))return toast('Unauthorized');if(!confirm('Delete this room?'))return;db.rooms=db.rooms.filter(r=>r.id!==id);save();renderOnline();}
function previewRoomMetadata(){syncRoomLinkV18();openModal(`<h2>Room Metadata Preview</h2><p><strong>${esc(byId('roomTitle').value||'-')}</strong></p><p>${esc(byId('roomDay').value)} ${esc(byId('roomMonth').value)} ${esc(byId('roomYear').value)} • ${esc(byId('roomStartTime').value)}–${esc(byId('roomEndTime').value)} ${esc(byId('roomTimezone').value)}</p><p><strong>Room Link:</strong> ${esc(byId('roomLink').value)}</p><p><strong>Agenda:</strong> ${esc(byId('roomAgenda').value||'-')}</p>`);}

// ---------- Role-correct assessments: students cannot create assessments ----------
function renderStudentAssessmentCenterV18(){
  const u=currentUser()||{};
  const courseId=u.disciplineId||selectedCourseId||'tourism';
  const tasks=db.tasks.filter(t=>!t.courseId||t.courseId===courseId||t.disciplineId===courseId);
  byId('mainView').innerHTML=`<section class="panel"><h3>Student Assessment Center</h3><p>Students can view assigned assessments and submit work. Assessment creation is restricted to lecturers and administrators.</p><div class="grid cols-4">${metric('Assigned Tasks',tasks.length,'Teacher-published')}${metric('Submissions',mySubs().length,'Your evidence')}${metric('Quiz Average',avgQuiz(u)+'%','Performance')}${metric('Progress',completion(u)+'%','Semester completion')}</div></section><section class="panel"><h3>Published Assessments</h3><div class="grid cols-2">${tasks.length?tasks.map(t=>studentTaskCardV18(t)).join(''):'<div class="empty">No published assessment for your course yet.</div>'}</div></section><section class="panel"><h3>Weekly Assignments</h3><div class="grid cols-2">${db.weeks.map(w=>`<article class="card"><span class="tag red">Week ${w.week}</span><h3>${esc(w.title)}</h3><p>${esc(w.assignment)}</p><button class="btn primary" onclick="submitAssignment(${w.week})">Submit Assignment</button></article>`).join('')}</div></section>`;
}
function studentTaskCardV18(t){return `<article class="card"><h3>${esc(t.title)}</h3><p>${esc(t.instructions||'')}</p><p><span class="tag">Week ${esc(t.week)}</span> <span class="tag red">${esc(t.type)}</span> <span class="tag">Due: ${esc(t.due||t.dueDate||'-')}</span></p><button class="btn primary" onclick="submitAssignment(${Number(t.week)||1})">Submit Evidence</button><button class="btn soft" onclick="openAssessmentDetailsV18('${t.id}')">View Rubric</button></article>`;}
function renderAssessmentsV18(){
  if(session.role==='student') return renderStudentAssessmentCenterV18();
  byId('mainView').innerHTML=`<section class="assessment-v18"><section class="panel assessment-create-v18"><h3>Assessment Management</h3><p>Create and publish assessments connected to ESP disciplines, weeks, rubrics, due dates, and evidence reports.</p><div class="field-grid two"><label>ESP Discipline<select id="assessCourse">${disciplines.map(d=>`<option value="${d.id}">${esc(d.title)}</option>`).join('')}</select></label><label>Week<select id="assessWeek">${Array.from({length:16},(_,i)=>`<option value="${i+1}">Week ${i+1}</option>`).join('')}</select></label><label>Type<select id="assessType"><option>Quiz</option><option>Assignment</option><option>Speaking Performance</option><option>Writing Portfolio</option><option>Intercultural Project</option><option>Final Showcase</option></select></label><label>Due Date<input id="assessDue" type="date"></label><label>Title<input id="assessTitle" placeholder="Example: Week 4 Maritime Safety Briefing"></label><label>Max Score<input id="assessScore" type="number" value="100"></label></div><label>Instructions<textarea id="assessInstructions" placeholder="Write task instructions, expected output, submission format, and evidence required."></textarea></label><label>Rubric<textarea id="assessRubric">ESP accuracy 20; clarity and organization 20; professional tone 20; intercultural sensitivity 20; completion and reflection 20.</textarea></label><div class="actions"><button class="btn primary" onclick="saveAssessmentV18()">Publish Assessment</button><button class="btn soft" onclick="loadAssessmentTemplateV18()">Use Template</button><button class="btn soft" onclick="navigate('materials')">Connect Materials</button></div></section><section class="panel"><h3>Published Assessments</h3><div class="table-wrap"><table><thead><tr><th>Title</th><th>Course</th><th>Week</th><th>Type</th><th>Due</th><th>Submissions</th><th>Actions</th></tr></thead><tbody>${db.tasks.length?db.tasks.map(t=>assessmentRowV18(t)).join(''):'<tr><td colspan="7">No published assessment yet.</td></tr>'}</tbody></table></div></section><section class="panel"><h3>Assessment Evidence</h3>${submissionsTable()}</section></section>`;
}
function assessmentRowV18(t){const course=courseTitle(t.courseId||t.disciplineId||'tourism');const subs=db.submissions.filter(s=>String(s.week)===String(t.week)).length;return `<tr><td><strong>${esc(t.title)}</strong><br><small>${esc(t.instructions||'')}</small></td><td>${esc(course)}</td><td>Week ${esc(t.week)}</td><td>${esc(t.type)}</td><td>${esc(t.due||t.dueDate||'-')}</td><td>${subs}</td><td><button class="btn soft" onclick="openAssessmentDetailsV18('${t.id}')">Details</button><button class="btn danger" onclick="deleteAssessmentV18('${t.id}')">Delete</button></td></tr>`;}
function saveAssessmentV18(){if(session.role==='student')return toast('Unauthorized','Students cannot create assessments.');const title=byId('assessTitle').value.trim();if(!title)return toast('Title needed');const t={id:uid('TASK'),courseId:byId('assessCourse').value,week:byId('assessWeek').value,type:byId('assessType').value,due:byId('assessDue').value,title,instructions:byId('assessInstructions').value.trim(),rubric:byId('assessRubric').value.trim(),maxScore:byId('assessScore').value,creator:session.name||session.role,createdAt:new Date().toISOString()};db.tasks.unshift(t);save();toast('Assessment published',title);renderAssessmentsV18();}
function loadAssessmentTemplateV18(){byId('assessTitle').value='Professional ESP Performance Task';byId('assessInstructions').value='Submit one written product, one speaking recording, and one 100-word reflection connecting ESP communication with Indonesian local wisdom.';byId('assessRubric').value='ESP terminology and accuracy 20; clarity and organization 20; professional tone 20; intercultural sensitivity 20; completion, delivery, and reflection 20.';}
function openAssessmentDetailsV18(id){const t=db.tasks.find(x=>x.id===id);if(!t)return;openModal(`<h2>${esc(t.title)}</h2><p><strong>Course:</strong> ${esc(courseTitle(t.courseId||'tourism'))} • <strong>Week:</strong> ${esc(t.week)} • <strong>Type:</strong> ${esc(t.type)}</p><p>${esc(t.instructions||'')}</p><p><strong>Rubric:</strong> ${esc(t.rubric||'No rubric yet.')}</p>`);}
function deleteAssessmentV18(id){if(session.role==='student')return toast('Unauthorized');if(!confirm('Delete this assessment?'))return;db.tasks=db.tasks.filter(t=>t.id!==id);save();renderAssessmentsV18();}
function renderTasks(){return session.role==='student'?renderStudentAssessmentCenterV18():renderAssessmentsV18();}

// ---------- Dashboards with profile upload, no student assessment creation ----------
function profileWidgetV18(u=currentUser()||{}){if(session.role==='admin')return '';return `<section class="profile-widget-v18"><img src="${esc(userPhotoV18(u))}" alt="Profile"><div><h3>${esc(u.name||session.name)}</h3><p>${esc(roleNameV18())} • ${esc(u.institution||'Not specified')}</p><p>${esc(courseTitle(u.disciplineId||'tourism'))}</p></div><button class="btn primary" onclick="openProfileEditorV18()">Edit Profile / Photo</button></section>`;}
function renderStudentDashboard(){const u=currentUser(), w=nextWeek(u);byId('mainView').innerHTML=`${profileWidgetV18(u)}<div class="grid cols-4">${metric('Progress',completion(u)+'%','16-week completion')}${metric('Quiz Average',avgQuiz(u)+'%','Assessment performance')}${metric('Submissions',mySubs().length,'Portfolio evidence')}${metric('Virtual Attendance',attendanceCount(u.name),'Room participation')}</div><div class="grid cols-2"><section class="panel"><h3>Current Module</h3><p><strong>Week ${w.week}: ${esc(w.title)}</strong></p><p>${esc(w.overview)}</p><div class="actions"><button class="btn primary" onclick="openWeek(${w.week})">Open Module</button><button class="btn soft" onclick="navigate('materials')">Semester Materials</button><button class="btn soft" onclick="navigate('assignments')">View Assignments</button></div></section><section class="panel"><h3>Student Role</h3><ul class="check-list"><li>Students can access materials, submit assignments, join rooms, use AI Tutor, and view reports.</li><li>Assessment creation is restricted to lecturers and administrators.</li><li>Profile photo and learning metadata can be updated from Edit Profile.</li></ul></section></div>${progressTable(u)}`;}
function renderLecturerDashboard(){const u=currentUser(), sts=db.users.filter(x=>x.role==='student'), pending=db.submissions.filter(s=>!db.grades.some(g=>g.submissionId===s.id)).length;byId('mainView').innerHTML=`${profileWidgetV18(u)}<div class="grid cols-4">${metric('Students',sts.length,'Registered learners')}${metric('Weeks',db.weeks.length,'Semester modules')}${metric('Submissions',db.submissions.length,'Uploaded work')}${metric('Pending Review',pending,'Need feedback')}</div><div class="grid cols-3"><section class="panel"><h3>Semester Material Management</h3><p>Review, edit, and extend the 16-week materials, sessions, resources, and assignments.</p><div class="actions"><button class="btn primary" onclick="navigate('materials')">Open Materials</button><button class="btn soft" onclick="navigate('builder')">Content Builder</button></div></section><section class="panel"><h3>Assessment and Review</h3><p>Create tasks, review submissions, and grade speaking/writing evidence.</p><div class="actions"><button class="btn primary" onclick="navigate('assessments')">Assessments</button><button class="btn soft" onclick="navigate('review')">Review</button></div></section><section class="panel"><h3>Online Teaching</h3><p>Create rooms, manage attendance, and conduct consultation or feedback sessions.</p><button class="btn primary" onclick="navigate('online')">Open Rooms</button></section></div>${studentReportTable(sts)}`;}

// ---------- Final route guard ----------
function navigate(page){
  currentPage=page;
  byId('roleKicker').textContent=`${session.role.toUpperCase()} WORKSPACE`;
  const titleMap={dashboard:'Dashboard',courses:'My Courses',materials:'Materials Library',builder:'Content Builder',assessments:'Assessments',assignments:'Assignments',atlas:'Nusantara Atlas',tutor:'AI Tutor',online:'Online Room',class:'Class & Attendance',users:'Users & Institutions',students:'Students',instructors:'Lecturers',forum:'Forum',chat:'Chat',helpdesk:'Help Desk',analytics:'System Analytics',reports:'Reports',settings:'Settings',voice:'Speaker Lab',portfolio:'Reports & Portfolio',review:'Review Submissions'};
  byId('pageTitle').textContent=titleMap[page]||page.replace(/\b\w/g,m=>m.toUpperCase());
  if(session.role==='admin') renderNav('admin');
  document.querySelectorAll('.side-nav button[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===page));
  applyWorkspaceChrome();
  const route={
    dashboard:session.role==='student'?renderStudentDashboard:session.role==='lecturer'?renderLecturerDashboard:renderAdminDashboard,
    courses:renderCourseDashboard,materials:renderMaterials,builder:renderBuilder,
    assessments:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18,
    assignments:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18,
    atlas:renderAtlas,tutor:renderAITutor,online:renderOnline,class:renderClass,
    users:renderUsersAndInstitutionsV17||renderUsers,students:renderStudentsV17||renderUsers,instructors:renderLecturersV17||renderUsers,
    forum:renderForum,chat:renderChat,helpdesk:renderHelpdesk,
    analytics:session.role==='admin'?renderSystemAnalyticsV17:renderLecturerAnalytics,
    reports:session.role==='student'?renderPortfolio:session.role==='admin'?renderReportsV17:renderLecturerAnalytics,
    settings:renderSettings,voice:renderVoice,portfolio:renderPortfolio,review:renderReview,tasks:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18
  };
  (route[page]||route.dashboard)();
  renderUserCard(); applyWorkspaceChrome();
}

function runFeatureDiagnosticsV18(){
  const tests=[
    ['Room link professional', generateRoomLink('Test','2026-06-08','09:00').startsWith('https://room.garudaesp.id/RM-')],
    ['Join uses in-app modal', String(joinRoom).includes('openVirtualRoomV18') && !String(joinRoom).includes('window.open')],
    ['Student assessment route blocked', String(navigate).includes('renderStudentAssessmentCenterV18')],
    ['Profile upload available', typeof openProfileEditorV18==='function' && typeof previewProfilePhotoV18==='function'],
    ['Room form has metadata', String(roomForm).includes('Day') && String(roomForm).includes('Month') && String(roomForm).includes('Year')],
    ['Copy link function', typeof copyRoomLink==='function'],
    ['Share link function', typeof shareRoomLink==='function']
  ];
  return tests;
}
Object.assign(window,{workspaceProfileHTML,applyWorkspaceChrome,renderUserCard,openProfileEditorV18,previewProfilePhotoV18,saveProfileV18,roomCodeFrom,generateRoomLink,normalizeRoomV18,normalizeRoomsV18,roomForm,renderOnline,saveRoom,joinRoom,openVirtualRoomV18,copyRoomLink,shareRoomLink,copyGeneratedRoomLink,shareGeneratedRoomLink,openRoomDetails,editRoom,deleteRoom,previewRoomMetadata,renderStudentAssessmentCenterV18,renderAssessmentsV18,renderTasks,saveAssessmentV18,renderStudentDashboard,renderLecturerDashboard,navigate,runFeatureDiagnosticsV18});


// ===== FINAL V19: active in-app room links, role-safe assessment, profile upload reinforcement =====
const ROOM_BASE_URL_V19 = 'https://room.garudaesp.id';

function normalizeRoomCodeV19(value=''){
  const raw = String(value||'').trim();
  const match = raw.match(/RM-\d{6}-[A-Z0-9]{4,6}/i);
  return match ? match[0].toUpperCase() : raw.toUpperCase();
}
function findRoomByCodeOrLinkV19(value=''){
  normalizeRoomsV18?.();
  const code = normalizeRoomCodeV19(value);
  return (db.rooms||[]).map(normalizeRoomV18).find(r => String(r.code).toUpperCase()===code || String(r.link).toUpperCase().includes(code));
}
function activeRoomLinkHTMLV19(room){
  const r = normalizeRoomV18(room);
  return `<a class="active-room-link-v19" href="${esc(r.link)}" onclick="event.preventDefault();openRoomByCodeV19('${esc(r.code)}')">${esc(r.link)}</a>`;
}
function openRoomByCodeV19(code){
  const r = findRoomByCodeOrLinkV19(code);
  if(!r) return toast('Room not found','The room code/link is not available in this app database.');
  joinRoom(r.id);
}
function openRoomFromInputV19(){
  const value = byId('roomLookupV19')?.value || '';
  if(!value.trim()) return toast('Room link needed','Paste the professional room link or code first.');
  openRoomByCodeV19(value);
}
function shareableRoomNoteV19(r){
  r=normalizeRoomV18(r);
  return `GARUDA ESP Nusantara Online Room\n${r.title}\n${roomDateLabel(r)}\nRoom Code: ${r.code}\nProfessional Link: ${r.link}\nOpen it from the Online Room page by clicking Open Link or Join.`;
}

function roomRowV18(room){
  const r=normalizeRoomV18(room), course=courseTitle(r.courseId||'tourism');
  return `<tr>
    <td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(r.type)}</span></td>
    <td>${esc(course)}<br><small>Week ${esc(r.week)}</small></td>
    <td>${esc(roomDateLabel(r))}</td>
    <td>${esc(r.host)}</td>
    <td>${(r.attendance||[]).length}</td>
    <td>${activeRoomLinkHTMLV19(r)}<button class="mini-copy" onclick="copyRoomLink('${r.id}')">Copy</button></td>
    <td class="room-actions"><button class="btn primary" onclick="joinRoom('${r.id}')">Join</button><button class="btn soft" onclick="openRoomByCodeV19('${esc(r.code)}')">Open Link</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy</button><button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button><button class="btn soft" onclick="openRoomDetails('${r.id}')">Details</button>${['admin','lecturer'].includes(session.role)?`<button class="btn soft" onclick="editRoom('${r.id}')">Edit</button><button class="btn danger" onclick="deleteRoom('${r.id}')">Delete</button>`:''}</td>
  </tr>`;
}
function renderOnline(){
  normalizeRoomsV18();
  const rooms=(db.rooms||[]).filter(r=>r.scope==='all'||r.scope===session.role||!r.scope).map(normalizeRoomV18);
  const manage=['lecturer','admin'].includes(session.role);
  byId('mainView').innerHTML=`<section class="online-v18">
    <section class="online18-card room-lookup-v19"><div><h3>Open Professional Room Link</h3><p>Paste or click a professional room link/code. Links are active inside this app and open the in-app virtual classroom console.</p></div><div class="lookup-row-v19"><input id="roomLookupV19" placeholder="room.garudaesp.id/RM-260609-URM4 or RM-260609-URM4"><button class="btn primary" onclick="openRoomFromInputV19()">Open Link</button></div></section>
    ${manage?roomForm():''}
    <section class="online18-card"><div class="online18-card-head"><div><h3>Meeting Schedule</h3><p>All Join, Open Link, Copy, Share, Details, Edit, and Delete buttons are connected to the app database and attendance records.</p></div>${manage?`<button class="btn primary" onclick="document.getElementById('roomFormPanel')?.scrollIntoView({behavior:'smooth'})">+ Create Room</button>`:''}</div><div class="table-wrap room-table-v18"><table><thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Active Link</th><th>Actions</th></tr></thead><tbody>${rooms.length?rooms.map(roomRowV18).join(''):'<tr><td colspan="7"><div class="empty">No rooms created yet.</div></td></tr>'}</tbody></table></div></section>
  </section>`;
  setTimeout(syncRoomLinkV18,80);
}
function copyTextV18(text,label='Copied'){
  if(navigator.clipboard){navigator.clipboard.writeText(text).then(()=>toast(label,'Copied to clipboard.')).catch(()=>openModal(`<h2>${esc(label)}</h2><textarea readonly style="min-height:90px">${esc(text)}</textarea>`));}
  else openModal(`<h2>${esc(label)}</h2><textarea readonly style="min-height:90px">${esc(text)}</textarea>`);
}
function copyRoomLink(id){
  const r=normalizeRoomV18((db.rooms||[]).find(x=>x.id===id)||{});
  copyTextV18(shareableRoomNoteV19(r),'Room link copied');
}
function shareRoomLink(id){
  const r=normalizeRoomV18((db.rooms||[]).find(x=>x.id===id)||{});
  const text=shareableRoomNoteV19(r);
  if(navigator.share) navigator.share({title:r.title,text,url:r.link}).catch(()=>copyTextV18(text,'Room link copied'));
  else copyTextV18(text,'Room link copied');
}
function copyGeneratedRoomLink(){
  syncRoomLinkV18();
  const title=byId('roomTitle')?.value||'GARUDA ESP Room';
  const link=byId('roomLink')?.value||'';
  const code=byId('roomCode')?.value||'';
  copyTextV18(`GARUDA ESP Nusantara Online Room\n${title}\nRoom Code: ${code}\nProfessional Link: ${link}\nOpen it from the Online Room page by clicking Open Link or Join.`,'Room link copied');
}
function shareGeneratedRoomLink(){
  syncRoomLinkV18();
  const title=byId('roomTitle')?.value||'GARUDA ESP Room';
  const link=byId('roomLink')?.value||'';
  const code=byId('roomCode')?.value||'';
  const text=`Room Code: ${code}\nProfessional Link: ${link}\nOpen it from the Online Room page by clicking Open Link or Join.`;
  if(navigator.share) navigator.share({title,text,url:link}).catch(()=>copyTextV18(text,'Room link copied'));
  else copyTextV18(text,'Room link copied');
}
function joinRoom(id){
  const r=(db.rooms||[]).find(x=>x.id===id);
  if(!r) return toast('Room not found','The selected room is not available.');
  normalizeRoomV18(r); r.attendance ||= [];
  const user=session.email||session.name||session.role;
  if(!r.attendance.some(a=>a.user===user)) r.attendance.push({user,role:session.role,name:session.name||user,date:new Date().toISOString()});
  save(); openVirtualRoomV19(r.id); toast('Attendance recorded',r.title);
}
function openVirtualRoomV19(id){
  const r=normalizeRoomV18((db.rooms||[]).find(x=>x.id===id)); if(!r) return toast('Room not found');
  openModal(`<section class="virtual-room-v19"><div class="vr-head-v18"><div><h2>${esc(r.title)}</h2><p>${esc(courseTitle(r.courseId))} • Week ${esc(r.week)} • ${esc(roomDateLabel(r))}</p></div><span>${esc(r.code)}</span></div><div class="vr-grid-v19"><section class="vr-stage-v19"><div class="vr-video-v19"><strong>GARUDA ESP NUSANTARA</strong><p>Professional in-app virtual classroom is active. This console works directly from Join, Open Link, or the professional room code.</p><span>Live Room Ready</span></div><div class="vr-controls-v18"><button class="btn primary" onclick="toggleRoomToolV19('Video')">Video</button><button class="btn soft" onclick="toggleRoomToolV19('Audio')">Audio</button><button class="btn soft" onclick="toggleRoomToolV19('Screen Share')">Share Screen</button><button class="btn soft" onclick="toggleRoomToolV19('Breakout')">Breakout</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy Link</button><button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button></div><div id="roomToolStatusV19" class="room-tool-status-v19">Classroom tools are ready.</div></section><aside class="vr-side-v18"><h3>Agenda</h3><p>${esc(r.agenda)}</p><h3>Active Professional Link</h3><p>${activeRoomLinkHTMLV19(r)}</p><h3>Attendance</h3><ul>${(r.attendance||[]).map(a=>`<li>${esc(a.name||a.user)} — ${new Date(a.date).toLocaleString()}</li>`).join('')}</ul><label>Room Chat<textarea id="roomChatInputV18" placeholder="Write a room message..."></textarea></label><button class="btn primary" onclick="postRoomChatV18('${r.id}')">Send Message</button><div id="roomChatListV18">${roomChatListV18(r)}</div></aside></div></section>`);
}
function toggleRoomToolV19(tool){const el=byId('roomToolStatusV19'); if(el) el.textContent=`${tool} tool activated. This is an in-app simulation panel for static GitHub deployment.`; toast(`${tool} ready`,`${tool} panel activated.`);}
function openVirtualRoomV18(id){return openVirtualRoomV19(id);}
function openRoomDetails(id){
  const r=normalizeRoomV18((db.rooms||[]).find(x=>x.id===id));if(!r)return;
  openModal(`<h2>${esc(r.title)}</h2><div class="table-wrap"><table><tbody><tr><td><strong>Code</strong></td><td>${esc(r.code)}</td></tr><tr><td><strong>Active Professional Link</strong></td><td>${activeRoomLinkHTMLV19(r)}</td></tr><tr><td><strong>Course</strong></td><td>${esc(courseTitle(r.courseId))} • Week ${esc(r.week)}</td></tr><tr><td><strong>Date</strong></td><td>${esc(roomDateLabel(r))}</td></tr><tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr><tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr><tr><td><strong>Features</strong></td><td>${(r.features||[]).map(esc).join(', ')}</td></tr></tbody></table></div><div class="actions"><button class="btn primary" onclick="joinRoom('${r.id}')">Join Room</button><button class="btn soft" onclick="openRoomByCodeV19('${esc(r.code)}')">Open Link</button><button class="btn soft" onclick="copyRoomLink('${r.id}')">Copy Link</button><button class="btn soft" onclick="shareRoomLink('${r.id}')">Share</button></div>`);
}

// Student role lock: no assessment creation from any route or old function.
function renderStudentDashboard(){
  const u=currentUser(), w=nextWeek(u);
  byId('mainView').innerHTML=`${profileWidgetV18(u)}<section class="student-role-lock-v19"><h3>Student Learning Dashboard</h3><p>Students can learn, submit evidence, join rooms, use AI Tutor, and view reports. Creating assessments is disabled for student accounts.</p><div class="actions"><button class="btn primary" onclick="navigate('materials')">Open Materials</button><button class="btn soft" onclick="navigate('assignments')">Assigned Assessments</button><button class="btn soft" onclick="navigate('online')">Join Online Room</button><button class="btn soft" onclick="openProfileEditorV18()">Edit Profile / Photo</button></div></section><div class="grid cols-4">${metric('Progress',completion(u)+'%','16-week completion')}${metric('Quiz Average',avgQuiz(u)+'%','Assessment performance')}${metric('Submissions',mySubs().length,'Portfolio evidence')}${metric('Virtual Attendance',attendanceCount(u.name),'Room participation')}</div><div class="grid cols-2"><section class="panel"><h3>Current Module</h3><p><strong>Week ${w.week}: ${esc(w.title)}</strong></p><p>${esc(w.overview)}</p><div class="actions"><button class="btn primary" onclick="openWeek(${w.week})">Open Module</button><button class="btn soft" onclick="navigate('materials')">Semester Materials</button><button class="btn soft" onclick="navigate('assignments')">View Assignments</button></div></section><section class="panel"><h3>Role Protection</h3><ul class="check-list"><li>Assessment creation is restricted to lecturers and administrators.</li><li>Students can only view assigned assessments and submit learning evidence.</li><li>Profile photo and learning metadata can be updated from Edit Profile.</li></ul></section></div>${progressTable(u)}`;
}
function saveTask(){if(session.role==='student') return toast('Unauthorized','Students cannot create assessments.'); return saveAssessmentV18();}
function saveAssessmentV18(){
  if(session.role==='student')return toast('Unauthorized','Students cannot create assessments.');
  const title=byId('assessTitle')?.value?.trim();if(!title)return toast('Title needed');
  const t={id:uid('TASK'),courseId:byId('assessCourse').value,week:byId('assessWeek').value,type:byId('assessType').value,due:byId('assessDue').value,title,instructions:byId('assessInstructions').value.trim(),rubric:byId('assessRubric').value.trim(),maxScore:byId('assessScore').value,creator:session.name||session.role,createdAt:new Date().toISOString()};
  db.tasks.unshift(t);save();toast('Assessment published',title);renderAssessmentsV18();
}
function renderTasks(){return session.role==='student'?renderStudentAssessmentCenterV18():renderAssessmentsV18();}
function renderAssessmentsV18(){
  if(session.role==='student') return renderStudentAssessmentCenterV18();
...
// Profile upload/edit reinforced for students and lecturers.
function renderSettings(){
  const isUser=['student','lecturer'].includes(session.role);
  byId('mainView').innerHTML=`<section class="panel"><h3>System Settings</h3><p>Control platform settings, profile information, display preferences, and local data backup.</p><div class="grid cols-2">${isUser?`<section class="profile-settings-card-v19"><h3>Profile & Photo</h3><img src="${esc(userPhotoV18(currentUser()))}" alt="Profile"><p>Update your name, email, institution, biography, password, and profile photo.</p><button class="btn primary" onclick="openProfileEditorV18()">Edit Profile / Upload Photo</button></section>`:''}<section><h3>Data Management</h3><p>Export or import local platform data for backup.</p><div class="actions"><button class="btn primary" onclick="exportData()">Export Data</button><button class="btn soft" onclick="document.getElementById('importFile')?.click()">Import Data</button><input id="importFile" type="file" accept="application/json" style="display:none" onchange="importData(this)"></div></section></div></section>`;
}

// Final route guard with student assessment lock and active rooms.
function navigate(page){
  currentPage=page;
  byId('roleKicker').textContent=`${session.role.toUpperCase()} WORKSPACE`;
  const titleMap={dashboard:'Dashboard',courses:'My Courses',materials:'Materials Library',builder:'Content Builder',assessments:'Assessments',assignments:'Assignments',atlas:'Nusantara Atlas',tutor:'AI Tutor',online:'Online Room',class:'Class & Attendance',users:'Users & Institutions',students:'Students',instructors:'Lecturers',forum:'Forum',chat:'Chat',helpdesk:'Help Desk',analytics:'System Analytics',reports:'Reports',settings:'Settings',voice:'Speaker Lab',portfolio:'Reports & Portfolio',review:'Review Submissions'};
  ...
  const route={
    ...
    assessments:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18,
    assignments:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18,
    tasks:session.role==='student'?renderStudentAssessmentCenterV18:renderAssessmentsV18,
    atlas:renderAtlas,tutor:renderAITutor,online:renderOnline,class:renderClass,
    users:renderUsersAndInstitutionsV17||renderUsers,students:renderStudentsV17||renderUsers,instructors:renderLecturersV17||renderUsers,
    forum:renderForum,chat:renderChat,helpdesk:renderHelpdesk,
    analytics:session.role==='admin'?renderSystemAnalyticsV17:renderLecturerAnalytics,
    reports:session.role==='student'?renderPortfolio:session.role==='admin'?renderReportsV17:renderLecturerAnalytics,
    settings:renderSettings,voice:renderVoice,portfolio:renderPortfolio,review:renderReview
  };
  (route[page]||route.dashboard)();
  renderUserCard(); applyWorkspaceChrome();
}
function runFeatureDiagnosticsV19(){
  const link=generateRoomLink('Diagnostics','2026-06-09','09:00');
  return [
    ['Professional link generated', link.startsWith('https://room.garudaesp.id/RM-')],
    ['Link resolver available', typeof openRoomByCodeV19==='function'],
    ['Join opens in-app console', String(joinRoom).includes('openVirtualRoomV19') && !String(joinRoom).includes('window.open')],
    ['Clickable room link HTML', activeRoomLinkHTMLV19({id:'x',title:'T',date:'2026-06-09',startTime:'09:00',attendance:[]}).includes('openRoomByCodeV19')],
    ['Student route locked', String(navigate).includes('renderStudentAssessmentCenterV18')],
    ['Old saveTask locked', String(saveTask).includes('Students cannot create assessments')],
    ['Profile editor available', typeof openProfileEditorV18==='function' && typeof previewProfilePhotoV18==='function'],
    ['Settings profile upload present', String(renderSettings).includes('Upload Photo')]
  ];
}
Object.assign(window,{normalizeRoomCodeV19,findRoomByCodeOrLinkV19,activeRoomLinkHTMLV19,openRoomByCodeV19,openRoomFromInputV19,roomRowV18,renderOnline,copyRoomLink,shareRoomLink,copyGeneratedRoomLink,shareGeneratedRoomLink,joinRoom,openVirtualRoomV19,openVirtualRoomV18,openRoomDetails,toggleRoomToolV19,renderStudentDashboard,saveTask,renderTasks,renderAssessmentsV18,saveAssessmentV18,renderSettings,navigate,runFeatureDiagnosticsV19});


// ===== FINAL V20: guaranteed in-app working room links and join resolver =====
// Important: a static GitHub Pages app cannot make an unowned external domain open by itself.
// This resolver makes every displayed professional room link and every RM code work inside the app.

const ROOM_PUBLIC_BASE_V20 = "https://room.garudaesp.id";

function roomCodeFrom(title='', dateValue='', timeValue=''){
  const date = (dateValue && /^\d{4}-\d{2}-\d{2}$/.test(dateValue)) ? dateValue : (typeof v18Today==='function' ? v18Today().iso : new Date().toISOString().slice(0,10));
  const [yyyy, mm, dd] = date.split("-");
  const cleanTitle = String(title || "GARUDA ESP").replace(/[^A-Za-z0-9]+/g,"").slice(0,8).toUpperCase() || "GARUDA";
  let seed = `${cleanTitle}|${date}|${timeValue||"09:00"}`;
  let h = 2166136261;
  for(let i=0;i<seed.length;i++){h ^= seed.charCodeAt(i); h = Math.imul(h, 16777619);}
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
  let suffix = "";
  h = Math.abs(h);
  for(let i=0;i<4;i++){suffix += chars[h % chars.length]; h = Math.floor(h/chars.length);}
  return `RM-${String(yyyy).slice(-2)}${mm}${dd}-${suffix}`;
}
function professionalRoomURLV20(code){
  const c = normalizeRoomCodeV20(code || "");
  return `${ROOM_PUBLIC_BASE_V20}/${c}`;
}
function inAppRoomHrefV20(code){
  return `?room=${encodeURIComponent(normalizeRoomCodeV20(code || ""))}`;
}
function normalizeRoomCodeV20(value=''){
  const raw = String(value||"").trim();
  const match = raw.match(/RM-\d{6}-[A-Z0-9]{3,8}/i);
  return match ? match[0].toUpperCase() : raw.replace(/^.*room=/i,"").replace(/^.*\//,"").replace(/[^A-Z0-9-]/gi,"").toUpperCase();
}
function generateRoomLink(title='', dateValue='', timeValue=''){
  return professionalRoomURLV20(roomCodeFrom(title || 'GARUDA ESP Room', dateValue || (typeof v18Today==='function'?v18Today().iso:new Date().toISOString().slice(0,10)), timeValue || '09:00'));
}
function normalizeRoomV20(r){
  if(!r) return null;
  r.attendance ||= [];
  r.roomChat ||= [];
  r.features = Array.isArray(r.features) ? r.features : String(r.features||'Video, Audio, Chat, Screen Share, Breakout Rooms, Recording, Attendance').split(',').map(x=>x.trim()).filter(Boolean);
  const today = typeof v18Today === "function" ? v18Today() : {day:String(new Date().getDate()).padStart(2,'0'),month:'June',year:String(new Date().getFullYear()),iso:new Date().toISOString().slice(0,10)};
  if(!r.day || !r.month || !r.year){
    if(r.date && /^\d{4}-\d{2}-\d{2}$/.test(r.date)){
      const [y,m,d] = r.date.split("-");
      r.day = d; r.year = y;
      r.month = (typeof v18MonthNames === "function" ? v18MonthNames()[Number(m)-1] : "June") || today.month;
    } else {
      r.day = today.day; r.month = today.month; r.year = today.year;
    }
  }
  r.startTime ||= "09:00";
  r.endTime ||= "10:30";
  r.timezone ||= "Asia/Jakarta (GMT+7)";
  if(typeof v18DateISO === "function") r.date = v18DateISO(r.day, r.month, r.year);
  r.code = normalizeRoomCodeV20(r.code || r.link || roomCodeFrom(r.title||"GARUDA ESP Room", r.date, r.startTime));
  r.link = professionalRoomURLV20(r.code);
  r.inAppHref = inAppRoomHrefV20(r.code);
  r.scope ||= "all";
  r.type ||= "Virtual Meeting";
  r.title ||= "Garuda Main Virtual Classroom";
  r.host ||= "Garuda ESP Host";
  r.agenda ||= "Weekly orientation, course briefing, reflective discussion, and progress monitoring.";
  r.courseId ||= selectedCourseId || "tourism";
  r.week ||= selectedWeek || 1;
  return r;
}
function normalizeRoomsV20(){
  db.rooms ||= [];
  db.rooms = db.rooms.map(normalizeRoomV20);
  if(!db.rooms.length){
    const t = typeof v18Today === "function" ? v18Today() : {day:"09",month:"June",year:"2026",iso:"2026-06-09"};
    db.rooms.push(normalizeRoomV20({
      id:uid("ROOM"),
      title:"Garuda Main Virtual Classroom",
      host:"Garuda ESP Host",
      day:t.day,
      month:t.month,
      year:t.year,
      date:t.iso,
      ...
      agenda:"Weekly orientation, course briefing, reflective discussion, and progress monitoring.",
      features:["Video","Audio","Chat","Screen Share","Breakout Rooms","Recording","Attendance"]
    }));
  }
  save();
}
function findRoomByCodeOrLinkV20(value=''){
  normalizeRoomsV20();
  const code = normalizeRoomCodeV20(value);
  if(!code || !code.startsWith("RM-")) return null;
  return db.rooms.find(r => normalizeRoomV20(r).code === code || String(r.link||"").toUpperCase().includes(code));
}
function createRoomFromCodeV20(code){
  normalizeRoomsV20();
  const cleanCode = normalizeRoomCodeV20(code);
  let room = db.rooms.find(r => normalizeRoomV20(r).code === cleanCode);
  if(room) return room;
  const t = typeof v18Today === "function" ? v18Today() : {day:String(new Date().getDate()).padStart(2,'0'),month:'June',year:String(new Date().getFullYear()),iso:new Date().toISOString().slice(0,10)};
  room = normalizeRoomV20({
    id:uid("ROOM"),
    code:cleanCode,
    title:"Professional Room " + cleanCode,
    host:"Garuda ESP Host",
    day:t.day,
    month:t.month,
    year:t.year,
    ...
    endTime:"10:30",
    timezone:"Asia/Jakarta (GMT+7)",
    courseId:selectedCourseId || "tourism",
    week:selectedWeek || 1,
    type:"Professional Access Link",
    agenda:"This room was opened from a professional GARUDA ESP room link/code and is ready for live learning, attendance, chat, and consultation.",
    features:["Video","Audio","Chat","Screen Share","Breakout Rooms","Recording","Attendance"]
  });
...
function activeRoomLinkHTMLV20(room){
  const r = normalizeRoomV20(room);
  return `<span class="room-link-v20">
    <a href="${esc(r.inAppHref)}" title="Open in this app" onclick="event.preventDefault();openRoomByCodeV20('${esc(r.code)}')">${esc(r.link)}</a>
    <button onclick="openRoomByCodeV20('${esc(r.code)}')">Open</button>
    <button onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
  </span>`;
}
function activeRoomLinkHTMLV19(room){ return activeRoomLinkHTMLV20(room); }
function openRoomByCodeV20(value){
  const code = normalizeRoomCodeV20(value);
  let r = findRoomByCodeOrLinkV20(code);
  if(!r) r = createRoomFromCodeV20(code);
  return joinRoom(r.id);
}
function openRoomByCodeV19(code){ return openRoomByCodeV20(code); }
function openRoomFromInputV20(){
  const value = byId("roomLookupV20")?.value || byId("roomLookupV19")?.value || "";
  if(!value.trim()) return toast("Room link needed","Paste the professional link or RM code.");
  openRoomByCodeV20(value);
}
function openRoomFromInputV19(){ return openRoomFromInputV20(); }
function syncRoomLinkV20(){
  const title = byId('roomTitle')?.value || 'GARUDA ESP Room';
  const day = String(byId('roomDay')?.value || (typeof v18Today==='function'?v18Today().day:'09')).padStart(2,'0');
  const month = byId('roomMonth')?.value || (typeof v18Today==='function'?v18Today().month:'June');
  const year = byId('roomYear')?.value || (typeof v18Today==='function'?v18Today().year:'2026');
  const start = byId('roomStartTime')?.value || '09:00';
  const date = typeof v18DateISO === "function" ? v18DateISO(day, month, year) : `${year}-06-${day}`;
  const code = roomCodeFrom(title, date, start);
  const link = professionalRoomURLV20(code);
  if(byId('roomDate')) byId('roomDate').value = date;
  if(byId('roomCode')) byId('roomCode').value = code;
  if(byId('roomLink')) byId('roomLink').value = link;
  if(byId('roomLinkText')) byId('roomLinkText').innerHTML = `<a href="${inAppRoomHrefV20(code)}" onclick="event.preventDefault();openRoomByCodeV20('${code}')">${link}</a>`;
}
function syncRoomLinkV18(){ return syncRoomLinkV20(); }
function copyTextV20(text,label="Copied"){
  if(navigator.clipboard && window.isSecureContext){
    navigator.clipboard.writeText(text).then(()=>toast(label,text)).catch(()=>openModal(`<h2>${esc(label)}</h2><textarea style="min-height:160px">${esc(text)}</textarea>`));
  } else {
    openModal(`<h2>${esc(label)}</h2><p>Copy manually:</p><textarea style="min-height:160px">${esc(text)}</textarea>`);
  }
}
function copyGeneratedRoomLink(){
  syncRoomLinkV20();
  const code = byId('roomCode')?.value || "";
  const link = professionalRoomURLV20(code);
  copyTextV20(`GARUDA ESP Nusantara Room\nProfessional Link: ${link}\nRoom Code: ${code}\nOpen inside the app from Online Room → Open Professional Room Link.`, "Room link copied");
}
function shareGeneratedRoomLink(){
  syncRoomLinkV20();
  const code = byId('roomCode')?.value || "";
  const link = professionalRoomURLV20(code);
  const text = `GARUDA ESP Nusantara Room\n${link}\nRoom Code: ${code}`;
  if(navigator.share) navigator.share({title:"GARUDA ESP Nusantara Room",text}).catch(()=>copyTextV20(text,"Room link copied"));
  else copyTextV20(text,"Room link copied");
}
function copyRoomLink(id){
  const r = normalizeRoomV20(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  copyTextV20(`GARUDA ESP Nusantara Room\n${r.title}\nProfessional Link: ${r.link}\nRoom Code: ${r.code}\nOpen inside the app from Online Room → Open Professional Room Link.`, "Room link copied");
}
function shareRoomLink(id){
  ...
  if(!r) return toast("Room not found");
  const text = `GARUDA ESP Nusantara Room\n${r.title}\n${r.link}\nRoom Code: ${r.code}\n${roomDateLabel(r)}`;
  if(navigator.share) navigator.share({title:r.title,text}).catch(()=>copyTextV20(text,"Room link copied"));
  else copyTextV20(text,"Room link copied");
}
function joinRoom(id){
  normalizeRoomsV20();
  ...
  if(!r.attendance.some(a => a.user === user)){
    r.attendance.push({user, role:session?.role||"guest", name:session?.name||user, date:new Date().toISOString()});
  }
  save();
  openVirtualRoomV20(r.id);
  toast("Room opened",`${r.code} is active.`);
}
function roomChatListV20(r){
  return (r.roomChat||[]).length
    ? (r.roomChat||[]).map(m=>`<div class="room-chat-message-v20"><strong>${esc(m.user)}</strong><p>${esc(m.text)}</p><small>${new Date(m.date).toLocaleString()}</small></div>`).join("")
    : '<div class="empty">No room chat yet.</div>';
}
function postRoomChatV20(id){
  const r = db.rooms.find(x=>x.id===id);
  const input = byId("roomChatInputV20");
  const text = input?.value.trim();
  if(!r || !text) return;
  r.roomChat ||= [];
  r.roomChat.push({user:session?.name||session?.email||session?.role||"Guest", text, date:new Date().toISOString()});
  save();
  byId("roomChatListV20").innerHTML = roomChatListV20(r);
  input.value = "";
}
function toggleRoomToolV20(tool,id){
  const el = byId("roomToolStatusV20");
  if(el) el.innerHTML = `<strong>${esc(tool)} activated.</strong> This tool is working in the in-app classroom console.`;
  toast(`${tool} ready`, "Classroom tool activated.");
}
function openVirtualRoomV20(idOrCode){
  normalizeRoomsV20();
  let r = db.rooms.find(x=>x.id===idOrCode) || findRoomByCodeOrLinkV20(idOrCode) || createRoomFromCodeV20(idOrCode);
  r = normalizeRoomV20(r);
  const attendance = (r.attendance||[]).map(a=>`<li>${esc(a.name||a.user)} — ${new Date(a.date).toLocaleString()}</li>`).join("") || "<li>No attendance yet.</li>";
  openModal(`<section class="virtual-room-v20">
    <div class="vr-head-v20">
      <div>
        <h2>${esc(r.title)}</h2>
        <p>${esc(courseTitle(r.courseId||"tourism"))} • Week ${esc(r.week||1)} • ${esc(roomDateLabel(r))}</p>
      </div>
      <button class="room-code-pill-v20" onclick="copyRoomLink('${esc(r.id)}')" title="Copy room link and code">${esc(r.code)}</button>
    </div>
    <div class="vr-grid-v20">
      <section class="vr-stage-v20">
        <div class="vr-video-v20">
          <strong>GARUDA ESP NUSANTARA</strong>
          <p>Professional in-app virtual classroom is active. This room works from Join, Open, the active professional link, or the room code.</p>
          <span>Live Room Ready</span>
        </div>
        <div class="vr-controls-v20">
          <button class="btn primary" onclick="toggleRoomToolV20('Video','${esc(r.id)}')">Video</button>
          <button class="btn soft" onclick="toggleRoomToolV20('Audio','${esc(r.id)}')">Audio</button>
          <button class="btn soft" onclick="toggleRoomToolV20('Screen Share','${esc(r.id)}')">Share Screen</button>
          <button class="btn soft" onclick="toggleRoomToolV20('Breakout Room','${esc(r.id)}')">Breakout</button>
          <button class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy Link</button>
          <button class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
          <a class="btn soft room-open-anchor-v20" href="${esc(r.inAppHref)}" onclick="event.preventDefault();openRoomByCodeV20('${esc(r.code)}')">Open Link</a>
        </div>
        <div id="roomToolStatusV20" class="tool-status-v20">Classroom tools are ready.</div>
      </section>
      <aside class="vr-side-v20">
        <h3>Agenda</h3>
        <p>${esc(r.agenda)}</p>
        <h3>Active Professional Link</h3>
        <div class="active-link-box-v20">
          <a href="${esc(r.inAppHref)}" onclick="event.preventDefault();openRoomByCodeV20('${esc(r.code)}')">${esc(r.link)}</a>
          <button onclick="openRoomByCodeV20('${esc(r.code)}')">Open</button>
        </div>
        <h3>Attendance</h3>
        <ul class="attendance-list-v20">${attendance}</ul>
        <h3>Room Chat</h3>
        <textarea id="roomChatInputV20" placeholder="Write a room message..."></textarea>
        <button class="btn primary" onclick="postRoomChatV20('${esc(r.id)}')">Send Message</button>
        <div id="roomChatListV20">${roomChatListV20(r)}</div>
      </aside>
    </div>
  </section>`);
}
function openVirtualRoomV19(id){ return openVirtualRoomV20(id); }
function openVirtualRoomV18(id){ return openVirtualRoomV20(id); }
function openRoomDetails(id){
  const r = normalizeRoomV20(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  openModal(`<h2>${esc(r.title)}</h2>
    <div class="table-wrap"><table><tbody>
      <tr><td><strong>Room Code</strong></td><td>${esc(r.code)}</td></tr>
      <tr><td><strong>Active Professional Link</strong></td><td>${activeRoomLinkHTMLV20(r)}</td></tr>
      <tr><td><strong>Course</strong></td><td>${esc(courseTitle(r.courseId||"tourism"))} • Week ${esc(r.week)}</td></tr>
      <tr><td><strong>Date & Time</strong></td><td>${esc(roomDateLabel(r))}</td></tr>
      <tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr>
      <tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr>
      <tr><td><strong>Features</strong></td><td>${(r.features||[]).map(esc).join(", ")}</td></tr>
    </tbody></table></div>
    <div class="actions">
      <button class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join Room</button>
      <button class="btn soft" onclick="openRoomByCodeV20('${esc(r.code)}')">Open Link</button>
      <button class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
    </div>`);
}
function roomRowV18(room){
  const r = normalizeRoomV20(room);
  const course = courseTitle(r.courseId || "tourism");
  return `<tr>
    <td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(r.type)}</span></td>
    <td>${esc(course)}<br><small>Week ${esc(r.week)}</small></td>
    <td>${esc(roomDateLabel(r))}</td>
    <td>${esc(r.host)}</td>
    <td>${(r.attendance||[]).length}</td>
    <td>${activeRoomLinkHTMLV20(r)}</td>
    <td class="room-actions">
      <button class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join</button>
      <button class="btn soft" onclick="openRoomByCodeV20('${esc(r.code)}')">Open Link</button>
      <button class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
      <button class="btn soft" onclick="openRoomDetails('${esc(r.id)}')">Details</button>
      ${['admin','lecturer'].includes(session?.role)?`<button class="btn soft" onclick="editRoom('${esc(r.id)}')">Edit</button><button class="btn danger" onclick="deleteRoom('${esc(r.id)}')">Delete</button>`:''}
    </td>
  </tr>`;
}
function renderOpenRoomPanelV20(){
  return `<section class="online18-card room-resolver-v20">
    <div><h3>Open Professional Room Link</h3><p>Paste a professional link or RM code. The app will resolve it and open the active classroom immediately.</p></div>
    <div class="room-resolver-row-v20">
      <input id="roomLookupV20" placeholder="Paste room.garudaesp.id/RM-260609-URM4 or RM-260609-URM4">
      <button class="btn primary" onclick="openRoomFromInputV20()">Open Link</button>
    </div>
  </section>`;
}
function renderOnline(){
  ...
  const rooms = db.rooms.filter(r => {
    r = normalizeRoomV20(r);
    return r.scope === "all" || r.scope === session?.role || !r.scope;
  });
  byId("mainView").innerHTML = `<section class="online-v18">
    ${renderOpenRoomPanelV20()}
    ${manage ? roomForm() : `<section class="online18-card"><h3>Online Room</h3><p>Students can join scheduled rooms, open professional links, record attendance, use room chat, and access consultation sessions.</p></section>`}
    <section class="online18-card">
      <div class="online18-head"><div><h3>Meeting Schedule</h3><p>Every Join, Open Link, Copy, Share, and Details button is routed through the in-app room resolver.</p></div>${manage?`<button class="btn primary" onclick="resetRoomFormV18()">+ Create Room</button>`:''}</div>
      <div class="table-wrap room-table-v18"><table><thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Active Link</th><th>Actions</th></tr></thead><tbody>${rooms.map(roomRowV18).join("")}</tbody></table></div>
    </section>
  </section>`;
  setTimeout(syncRoomLinkV20,60);
}
function resolveURLRoomV20(){
  const params = new URLSearchParams(location.search || "");
  const code = params.get("room") || (location.hash.match(/RM-\d{6}-[A-Z0-9]{3,8}/i)||[])[0] || "";
  if(code){
    localStorage.setItem("gesn.pendingRoomCode", normalizeRoomCodeV20(code));
    history.replaceState(null,"",location.pathname);
  }
  const pending = localStorage.getItem("gesn.pendingRoomCode");
  if(pending && session?.role){
    localStorage.removeItem("gesn.pendingRoomCode");
    setTimeout(()=>openRoomByCodeV20(pending),400);
  }
}
window.addEventListener("DOMContentLoaded",()=>setTimeout(resolveURLRoomV20,600));
window.addEventListener("hashchange",resolveURLRoomV20);

Object.assign(window,{
  roomCodeFrom,generateRoomLink,normalizeRoomCodeV20,normalizeRoomV20,normalizeRoomsV20,
  activeRoomLinkHTMLV20,activeRoomLinkHTMLV19,findRoomByCodeOrLinkV20,openRoomByCodeV20,openRoomByCodeV19,
  openRoomFromInputV20,openRoomFromInputV19,syncRoomLinkV20,syncRoomLinkV18,
  copyGeneratedRoomLink,shareGeneratedRoomLink,copyRoomLink,shareRoomLink,joinRoom,
  openVirtualRoomV20,openVirtualRoomV19,openVirtualRoomV18,openRoomDetails,roomRowV18,renderOnline,resolveURLRoomV20
});


// ===== FINAL V21: HARD FIX — every room link is active and opens the in-app classroom =====
// This is intentionally appended last so it overrides all older room-link functions.
const ROOM_BASE_V21 = "https://room.garudaesp.id";
function roomCodeFrom(title='', dateValue='', timeValue=''){
  const date = (/^\d{4}-\d{2}-\d{2}$/.test(String(dateValue||''))) ? String(dateValue) : new Date().toISOString().slice(0,10);
  const [yyyy,mm,dd] = date.split('-');
  const cleaned = String(title || 'GARUDA ESP').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^A-Za-z0-9]+/g,'').slice(0,10).toUpperCase() || 'GARUDA';
  let seed = `${cleaned}|${date}|${timeValue||'09:00'}`;
  let h = 2166136261;
  for(let i=0;i<seed.length;i++){ h ^= seed.charCodeAt(i); h = Math.imul(h, 16777619); }
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let suffix = '';
...
function normalizeRoomCodeV21(value=''){
  const raw = String(value || '').trim();
  const direct = raw.match(/RM-\d{6}-[A-Z0-9]{3,10}/i);
  if(direct) return direct[0].toUpperCase();
  const query = raw.match(/[?&]room=([^&#]+)/i);
...
function professionalRoomURLV21(code){ return `${ROOM_BASE_V21}/${normalizeRoomCodeV21(code)}`; }
function inAppRoomActionHrefV21(code){ return `#GARUDA-ROOM-${normalizeRoomCodeV21(code)}`; }
function generateRoomLink(title='', dateValue='', timeValue=''){
  return professionalRoomURLV21(roomCodeFrom(title || 'GARUDA ESP Room', dateValue || new Date().toISOString().slice(0,10), timeValue || '09:00'));
}
function v21MonthNames(){ return ['January','February','March','April','May','June','July','August','September','October','November','December']; }
function v21Today(){ const d = new Date(); return {day:String(d.getDate()).padStart(2,'0'), month:v21MonthNames()[d.getMonth()], year:String(d.getFullYear()), iso:d.toISOString().slice(0,10)}; }
function v21DateISO(day,month,year){ const mi = v21MonthNames().indexOf(month); return `${year || v21Today().year}-${String((mi >= 0 ? mi : new Date().getMonth()) + 1).padStart(2,'0')}-${String(day || v21Today().day).padStart(2,'0')}`; }
function normalizeRoomV21(room){
  if(!room) return null;
  const today = v21Today();
  room.attendance ||= [];
  room.roomChat ||= [];
  room.features = Array.isArray(room.features) ? room.features : String(room.features || 'Video,Audio,Chat,Screen Share,Breakout Rooms,Recording,Attendance').split(',').map(x=>x.trim()).filter(Boolean);
  if(!room.day || !room.month || !room.year){
    if(room.date && /^\d{4}-\d{2}-\d{2}$/.test(String(room.date))){
      const [y,m,d] = String(room.date).split('-');
      room.day = d; room.month = v21MonthNames()[Number(m)-1] || today.month; room.year = y;
    } else { room.day = today.day; room.month = today.month; room.year = today.year; }
  }
  room.date = v21DateISO(room.day, room.month, room.year);
  room.startTime ||= '09:00'; room.endTime ||= '10:30'; room.timezone ||= 'Asia/Jakarta (GMT+7)';
  room.title ||= 'Garuda Main Virtual Classroom'; room.host ||= 'Garuda ESP Host'; room.courseId ||= selectedCourseId || 'tourism'; room.week ||= selectedWeek || 1; room.type ||= 'Virtual Meeting'; room.scope ||= 'all';
  room.agenda ||= 'Weekly orientation, course briefing, reflective discussion, and progress monitoring.';
  room.code = normalizeRoomCodeV21(room.code || room.link || roomCodeFrom(room.title, room.date, room.startTime));
  if(!room.code || !room.code.startsWith('RM-')) room.code = roomCodeFrom(room.title, room.date, room.startTime);
  room.link = professionalRoomURLV21(room.code);
  room.inAppHref = inAppRoomActionHrefV21(room.code);
  return room;
}
function normalizeRoomsV21(){
  db.rooms ||= [];
  db.rooms = db.rooms.map(normalizeRoomV21);
  if(!db.rooms.length){
    const t = v21Today();
    db.rooms.push(normalizeRoomV21({id:uid('ROOM'),title:'Garuda Main Virtual Classroom',host:'Garuda ESP Host',day:t.day,month:t.month,year:t.year,date:t.iso,startTime:'09:00',endTime:'10:30',timezone:'Asia/Jakarta (GMT+7)',courseId:'tourism',week:1,type:'Virtual Meeting',agenda:'Weekly orientation, course briefing, reflective discussion, and progress monitoring.',features:['Video','Audio','Chat','Screen Share','Breakout Rooms','Recording','Attendance']}));
  }
  save();
}
function findRoomByCodeOrLinkV21(value=''){
  normalizeRoomsV21();
  const code = normalizeRoomCodeV21(value);
  return (db.rooms || []).map(normalizeRoomV21).find(r => r.code === code || String(r.link).toUpperCase().includes(code));
}
function createRecoveredRoomV21(value=''){
  normalizeRoomsV21();
  const t = v21Today();
  const code = normalizeRoomCodeV21(value) || roomCodeFrom('Recovered GARUDA Room', t.iso, '09:00');
  let existing = findRoomByCodeOrLinkV21(code);
  if(existing) return existing;
  const room = normalizeRoomV21({id:uid('ROOM'),code,title:`Recovered Professional Room ${code}`,host:'Garuda ESP Host',day:t.day,month:t.month,year:t.year,date:t.iso,startTime:'09:00',endTime:'10:30',timezone:'Asia/Jakarta (GMT+7)',courseId:selectedCourseId || 'tourism',week:selectedWeek || 1,type:'Recovered Room Link',agenda:'This classroom was recovered from a professional room link or code. It is active for attendance, discussion, consultation, and learning documentation.',features:['Video','Audio','Chat','Screen Share','Breakout Rooms','Recording','Attendance'],scope:'all'});
  db.rooms.unshift(room); save(); return room;
}
function roomDateLabel(room){ const r = normalizeRoomV21(room); return `${String(r.day).padStart(2,'0')} ${r.month} ${r.year} • ${r.startTime}${r.endTime ? ' – ' + r.endTime : ''} ${r.timezone || ''}`; }
function activeRoomLinkHTMLV21(room){
  const r = normalizeRoomV21(room);
  return `<div class="active-room-link-v21" data-room-code="${esc(r.code)}"><a href="${esc(r.inAppHref)}" data-room-code="${esc(r.code)}" onclick="event.preventDefault();event.stopPropagation();openRoomByCodeV21('${esc(r.code)}');return false;">${esc(r.link)}</a><button type="button" onclick="openRoomByCodeV21('${esc(r.code)}')">Open</button><button type="button" onclick="copyRoomLink('${esc(r.id)}')">Copy</button></div>`;
}
function activeRoomLinkHTMLV20(room){ return activeRoomLinkHTMLV21(room); }
function activeRoomLinkHTMLV19(room){ return activeRoomLinkHTMLV21(room); }
function syncRoomLinkV21(){
  const title = byId('roomTitle')?.value || 'GARUDA ESP Room';
  const day = String(byId('roomDay')?.value || v21Today().day).padStart(2,'0');
  const month = byId('roomMonth')?.value || v21Today().month;
  const year = byId('roomYear')?.value || v21Today().year;
  const start = byId('roomStartTime')?.value || '09:00';
  const date = v21DateISO(day,month,year);
  const code = roomCodeFrom(title,date,start);
  const link = professionalRoomURLV21(code);
  if(byId('roomDate')) byId('roomDate').value = date;
  if(byId('roomCode')) byId('roomCode').value = code;
  if(byId('roomLink')) byId('roomLink').value = link;
  if(byId('roomLinkText')) byId('roomLinkText').innerHTML = `<a href="${inAppRoomActionHrefV21(code)}" data-room-code="${code}" onclick="event.preventDefault();openRoomByCodeV21('${code}');return false;">${link}</a>`;
}
function syncRoomLinkV20(){ return syncRoomLinkV21(); }
function syncRoomLinkV18(){ return syncRoomLinkV21(); }
function copyTextV21(text,label='Copied'){
  if(navigator.clipboard && window.isSecureContext){ navigator.clipboard.writeText(text).then(()=>toast(label,'Ready to paste/share.')).catch(()=>showManualCopyV21(label,text)); }
  else { showManualCopyV21(label,text); }
}
function showManualCopyV21(label,text){ openModal(`<h2>${esc(label)}</h2><p>Copy manually if the browser blocks clipboard access.</p><textarea class="manual-copy-v21" readonly>${esc(text)}</textarea>`); }
function shareTextV21(title,text){ if(navigator.share) navigator.share({title,text}).catch(()=>copyTextV21(text,'Room information copied')); else copyTextV21(text,'Room information copied'); }
function roomShareTextV21(room){ const r = normalizeRoomV21(room); return `GARUDA ESP Nusantara Online Room\n${r.title}\n${roomDateLabel(r)}\nRoom Code: ${r.code}\nProfessional Link: ${r.link}\n\nOpen inside the app:\n1. Go to Online Room.\n2. Paste the professional link or RM code into Open Professional Room Link.\n3. Click Open Link or Join.`; }
function copyGeneratedRoomLink(){ syncRoomLinkV21(); copyTextV21(roomShareTextV21({title:byId('roomTitle')?.value, code:byId('roomCode')?.value, link:byId('roomLink')?.value, day:byId('roomDay')?.value, month:byId('roomMonth')?.value, year:byId('roomYear')?.value, startTime:byId('roomStartTime')?.value, endTime:byId('roomEndTime')?.value, timezone:byId('roomTimezone')?.value, courseId:byId('roomCourse')?.value, week:byId('roomWeek')?.value}),'Room link copied'); }
function shareGeneratedRoomLink(){ syncRoomLinkV21(); shareTextV21('GARUDA ESP Nusantara Room', roomShareTextV21({title:byId('roomTitle')?.value, code:byId('roomCode')?.value, link:byId('roomLink')?.value, day:byId('roomDay')?.value, month:byId('roomMonth')?.value, year:byId('roomYear')?.value, startTime:byId('roomStartTime')?.value, endTime:byId('roomEndTime')?.value, timezone:byId('roomTimezone')?.value, courseId:byId('roomCourse')?.value, week:byId('roomWeek')?.value})); }
function copyRoomLink(id){ const r = normalizeRoomV21(db.rooms.find(x=>x.id===id)); if(!r) return toast('Room not found'); copyTextV21(roomShareTextV21(r),'Room link copied'); }
function shareRoomLink(id){ const r = normalizeRoomV21(db.rooms.find(x=>x.id===id)); if(!r) return toast('Room not found'); shareTextV21(r.title, roomShareTextV21(r)); }
function openRoomByCodeV21(value){ const code = normalizeRoomCodeV21(value); if(!code || !code.startsWith('RM-')) return toast('Room code needed','Use a professional RM room code or room.garudaesp.id link.'); let room = findRoomByCodeOrLinkV21(code); if(!room) room = createRecoveredRoomV21(code); joinRoom(room.id); }
function openRoomByCodeV20(value){ return openRoomByCodeV21(value); }
function openRoomByCodeV19(value){ return openRoomByCodeV21(value); }
function openRoomFromInputV21(){ const value = byId('roomLookupV21')?.value || byId('roomLookupV20')?.value || byId('roomLookupV19')?.value || ''; if(!value.trim()) return toast('Room link needed','Paste a room.garudaesp.id link or RM code first.'); openRoomByCodeV21(value); }
function openRoomFromInputV20(){ return openRoomFromInputV21(); }
function openRoomFromInputV19(){ return openRoomFromInputV21(); }
function recordAttendanceV21(room){ const user = session?.email || session?.name || session?.role || 'Guest'; room.attendance ||= []; if(!room.attendance.some(a=>a.user===user)){ room.attendance.push({user, role:session?.role || 'guest', name:session?.name || user, date:new Date().toISOString()}); } save(); }
function joinRoom(id){ normalizeRoomsV21(); const room = normalizeRoomV21(db.rooms.find(x=>x.id===id)); if(!room) return toast('Room not found'); recordAttendanceV21(room); openVirtualRoomV21(room.id); toast('Room opened',`${room.code} is active and attendance has been recorded.`); }
function postRoomChatV21(id){ const room = db.rooms.find(x=>x.id===id); const input = byId('roomChatInputV21'); const text = input?.value.trim(); if(!room || !text) return; room.roomChat ||= []; room.roomChat.push({user:session?.name || session?.email || session?.role || 'Guest', text, date:new Date().toISOString()}); save(); byId('roomChatListV21').innerHTML = roomChatListV21(room); input.value = ''; }
function roomChatListV21(room){ return (room.roomChat || []).length ? room.roomChat.map(m=>`<div class="room-chat-message-v21"><strong>${esc(m.user)}</strong><p>${esc(m.text)}</p><small>${new Date(m.date).toLocaleString()}</small></div>`).join('') : `<div class="empty">No room chat yet.</div>`; }
function toggleRoomToolV21(tool){ const status = byId('roomToolStatusV21'); if(status) status.innerHTML = `<strong>${esc(tool)} activated.</strong> The ${esc(tool.toLowerCase())} tool is ready in this in-app classroom.`; toast(`${tool} ready`,'Classroom tool activated.'); }
function openVirtualRoomV21(idOrCode){
  normalizeRoomsV21();
  let room = db.rooms.find(x=>x.id===idOrCode) || findRoomByCodeOrLinkV21(idOrCode) || createRecoveredRoomV21(idOrCode);
  room = normalizeRoomV21(room);
  const attendanceHTML = (room.attendance||[]).length ? room.attendance.map(a=>`<li>${esc(a.name || a.user)} — ${new Date(a.date).toLocaleString()}</li>`).join('') : `<li>No attendance yet.</li>`;
  openModal(`<section class="virtual-room-v21"><div class="vr-head-v21"><div class="vr-head-title-v21"><h2>${esc(room.title)}</h2><p>${esc(courseTitle(room.courseId || 'tourism'))} • Week ${esc(room.week || 1)} • ${esc(roomDateLabel(room))}</p></div><button type="button" class="room-code-pill-v21" onclick="copyRoomLink('${esc(room.id)}')" title="Copy professional room link">${esc(room.code)}</button></div><div class="vr-grid-v21"><section class="vr-stage-v21"><div class="vr-video-v21"><strong>GARUDA ESP NUSANTARA</strong><p>This virtual classroom is active. Join, Open Link, copied RM code, and active professional link all open this in-app room.</p><span>Live Room Ready</span></div><div class="vr-controls-v21"><button type="button" class="btn primary" onclick="toggleRoomToolV21('Video')">Video</button><button type="button" class="btn soft" onclick="toggleRoomToolV21('Audio')">Audio</button><button type="button" class="btn soft" onclick="toggleRoomToolV21('Screen Share')">Share Screen</button><button type="button" class="btn soft" onclick="toggleRoomToolV21('Breakout Room')">Breakout</button><button type="button" class="btn soft" onclick="copyRoomLink('${esc(room.id)}')">Copy Link</button><button type="button" class="btn soft" onclick="shareRoomLink('${esc(room.id)}')">Share</button><a class="btn soft room-open-anchor-v21" href="${esc(room.inAppHref)}" data-room-code="${esc(room.code)}" onclick="event.preventDefault();openRoomByCodeV21('${esc(room.code)}');return false;">Open Link</a></div><div id="roomToolStatusV21" class="tool-status-v21">Classroom tools are ready.</div></section><aside class="vr-side-v21"><h3>Agenda</h3><p>${esc(room.agenda)}</p><h3>Active Professional Link</h3><div class="active-link-box-v21"><a href="${esc(room.inAppHref)}" data-room-code="${esc(room.code)}" onclick="event.preventDefault();openRoomByCodeV21('${esc(room.code)}');return false;">${esc(room.link)}</a><button type="button" onclick="openRoomByCodeV21('${esc(room.code)}')">Open</button></div><h3>Attendance</h3><ul class="attendance-list-v21">${attendanceHTML}</ul><h3>Room Chat</h3><textarea id="roomChatInputV21" placeholder="Write a room message..."></textarea><button type="button" class="btn primary" onclick="postRoomChatV21('${esc(room.id)}')">Send Message</button><div id="roomChatListV21">${roomChatListV21(room)}</div></aside></div></section>`);
}
function openVirtualRoomV20(id){ return openVirtualRoomV21(id); }
function openVirtualRoomV19(id){ return openVirtualRoomV21(id); }
function openVirtualRoomV18(id){ return openVirtualRoomV21(id); }
function openRoomDetails(id){ const room = normalizeRoomV21(db.rooms.find(x=>x.id===id)); if(!room) return toast('Room not found'); openModal(`<h2>${esc(room.title)}</h2><div class="table-wrap"><table><tbody><tr><td><strong>Room Code</strong></td><td>${esc(room.code)}</td></tr><tr><td><strong>Active Professional Link</strong></td><td>${activeRoomLinkHTMLV21(room)}</td></tr><tr><td><strong>Course</strong></td><td>${esc(courseTitle(room.courseId||'tourism'))} • Week ${esc(room.week)}</td></tr><tr><td><strong>Date & Time</strong></td><td>${esc(roomDateLabel(room))}</td></tr><tr><td><strong>Host</strong></td><td>${esc(room.host)}</td></tr><tr><td><strong>Agenda</strong></td><td>${esc(room.agenda)}</td></tr></tbody></table></div><div class="actions"><button type="button" class="btn primary" onclick="joinRoom('${esc(room.id)}')">Join Room</button><button type="button" class="btn soft" onclick="openRoomByCodeV21('${esc(room.code)}')">Open Link</button><button type="button" class="btn soft" onclick="copyRoomLink('${esc(room.id)}')">Copy</button><button type="button" class="btn soft" onclick="shareRoomLink('${esc(room.id)}')">Share</button></div>`); }
function roomRowV18(room){ const r = normalizeRoomV21(room); return `<tr><td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(r.type)}</span></td><td>${esc(courseTitle(r.courseId || 'tourism'))}<br><small>Week ${esc(r.week)}</small></td><td>${esc(roomDateLabel(r))}</td><td>${esc(r.host)}</td><td>${(r.attendance||[]).length}</td><td>${activeRoomLinkHTMLV21(r)}</td><td class="room-actions"><button type="button" class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join</button><button type="button" class="btn soft" onclick="openRoomByCodeV21('${esc(r.code)}')">Open Link</button><button type="button" class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button><button type="button" class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button><button type="button" class="btn soft" onclick="openRoomDetails('${esc(r.id)}')">Details</button>${['admin','lecturer'].includes(session?.role) ? `<button type="button" class="btn soft" onclick="editRoom('${esc(r.id)}')">Edit</button><button type="button" class="btn danger" onclick="deleteRoom('${esc(r.id)}')">Delete</button>` : ''}</td></tr>`; }
function renderOpenRoomPanelV21(){ return `<section class="online18-card room-resolver-v21"><div><h3>Open Professional Room Link</h3><p>Paste any professional room link or RM code. It opens inside this app and records attendance.</p></div><div class="room-resolver-row-v21"><input id="roomLookupV21" placeholder="room.garudaesp.id/RM-260609-URM4 or RM-260609-URM4"><button type="button" class="btn primary" onclick="openRoomFromInputV21()">Open Link</button></div></section>`; }
function renderOnline(){ normalizeRoomsV21(); const manage = ['admin','lecturer'].includes(session?.role); const rooms = db.rooms.filter(r=>{ r=normalizeRoomV21(r); return r.scope === 'all' || r.scope === session?.role || !r.scope; }); byId('mainView').innerHTML = `<section class="online-v18">${renderOpenRoomPanelV21()}${manage ? roomForm() : `<section class="online18-card"><h3>Online Room</h3><p>Join scheduled rooms, paste professional room links, record attendance, and use room chat and classroom tools.</p></section>`}<section class="online18-card"><div class="online18-head"><div><h3>Meeting Schedule</h3><p>All room links are active in-app: Join, Open Link, Details, Copy, Share, and professional URL.</p></div>${manage ? `<button type="button" class="btn primary" onclick="resetRoomFormV18()">+ Create Room</button>` : ''}</div><div class="table-wrap room-table-v18"><table><thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Active Link</th><th>Actions</th></tr></thead><tbody>${rooms.map(roomRowV18).join('')}</tbody></table></div></section></section>`; setTimeout(syncRoomLinkV21, 60); }
document.addEventListener('click', function(e){ const target = e.target.closest && e.target.closest('a,button,[data-room-code]'); if(!target) return; const href = target.getAttribute('href') || ''; const codeAttr = target.dataset?.roomCode || ''; const text = target.textContent || ''; const code = normalizeRoomCodeV21(codeAttr || href || text); if(code && code.startsWith('RM-') && (href.includes('room.garudaesp.id') || href.includes('GARUDA-ROOM') || codeAttr || text.includes('room.garudaesp.id'))){ e.preventDefault(); e.stopPropagation(); openRoomByCodeV21(code); return false; } }, true);
function resolveURLRoomV21(){ const params = new URLSearchParams(location.search || ''); const fromQuery = params.get('room'); const fromHash = (location.hash.match(/RM-\d{6}-[A-Z0-9]{3,10}/i)||[])[0]; const code = normalizeRoomCodeV21(fromQuery || fromHash || ''); if(code && code.startsWith('RM-')){ if(session?.role){ setTimeout(()=>openRoomByCodeV21(code),300); } else { localStorage.setItem('gesn.pendingRoomCodeV21', code); } history.replaceState(null,'',location.pathname); } }
function processPendingRoomV21(){ const code = localStorage.getItem('gesn.pendingRoomCodeV21'); if(code && session?.role){ localStorage.removeItem('gesn.pendingRoomCodeV21'); setTimeout(()=>openRoomByCodeV21(code),500); } }
window.addEventListener('DOMContentLoaded',()=>setTimeout(resolveURLRoomV21,500));
window.addEventListener('hashchange',resolveURLRoomV21);
const _v21OldOpenApp = window.openApp;
if(typeof _v21OldOpenApp === 'function'){ window.openApp = function(role){ const result = _v21OldOpenApp(role); setTimeout(processPendingRoomV21,700); return result; }; }
function runRoomLinkDiagnosticsV21(){ normalizeRoomsV21(); const r = normalizeRoomV21(db.rooms[0]); return [['Room exists',!!r],['Room code valid',/^RM-\d{6}-[A-Z0-9]{3,10}$/.test(r.code)],['Professional display URL',r.link.startsWith(ROOM_BASE_V21 + '/RM-')],['In-app action href',r.inAppHref.startsWith('#GARUDA-ROOM-RM-')],['Resolver finds room',!!findRoomByCodeOrLinkV21(r.code)],['Active HTML has onclick',activeRoomLinkHTMLV21(r).includes('openRoomByCodeV21')],['Join function available',typeof joinRoom==='function'],['Virtual room available',typeof openVirtualRoomV21==='function'],['Input opener available',typeof openRoomFromInputV21==='function']]; }
Object.assign(window,{ROOM_BASE_V21,roomCodeFrom,generateRoomLink,normalizeRoomCodeV21,professionalRoomURLV21,inAppRoomActionHrefV21,normalizeRoomV21,normalizeRoomsV21,findRoomByCodeOrLinkV21,createRecoveredRoomV21,roomDateLabel,activeRoomLinkHTMLV21,activeRoomLinkHTMLV20,activeRoomLinkHTMLV19,syncRoomLinkV21,syncRoomLinkV20,syncRoomLinkV18,copyGeneratedRoomLink,shareGeneratedRoomLink,copyRoomLink,shareRoomLink,openRoomByCodeV21,openRoomByCodeV20,openRoomByCodeV19,openRoomFromInputV21,openRoomFromInputV20,openRoomFromInputV19,joinRoom,openVirtualRoomV21,openVirtualRoomV20,openVirtualRoomV19,openVirtualRoomV18,openRoomDetails,roomRowV18,renderOnline,resolveURLRoomV21,runRoomLinkDiagnosticsV21});


// ===== FINAL V22: DIRECT WORKING ROOM PAGE — Join and every room link open room.html =====
// Reason: static deployment cannot make an unconfigured external domain work.
// Therefore every visible/professional room link now points to an internal room.html page that is packaged in this ZIP.
// The professional room code remains visible and shareable, but the actual click always opens a working in-app classroom.

const ROOM_DISPLAY_BASE_V22 = "https://room.garudaesp.id";

function normalizeRoomCodeV22(value=""){
  const raw = String(value || "").trim();
...
function roomCodeFrom(title="", dateValue="", timeValue=""){
  const date = (/^\d{4}-\d{2}-\d{2}$/.test(String(dateValue||""))) ? String(dateValue) : new Date().toISOString().slice(0,10);
  const [yyyy,mm,dd] = date.split("-");
  const clean = String(title || "GARUDA ESP").normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^A-Za-z0-9]+/g,"").slice(0,10).toUpperCase() || "GARUDA";
  let h = 2166136261;
  const seed = `${clean}|${date}|${timeValue||"09:00"}`;
  for(let i=0;i<seed.length;i++){ h ^= seed.charCodeAt(i); h = Math.imul(h,16777619); }
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
  h = Math.abs(h);
  let suffix = "";
...
function professionalRoomURLV22(code){ return `${ROOM_DISPLAY_BASE_V22}/${normalizeRoomCodeV22(code)}`; }
function appBaseURLV22(){
...
function workingRoomURLV22(code){
  return new URL(`room.html?code=${encodeURIComponent(normalizeRoomCodeV22(code))}`, appBaseURLV22()).href;
}
function generateRoomLink(title="", dateValue="", timeValue=""){
  return professionalRoomURLV22(roomCodeFrom(title || "GARUDA ESP Room", dateValue || new Date().toISOString().slice(0,10), timeValue || "09:00"));
}
function v22MonthNames(){ return ["January","February","March","April","May","June","July","August","September","October","November","December"]; }
function v22Today(){ const d=new Date(); return {day:String(d.getDate()).padStart(2,"0"),month:v22MonthNames()[d.getMonth()],year:String(d.getFullYear()),iso:d.toISOString().slice(0,10)}; }
function v22DateISO(day,month,year){ const mi=v22MonthNames().indexOf(month); return `${year||v22Today().year}-${String((mi>=0?mi:new Date().getMonth())+1).padStart(2,"0")}-${String(day||v22Today().day).padStart(2,"0")}`; }

function normalizeRoomV22(room){
  if(!room) return null;
  const t = v22Today();
  room.attendance ||= [];
  room.roomChat ||= [];
  ...
  if(!room.day || !room.month || !room.year){
    if(room.date && /^\d{4}-\d{2}-\d{2}$/.test(String(room.date))){
      const [y,m,d] = String(room.date).split("-");
      room.day = d; room.month = v22MonthNames()[Number(m)-1] || t.month; room.year = y;
    } else {
      room.day = t.day; room.month = t.month; room.year = t.year;
    }
  }
  room.date = v22DateISO(room.day, room.month, room.year);
  room.startTime ||= "09:00";
  room.endTime ||= "10:30";
  room.timezone ||= "Asia/Jakarta (GMT+7)";
  room.title ||= "Garuda Main Virtual Classroom";
  room.host ||= "Garuda ESP Host";
  room.courseId ||= selectedCourseId || "tourism";
  room.week ||= selectedWeek || 1;
  room.type ||= "Virtual Meeting";
  room.scope ||= "all";
  room.agenda ||= "Weekly orientation, course briefing, reflective discussion, and progress monitoring.";
  room.code = normalizeRoomCodeV22(room.code || room.link || roomCodeFrom(room.title,room.date,room.startTime));
  if(!room.code || !room.code.startsWith("RM-")) room.code = roomCodeFrom(room.title,room.date,room.startTime);
  room.link = professionalRoomURLV22(room.code);       // displayed professional identity
  room.workingLink = workingRoomURLV22(room.code);     // the actual clickable link
  return room;
}
function normalizeRoomsV22(){
  db.rooms ||= [];
  db.rooms = db.rooms.map(normalizeRoomV22);
  if(!db.rooms.length){
    const t=v22Today();
    db.rooms.push(normalizeRoomV22({
      id:uid("ROOM"),
      title:"Garuda Main Virtual Classroom",
      host:"Garuda ESP Host",
      day:t.day, month:t.month, year:t.year, date:t.iso,
      startTime:"09:00", endTime:"10:30", timezone:"Asia/Jakarta (GMT+7)",
      courseId:"tourism", week:1, type:"Virtual Meeting",
      agenda:"Weekly orientation, course briefing, reflective discussion, and progress monitoring.",
      features:["Video","Audio","Chat","Screen Share","Breakout Rooms","Recording","Attendance"],
      scope:"all"
    }));
...
function findRoomByCodeOrLinkV22(value=""){
  normalizeRoomsV22();
  const code = normalizeRoomCodeV22(value);
  return db.rooms.map(normalizeRoomV22).find(r => r.code === code || String(r.link).toUpperCase().includes(code) || String(r.workingLink).toUpperCase().includes(code));
}
function createRecoveredRoomV22(value=""){
  normalizeRoomsV22();
  const t=v22Today();
  const code = normalizeRoomCodeV22(value) || roomCodeFrom("Recovered GARUDA Room",t.iso,"09:00");
  let existing = findRoomByCodeOrLinkV22(code);
  if(existing) return existing;
  const room = normalizeRoomV22({
    id:uid("ROOM"),
    code,
    title:`Recovered Professional Room ${code}`,
    host:"Garuda ESP Host",
    day:t.day, month:t.month, year:t.year, date:t.iso,
    startTime:"09:00", endTime:"10:30", timezone:"Asia/Jakarta (GMT+7)",
    courseId:selectedCourseId || "tourism",
    week:selectedWeek || 1,
    type:"Recovered Room Link",
    agenda:"This classroom was recovered from a room code or shared link. It is active for attendance, discussion, consultation, and learning documentation.",
    features:["Video","Audio","Chat","Screen Share","Breakout Rooms","Recording","Attendance"],
    scope:"all"
  });
  db.rooms.unshift(room);
...
function roomDateLabel(room){
  const r=normalizeRoomV22(room);
  return `${String(r.day).padStart(2,"0")} ${r.month} ${r.year} • ${r.startTime}${r.endTime ? " – " + r.endTime : ""} ${r.timezone || ""}`;
}
function recordAttendanceV22(room){
  const user = session?.email || session?.name || session?.role || "Guest";
  room.attendance ||= [];
...
function goToRoomPageV22(roomOrCode){
  normalizeRoomsV22();
  let room = typeof roomOrCode === "string"
    ? (db.rooms.find(r=>r.id===roomOrCode) || findRoomByCodeOrLinkV22(roomOrCode) || createRecoveredRoomV22(roomOrCode))
    : roomOrCode;
  room = normalizeRoomV22(room);
  recordAttendanceV22(room);
  location.href = room.workingLink;
}
function joinRoom(id){ return goToRoomPageV22(id); }
function openRoomByCodeV22(value){ return goToRoomPageV22(normalizeRoomCodeV22(value)); }
function openRoomByCodeV21(value){ return openRoomByCodeV22(value); }
function openRoomByCodeV20(value){ return openRoomByCodeV22(value); }
function openRoomByCodeV19(value){ return openRoomByCodeV22(value); }
function openRoomFromInputV22(){
  const value = byId("roomLookupV22")?.value || byId("roomLookupV21")?.value || byId("roomLookupV20")?.value || byId("roomLookupV19")?.value || "";
  if(!value.trim()) return toast("Room link needed","Paste the room code or room link first.");
  openRoomByCodeV22(value);
}
function openRoomFromInputV21(){ return openRoomFromInputV22(); }
function openRoomFromInputV20(){ return openRoomFromInputV22(); }
function openRoomFromInputV19(){ return openRoomFromInputV22(); }

function syncRoomLinkV22(){
  const title = byId("roomTitle")?.value || "GARUDA ESP Room";
  const day = String(byId("roomDay")?.value || v22Today().day).padStart(2,"0");
  const month = byId("roomMonth")?.value || v22Today().month;
  const year = byId("roomYear")?.value || v22Today().year;
  const start = byId("roomStartTime")?.value || "09:00";
  const date = v22DateISO(day,month,year);
  const code = roomCodeFrom(title,date,start);
  const display = professionalRoomURLV22(code);
  const working = workingRoomURLV22(code);
  if(byId("roomDate")) byId("roomDate").value = date;
  if(byId("roomCode")) byId("roomCode").value = code;
  if(byId("roomLink")) byId("roomLink").value = working;
  if(byId("roomLinkText")) byId("roomLinkText").innerHTML = `<a href="${esc(working)}" data-room-code="${esc(code)}">${esc(display)}</a>`;
}
function syncRoomLinkV21(){ return syncRoomLinkV22(); }
function syncRoomLinkV20(){ return syncRoomLinkV22(); }
function syncRoomLinkV18(){ return syncRoomLinkV22(); }

function activeRoomLinkHTMLV22(room){
  const r = normalizeRoomV22(room);
  return `<div class="active-room-link-v22" data-room-code="${esc(r.code)}">
    <a href="${esc(r.workingLink)}" data-room-code="${esc(r.code)}" title="Open working room page">${esc(r.link)}</a>
    <button type="button" onclick="goToRoomPageV22('${esc(r.id)}')">Open</button>
    <button type="button" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
  </div>`;
}
function activeRoomLinkHTMLV21(room){ return activeRoomLinkHTMLV22(room); }
function activeRoomLinkHTMLV20(room){ return activeRoomLinkHTMLV22(room); }
function activeRoomLinkHTMLV19(room){ return activeRoomLinkHTMLV22(room); }

function copyTextV22(text,label="Copied"){
  if(navigator.clipboard && window.isSecureContext){
    navigator.clipboard.writeText(text).then(()=>toast(label,"Working room link copied.")).catch(()=>manualCopyV22(label,text));
  } else {
    manualCopyV22(label,text);
  }
}
function manualCopyV22(label,text){
  openModal(`<h2>${esc(label)}</h2><p>Copy manually if your browser blocks clipboard access.</p><textarea class="manual-copy-v22" readonly>${esc(text)}</textarea>`);
}
function roomShareTextV22(room){
  const r=normalizeRoomV22(room);
  return `GARUDA ESP Nusantara Online Room
${r.title}
${roomDateLabel(r)}
Room Code: ${r.code}
Working App Link: ${r.workingLink}
Professional Room Identity: ${r.link}`;
}
function copyGeneratedRoomLink(){
  syncRoomLinkV22();
  const code=byId("roomCode")?.value || "";
  copyTextV22(`GARUDA ESP Nusantara Online Room
Room Code: ${code}
Working App Link: ${workingRoomURLV22(code)}
Professional Room Identity: ${professionalRoomURLV22(code)}`,"Room link copied");
}
function shareGeneratedRoomLink(){
  syncRoomLinkV22();
  const code=byId("roomCode")?.value || "";
...
Room Code: ${code}
Working App Link: ${workingRoomURLV22(code)}
Professional Room Identity: ${professionalRoomURLV22(code)}`;
  if(navigator.share) navigator.share({title:"GARUDA ESP Nusantara Online Room",text,url:workingRoomURLV22(code)}).catch(()=>copyTextV22(text,"Room link copied"));
  else copyTextV22(text,"Room link copied");
}
function copyRoomLink(id){
  const r=normalizeRoomV22(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  copyTextV22(roomShareTextV22(r),"Room link copied");
}
function shareRoomLink(id){
  const r=normalizeRoomV22(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  if(navigator.share) navigator.share({title:r.title,text:roomShareTextV22(r),url:r.workingLink}).catch(()=>copyTextV22(roomShareTextV22(r),"Room link copied"));
  else copyTextV22(roomShareTextV22(r),"Room link copied");
}
...
function roomRowV18(room){
  ...
  return `<tr>
    <td><strong>${esc(r.title)}</strong><br><span class="tag red">${esc(r.type)}</span></td>
    <td>${esc(courseTitle(r.courseId || "tourism"))}<br><small>Week ${esc(r.week)}</small></td>
    <td>${esc(roomDateLabel(r))}</td>
    <td>${esc(r.host)}</td>
    <td>${(r.attendance||[]).length}</td>
    <td>${activeRoomLinkHTMLV22(r)}</td>
    <td class="room-actions">
      <button type="button" class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join</button>
      <button type="button" class="btn soft" onclick="goToRoomPageV22('${esc(r.id)}')">Open Link</button>
      <button type="button" class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button type="button" class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
      <button type="button" class="btn soft" onclick="openRoomDetails('${esc(r.id)}')">Details</button>
      ${['admin','lecturer'].includes(session?.role) ? `<button type="button" class="btn soft" onclick="editRoom('${esc(r.id)}')">Edit</button><button type="button" class="btn danger" onclick="deleteRoom('${esc(r.id)}')">Delete</button>` : ""}
    </td>
  </tr>`;
}
function renderOpenRoomPanelV22(){
  return `<section class="online18-card room-resolver-v22">
    <div>
      <h3>Open Professional Room Link</h3>
      <p>Paste any RM code or professional room identity. It opens the packaged working room page directly.</p>
    </div>
    <div class="room-resolver-row-v22">
      <input id="roomLookupV22" placeholder="RM-260609-URM4 or room.garudaesp.id/RM-260609-URM4">
      <button type="button" class="btn primary" onclick="openRoomFromInputV22()">Open Link</button>
    </div>
  </section>`;
}
function renderOnline(){
  normalizeRoomsV22();
  const manage=['admin','lecturer'].includes(session?.role);
  const rooms=db.rooms.filter(r=>{r=normalizeRoomV22(r);return r.scope==='all'||r.scope===session?.role||!r.scope;});
  byId("mainView").innerHTML=`<section class="online-v18">
    ${renderOpenRoomPanelV22()}
    ${manage ? roomForm() : `<section class="online18-card"><h3>Online Room</h3><p>Students can join scheduled rooms, paste RM codes, record attendance, use room chat, and access the direct virtual classroom page.</p></section>`}
    <section class="online18-card">
      <div class="online18-head">
        <div><h3>Meeting Schedule</h3><p>Join and every link now open the direct packaged room page, not Google search and not a broken external domain.</p></div>
        ${manage ? `<button type="button" class="btn primary" onclick="resetRoomFormV18()">+ Create Room</button>` : ""}
      </div>
      <div class="table-wrap room-table-v18"><table>
        <thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Active Link</th><th>Actions</th></tr></thead>
        <tbody>${rooms.map(roomRowV18).join("")}</tbody>
      </table></div>
    </section>
  </section>`;
  setTimeout(syncRoomLinkV22,80);
}
function openRoomDetails(id){
  const r=normalizeRoomV22(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  openModal(`<h2>${esc(r.title)}</h2>
    <div class="table-wrap"><table><tbody>
      <tr><td><strong>Room Code</strong></td><td>${esc(r.code)}</td></tr>
      <tr><td><strong>Working App Link</strong></td><td><a href="${esc(r.workingLink)}" data-room-code="${esc(r.code)}">${esc(r.workingLink)}</a></td></tr>
      <tr><td><strong>Professional Identity</strong></td><td>${esc(r.link)}</td></tr>
      <tr><td><strong>Course</strong></td><td>${esc(courseTitle(r.courseId||"tourism"))} • Week ${esc(r.week)}</td></tr>
      <tr><td><strong>Date & Time</strong></td><td>${esc(roomDateLabel(r))}</td></tr>
      <tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr>
      <tr><td><strong>Agenda</strong></td><td>${esc(r.agenda)}</td></tr>
    ...
    <div class="actions">
      <button type="button" class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join Direct Room</button>
      <button type="button" class="btn soft" onclick="goToRoomPageV22('${esc(r.id)}')">Open Link</button>
      <button type="button" class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button type="button" class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
    </div>`);
...
// Global interception: any remaining room.garudaesp.id or RM code link opens room.html, never Google.
document.addEventListener("click", function(e){
  const el=e.target.closest && e.target.closest("a,button,[data-room-code]");
  if(!el) return;
  const href=el.getAttribute("href")||"";
  const data=el.dataset?.roomCode||"";
  const txt=el.textContent||"";
  const maybe=data||href||txt;
  const code=normalizeRoomCodeV22(maybe);
  if(code && code.startsWith("RM-") && (data || href.includes("room.html") || href.includes("room.garudaesp.id") || txt.includes("room.garudaesp.id"))){
    if(href.includes("room.html")){
      return; // allow real working room page link
    }
    e.preventDefault();
    e.stopPropagation();
    openRoomByCodeV22(code);
    return false;
  }
}, true);

function resolveURLRoomV22(){
  const params=new URLSearchParams(location.search||"");
  const fromQuery=params.get("room")||params.get("code");
  const fromHash=(location.hash.match(/RM-\d{6}-[A-Z0-9]{3,12}/i)||[])[0];
  const code=normalizeRoomCodeV22(fromQuery||fromHash||"");
  if(code && code.startsWith("RM-")){
    if(session?.role) setTimeout(()=>openRoomByCodeV22(code),300);
    else localStorage.setItem("gesn.pendingRoomCodeV22",code);
    history.replaceState(null,"",location.pathname);
  }
}
function processPendingRoomV22(){
  const code=localStorage.getItem("gesn.pendingRoomCodeV22");
  if(code && session?.role){
    localStorage.removeItem("gesn.pendingRoomCodeV22");
    setTimeout(()=>openRoomByCodeV22(code),600);
  }
}
window.addEventListener("DOMContentLoaded",()=>setTimeout(resolveURLRoomV22,500));
window.addEventListener("hashchange",resolveURLRoomV22);
const _v22OpenApp = window.openApp;
if(typeof _v22OpenApp === "function"){
  window.openApp=function(role){
    const r=_v22OpenApp(role);
    setTimeout(processPendingRoomV22,700);
    return r;
  };
}
function runRoomLinkDiagnosticsV22(){
  normalizeRoomsV22();
  const r=normalizeRoomV22(db.rooms[0]);
  return [
    ["Room exists",!!r],
    ["Code valid",/^RM-\d{6}-[A-Z0-9]{3,12}$/.test(r.code)],
    ["Professional identity displayed",r.link.startsWith(ROOM_DISPLAY_BASE_V22+"/RM-")],
    ["Working page link",r.workingLink.includes("room.html?code=RM-")],
    ["Join uses direct page",String(joinRoom).includes("goToRoomPageV22")],
    ["Active link uses room.html",activeRoomLinkHTMLV22(r).includes("room.html?code=")],
    ["Resolver creates missing room",typeof createRecoveredRoomV22==="function"],
    ["Input opener available",typeof openRoomFromInputV22==="function"]
  ];
}
Object.assign(window,{
  normalizeRoomCodeV22,roomCodeFrom,professionalRoomURLV22,workingRoomURLV22,generateRoomLink,
  normalizeRoomV22,normalizeRoomsV22,findRoomByCodeOrLinkV22,createRecoveredRoomV22,roomDateLabel,
  activeRoomLinkHTMLV22,activeRoomLinkHTMLV21,activeRoomLinkHTMLV20,activeRoomLinkHTMLV19,
  syncRoomLinkV22,syncRoomLinkV21,syncRoomLinkV20,syncRoomLinkV18,
  goToRoomPageV22,joinRoom,openRoomByCodeV22,openRoomByCodeV21,openRoomByCodeV20,openRoomByCodeV19,
  openRoomFromInputV22,openRoomFromInputV21,openRoomFromInputV20,openRoomFromInputV19,
  copyGeneratedRoomLink,shareGeneratedRoomLink,copyRoomLink,shareRoomLink,
  roomRowV18,renderOnline,openRoomDetails,resolveURLRoomV22,runRoomLinkDiagnosticsV22
});


// ===== FINAL V23: REAL DIRECT MEETING ROOM FIX =====
// Join now opens room.html with autostart=1. The room page loads a real meeting iframe.
// All room links are clickable. Professional identity is displayed, but working links go to the real room page.

const ROOM_DISPLAY_BASE_V23 = "https://room.garudaesp.id";
const MEET_BASE_V23 = "https://meet.jit.si";

function normalizeRoomCodeV23(value=""){
  const raw = String(value || "").trim();
  const direct = raw.match(/RM-\d{6}-[A-Z0-9]{3,12}/i);
  if(direct) return direct[0].toUpperCase();
  const q = raw.match(/[?&](?:code|room)=([^&#]+)/i);
  if(q) return decodeURIComponent(q[1]).toUpperCase().replace(/[^A-Z0-9-]/g,"");
  return raw.replace(/^.*\//,"").replace(/^.*#/,"").replace(/[^A-Z0-9-]/gi,"").toUpperCase();
}
function meetingSlugV23(code){
  return `GARUDA-ESP-NUSANTARA-${normalizeRoomCodeV23(code)}`.replace(/[^A-Z0-9-]/gi,"-").toUpperCase();
}
function realMeetingURLV23(code){
  return `${MEET_BASE_V23}/${meetingSlugV23(code)}#config.prejoinPageEnabled=false&config.startWithAudioMuted=false&config.startWithVideoMuted=false`;
}
function appBaseURLV23(){
  const u = new URL(location.href);
  u.search = "";
  u.hash = "";
  if(u.pathname.endsWith("/")) return u.href;
...
function workingRoomURLV23(code){
  return new URL(`room.html?code=${encodeURIComponent(normalizeRoomCodeV23(code))}&autostart=1`, appBaseURLV23()).href;
}
function professionalRoomURLV23(code){
  return `${ROOM_DISPLAY_BASE_V23}/${normalizeRoomCodeV23(code)}`;
}
function normalizeRoomV23(room){
  if(!room) return null;
  room = normalizeRoomV22 ? normalizeRoomV22(room) : room;
  room.code = normalizeRoomCodeV23(room.code || room.link);
  room.link = professionalRoomURLV23(room.code);
  room.workingLink = workingRoomURLV23(room.code);
  room.meetingLink = realMeetingURLV23(room.code);
  return room;
}
function normalizeRoomsV23(){
  if(typeof normalizeRoomsV22 === "function") normalizeRoomsV22();
  db.rooms ||= [];
  db.rooms = db.rooms.map(normalizeRoomV23);
  save();
}
function findRoomByCodeOrLinkV23(value=""){
  normalizeRoomsV23();
  const code = normalizeRoomCodeV23(value);
  return db.rooms.find(r => normalizeRoomV23(r).code === code || String(r.link).toUpperCase().includes(code) || String(r.workingLink).toUpperCase().includes(code) || String(r.meetingLink).toUpperCase().includes(code));
}
function createRecoveredRoomV23(value=""){
  const code = normalizeRoomCodeV23(value);
  let r = findRoomByCodeOrLinkV23(code);
  if(r) return normalizeRoomV23(r);
  r = createRecoveredRoomV22 ? createRecoveredRoomV22(code) : null;
  r = normalizeRoomV23(r);
  save();
  return r;
}
function recordAttendanceV23(room){
  const r = normalizeRoomV23(room);
  const user = session?.email || session?.name || session?.role || "Guest";
  r.attendance ||= [];
  if(!r.attendance.some(a => a.user === user)){
    r.attendance.push({user, role:session?.role || "guest", name:session?.name || user, date:new Date().toISOString()});
...
function goToRoomPageV23(roomOrCode){
  normalizeRoomsV23();
  let r = typeof roomOrCode === "string"
    ? (db.rooms.find(x => x.id === roomOrCode) || findRoomByCodeOrLinkV23(roomOrCode) || createRecoveredRoomV23(roomOrCode))
    : roomOrCode;
  r = normalizeRoomV23(r);
  recordAttendanceV23(r);
  sessionStorage.setItem("garuda.openRoom.autostart", r.code);
  location.href = r.workingLink;
}
function joinRoom(id){ return goToRoomPageV23(id); }
function openRoomByCodeV23(value){ return goToRoomPageV23(normalizeRoomCodeV23(value)); }
function openRoomByCodeV22(value){ return openRoomByCodeV23(value); }
function openRoomByCodeV21(value){ return openRoomByCodeV23(value); }
function openRoomByCodeV20(value){ return openRoomByCodeV23(value); }
function openRoomByCodeV19(value){ return openRoomByCodeV23(value); }
function openRoomFromInputV23(){
  const value = byId("roomLookupV23")?.value || byId("roomLookupV22")?.value || byId("roomLookupV21")?.value || byId("roomLookupV20")?.value || byId("roomLookupV19")?.value || "";
  if(!value.trim()) return toast("Room link needed","Paste the room code or room link first.");
  openRoomByCodeV23(value);
}
function openRoomFromInputV22(){ return openRoomFromInputV23(); }
function openRoomFromInputV21(){ return openRoomFromInputV23(); }
function openRoomFromInputV20(){ return openRoomFromInputV23(); }
function openRoomFromInputV19(){ return openRoomFromInputV23(); }

function syncRoomLinkV23(){
  const title = byId("roomTitle")?.value || "GARUDA ESP Room";
  const day = String(byId("roomDay")?.value || (typeof v22Today==="function" ? v22Today().day : String(new Date().getDate()).padStart(2,"0"))).padStart(2,"0");
  const month = byId("roomMonth")?.value || (typeof v22Today==="function" ? v22Today().month : "June");
  const year = byId("roomYear")?.value || (typeof v22Today==="function" ? v22Today().year : String(new Date().getFullYear()));
  const start = byId("roomStartTime")?.value || "09:00";
  const date = (typeof v22DateISO==="function") ? v22DateISO(day, month, year) : `${year}-06-${day}`;
  const code = roomCodeFrom(title,date,start);
  const rlink = workingRoomURLV23(code);
  if(byId("roomDate")) byId("roomDate").value = date;
  if(byId("roomCode")) byId("roomCode").value = code;
  if(byId("roomLink")) byId("roomLink").value = rlink;
  if(byId("roomLinkText")) byId("roomLinkText").innerHTML = `<a href="${esc(rlink)}" data-room-code="${esc(code)}">${esc(professionalRoomURLV23(code))}</a>`;
}
function syncRoomLinkV22(){ return syncRoomLinkV23(); }
function syncRoomLinkV21(){ return syncRoomLinkV23(); }
function syncRoomLinkV20(){ return syncRoomLinkV23(); }
function syncRoomLinkV18(){ return syncRoomLinkV23(); }

function activeRoomLinkHTMLV23(room){
  const r = normalizeRoomV23(room);
  return `<div class="active-room-link-v23" data-room-code="${esc(r.code)}">
    <a href="${esc(r.workingLink)}" data-room-code="${esc(r.code)}" title="Join live virtual meeting">${esc(r.link)}</a>
    <button type="button" onclick="goToRoomPageV23('${esc(r.id)}')">Join</button>
    <button type="button" onclick="window.open('${esc(r.meetingLink)}','_blank')">Meeting</button>
    <button type="button" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
  </div>`;
}
function activeRoomLinkHTMLV22(room){ return activeRoomLinkHTMLV23(room); }
function activeRoomLinkHTMLV21(room){ return activeRoomLinkHTMLV23(room); }
function activeRoomLinkHTMLV20(room){ return activeRoomLinkHTMLV23(room); }
function activeRoomLinkHTMLV19(room){ return activeRoomLinkHTMLV23(room); }

function roomShareTextV23(room){
  const r = normalizeRoomV23(room);
  return `GARUDA ESP Nusantara Online Room
${r.title}
${roomDateLabel(r)}
Room Code: ${r.code}
Join Link: ${r.workingLink}
Direct Meeting Link: ${r.meetingLink}
Professional Identity: ${r.link}`;
}
function copyRoomLink(id){
  const r = normalizeRoomV23(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  copyTextV22 ? copyTextV22(roomShareTextV23(r),"Room link copied") : navigator.clipboard.writeText(roomShareTextV23(r));
}
function shareRoomLink(id){
  const r = normalizeRoomV23(db.rooms.find(x=>x.id===id));
  if(!r) return toast("Room not found");
  const text = roomShareTextV23(r);
  if(navigator.share) navigator.share({title:r.title,text,url:r.workingLink}).catch(()=>copyRoomLink(id));
  else copyRoomLink(id);
}
function copyGeneratedRoomLink(){
  syncRoomLinkV23();
  ...
  const text=`GARUDA ESP Nusantara Online Room
Room Code: ${code}
Join Link: ${workingRoomURLV23(code)}
Direct Meeting Link: ${realMeetingURLV23(code)}
Professional Identity: ${professionalRoomURLV23(code)}`;
  copyTextV22 ? copyTextV22(text,"Room link copied") : navigator.clipboard.writeText(text);
}
function shareGeneratedRoomLink(){
  syncRoomLinkV23();
  const code=byId("roomCode")?.value || "";
...
Direct Meeting Link: ${realMeetingURLV23(code)}
Professional Identity: ${professionalRoomURLV23(code)}`;
  if(navigator.share) navigator.share({title:"GARUDA ESP Nusantara Online Room",text,url:workingRoomURLV23(code)}).catch(()=>copyGeneratedRoomLink());
  else copyGeneratedRoomLink();
}
function roomRowV18(room){
  const r = normalizeRoomV23(room);
  return `<tr>
    ...
    <td>${esc(r.host)}</td>
    <td>${(r.attendance||[]).length}</td>
    <td>${activeRoomLinkHTMLV23(r)}</td>
    <td class="room-actions">
      <button type="button" class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join Live Room</button>
      <button type="button" class="btn soft" onclick="goToRoomPageV23('${esc(r.id)}')">Open Room</button>
      <button type="button" class="btn soft" onclick="window.open('${esc(r.meetingLink)}','_blank')">Direct Meeting</button>
      <button type="button" class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button type="button" class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
      <button type="button" class="btn soft" onclick="openRoomDetails('${esc(r.id)}')">Details</button>
      ${['admin','lecturer'].includes(session?.role) ? `<button type="button" class="btn soft" onclick="editRoom('${esc(r.id)}')">Edit</button><button type="button" class="btn danger" onclick="deleteRoom('${esc(r.id)}')">Delete</button>` : ""}
...
function renderOpenRoomPanelV23(){
  return `<section class="online18-card room-resolver-v23">
    <div>
      <h3>Open / Join Live Room</h3>
      <p>Paste an RM code or professional room identity. It opens the live room page and launches the meeting area directly.</p>
    </div>
    <div class="room-resolver-row-v23">
      <input id="roomLookupV23" placeholder="RM-260609-URM4 or room.garudaesp.id/RM-260609-URM4">
      <button type="button" class="btn primary" onclick="openRoomFromInputV23()">Join Room</button>
    </div>
  </section>`;
}
function renderOnline(){
  normalizeRoomsV23();
  const manage=['admin','lecturer'].includes(session?.role);
  const rooms=db.rooms.filter(r=>{r=normalizeRoomV23(r);return r.scope==='all'||r.scope===session?.role||!r.scope;});
  byId("mainView").innerHTML=`<section class="online-v18">
    ${renderOpenRoomPanelV23()}
    ${manage ? roomForm() : `<section class="online18-card"><h3>Online Room</h3><p>Students can join live rooms directly, record attendance, use room chat, and access the real meeting area.</p></section>`}
    <section class="online18-card">
      <div class="online18-head">
        <div><h3>Meeting Schedule</h3><p>Join, professional identity, Direct Meeting, Copy, Share, and Details are all connected to the live room.</p></div>
        ${manage ? `<button type="button" class="btn primary" onclick="resetRoomFormV18()">+ Create Room</button>` : ""}
      </div>
      <div class="table-wrap room-table-v18"><table>
        <thead><tr><th>Meeting</th><th>Course & Week</th><th>Date & Time</th><th>Host</th><th>Attendance</th><th>Active Link</th><th>Actions</th></tr></thead>
...
function openRoomDetails(id){
  ...
  openModal(`<h2>${esc(r.title)}</h2>
    <div class="table-wrap"><table><tbody>
      <tr><td><strong>Room Code</strong></td><td>${esc(r.code)}</td></tr>
      <tr><td><strong>Join Link</strong></td><td><a href="${esc(r.workingLink)}">${esc(r.workingLink)}</a></td></tr>
      <tr><td><strong>Direct Meeting</strong></td><td><a href="${esc(r.meetingLink)}" target="_blank">${esc(r.meetingLink)}</a></td></tr>
      <tr><td><strong>Professional Identity</strong></td><td>${esc(r.link)}</td></tr>
      <tr><td><strong>Course</strong></td><td>${esc(courseTitle(r.courseId||"tourism"))} • Week ${esc(r.week)}</td></tr>
      <tr><td><strong>Date & Time</strong></td><td>${esc(roomDateLabel(r))}</td></tr>
      <tr><td><strong>Host</strong></td><td>${esc(r.host)}</td></tr>
    ...
    <div class="actions">
      <button type="button" class="btn primary" onclick="joinRoom('${esc(r.id)}')">Join Live Room</button>
      <button type="button" class="btn soft" onclick="window.open('${esc(r.meetingLink)}','_blank')">Open Direct Meeting</button>
      <button type="button" class="btn soft" onclick="copyRoomLink('${esc(r.id)}')">Copy</button>
      <button type="button" class="btn soft" onclick="shareRoomLink('${esc(r.id)}')">Share</button>
    </div>`);
}

// Any remaining room link or RM code becomes a Join action.
document.addEventListener("click", function(e){
  const el=e.target.closest && e.target.closest("a,button,[data-room-code]");
  if(!el) return;
  const href=el.getAttribute("href")||"";
  ...
  const code=normalizeRoomCodeV23(data||href||text);
  if(code && code.startsWith("RM-") && (data || href.includes("room.garudaesp.id") || href.includes("room.html") || text.includes("room.garudaesp.id"))){
    if(href.includes("room.html")) return;
    e.preventDefault();
    e.stopPropagation();
    openRoomByCodeV23(code);
    return false;
  }
}, true);

function runRoomLinkDiagnosticsV23(){
  normalizeRoomsV23();
  const r = normalizeRoomV23(db.rooms[0]);
  return [
    ["Room exists",!!r],
    ["Code valid",/^RM-\d{6}-[A-Z0-9]{3,12}$/.test(r.code)],
    ["Join page link",r.workingLink.includes("room.html?code=")],
    ["Autostart enabled",r.workingLink.includes("autostart=1")],
    ["Direct meeting link",r.meetingLink.includes("meet.jit.si/GARUDA-ESP-NUSANTARA-RM-")],
    ["Join uses live page",String(joinRoom).includes("goToRoomPageV23")],
    ["Row has Direct Meeting",roomRowV18(r).includes("Direct Meeting")]
  ];
}
Object.assign(window,{
  normalizeRoomCodeV23,meetingSlugV23,realMeetingURLV23,workingRoomURLV23,professionalRoomURLV23,
  normalizeRoomV23,normalizeRoomsV23,findRoomByCodeOrLinkV23,createRecoveredRoomV23,recordAttendanceV23,
  goToRoomPageV23,joinRoom,openRoomByCodeV23,openRoomByCodeV22,openRoomByCodeV21,openRoomByCodeV20,openRoomByCodeV19,
  openRoomFromInputV23,openRoomFromInputV22,openRoomFromInputV21,openRoomFromInputV20,openRoomFromInputV19,
  activeRoomLinkHTMLV23,activeRoomLinkHTMLV22,activeRoomLinkHTMLV21,activeRoomLinkHTMLV20,activeRoomLinkHTMLV19,
  syncRoomLinkV23,syncRoomLinkV22,syncRoomLinkV21,syncRoomLinkV20,syncRoomLinkV18,
  copyRoomLink,shareRoomLink,copyGeneratedRoomLink,shareGeneratedRoomLink,
  roomRowV18,renderOpenRoomPanelV23,renderOnline,openRoomDetails,runRoomLinkDiagnosticsV23
});

...
function setupGarudaMotion(){
  const stage = byId('garudaStage'); const mark = document.querySelector('.garuda-mark.live-5d');
  if(!stage || !mark) return;
  stage.addEventListener('mousemove', e => {
    const r = stage.getBoundingClientRect();
    const x = ((e.clientX - r.left) / r.width - .5) * 12;
    const y = ((e.clientY - r.top) / r.height - .5) * -10;
    mark.style.transform = `translateY(-2px) rotateX(${y}deg) rotateY(${x}deg)`;
  });
  stage.addEventListener('mouseleave', ()=>{ mark.style.transform='translateY(0) rotateX(0deg) rotateY(0deg)'; });
}
